...
app.get('/api/health', async (_req, res) => {
  const [aria2Version, ytDlpVersion] = await Promise.all([
    commandVersion(aria2Bin, ['--version']),
    commandVersion(ytDlpBin, ['--version'])
  ]);

  let aria2RpcReady = false;
  let aria2RpcError = '';
  if (aria2Version.available) {
    try {
  ...
  res.json({
    status: aria2RpcReady || ytDlpVersion.available ? 'connected' : 'degraded',
    name: 'NOVA daemon',
    version: process.env.VITE_APP_VERSION || '0.1.0',
    pid: process.pid,
    engines: {
      aria2: {
        available: aria2Version.available,
        version: aria2Version.version,
        rpcReady: aria2RpcReady,
        rpcPort: aria2RpcPort,
        error: aria2Version.error || aria2RpcError ? sanitizeEngineNames(aria2Version.error || aria2RpcError) : undefined
      },
      ytdlp: {
        available: ytDlpVersion.available,
        version: ytDlpVersion.version,
        error: ytDlpVersion.error ? sanitizeEngineNames(ytDlpVersion.error) : undefined
      }
    }
  });
});
...
function startServer(port: number) {
  ...
  server.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE' && port < requestedPort + 100) {
      console.warn(`[NOVA daemon] port ${port} in use, trying ${port + 1}...`);
      startServer(port + 1);
    } else if (err.code === 'EADDRINUSE') {
      console.error(`[NOVA daemon] could not find a free port after 100 attempts`);
      process.exit(1);
    } else {
...
async function createYtDlpTask(body: CreateDownloadBody): Promise<NovaTask> {
  const version = await commandVersion(ytDlpBin, ['--version']);
...
async function ensureAria2Daemon(): Promise<void> {
  ...
  const version = await commandVersion(aria2Bin, ['--version'], 5000);
  if (!version.available) {
