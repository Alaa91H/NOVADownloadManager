...
Dialog {
    ...
    function buildNetworkOptions() {
        ...
        options.keepaliveTimeSec = Number(a.keepaliveTimeSec || 0)
        options.bufferSize = Math.max(16384, Number(a.bufferSizeKb || 256) * 1024)
        options.insecure = Boolean(a.insecure)

        if (String(a.httpVersion || "").trim().length > 0)
            options.httpVersion = String(a.httpVersion).trim()
        if (String(a.caCert || "").trim().length > 0)
            options.caCert = String(a.caCert).trim()
        if (String(a.clientCert || "").trim().length > 0)
            options.cert = String(a.clientCert).trim()
        if (String(a.clientKey || "").trim().length > 0)
            options.key = String(a.clientKey).trim()
        if (String(a.tlsMin || "").trim().length > 0)
            options.tlsMin = String(a.tlsMin).trim()
        if (String(a.ciphers || "").trim().length > 0)
            options.ciphers = String(a.ciphers).trim()
