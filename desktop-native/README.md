...
- Release language scope is intentionally English + Arabic only, with complete 573/573 key parity between the two native dictionaries, full Arabic RTL behavior, and a CI gate that rejects missing Arabic keys. The legacy multi-language bridge remains dormant for post-completion expansion; System/Light/Dark themes, High Contrast, Reduced Motion and text scaling remain supported.
