...
import { Settings, Sliders, Cpu, Music, Shield, Info, Palette, Search, ChevronDown, ChevronUp } from 'lucide-react';
...
export const SettingsDialog: React.FC = () => {
  const { settings, updateSettings, themeSettings, updateThemeSettings, addToast, t } = useAppStore();
  ...
  const [automationSubTab, setAutomationSubTab] = useState<AutomationSubTab>('telegram');
  const [diagnosticsSubTab, setDiagnosticsSubTab] = useState<DiagnosticsSubTab>('bridge');
  ...
  const mainTabs = [
    ...
    {
      id: 'media_torrent' as const,
      label: t('set_tab_media_torrent'),
      desc: t('set_tab_media_torrent_desc'),
      icon: Music,
    },
    {
  ...
  if (prevActiveTab !== activeTab) {
    ...
    setExpandedTabId(activeTabObj && activeTabObj.subItems ? activeTab : '');
  }
  ...
  return (
    <div className={`flex flex-col h-[75vh] min-h-[550px] max-h-[700px] text-left`} dir={'ltr'}>
      ...
      <div
        className={`flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-[var(--border-color)] pb-3 mb-4 gap-3`}
      >
        <div className="space-y-0.5">
          ...
          <p className="text-[11px] text-[var(--text-secondary)]">{t('set_control_center_desc')}</p>
        </div>
      ...
      <div className="flex flex-row flex-1 min-h-0 overflow-hidden gap-4">
        ...
        >
                  ...
                  >
                    {tab.subItems.map((sub) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${isSubSelected && isSelected ? 'bg-[var(--accent-primary)]' : 'bg-slate-600'}`}
                          />
                          <span>{sub.label}</span>
                        </button>
                      );
          ...
          {filteredTabs.length === 0 && (
            <div className="p-2 text-slate-400 italic text-xs">{'No tabs match your search.'}</div>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-1 pl-1 scrollbar-thin" dir={'ltr'}>
          ...
          {activeTab === 'media_torrent' && (
            <MediaAndTorrent settings={localSettings} updateSetting={updateLocalSetting} onAddToast={addToast} />
          )}
