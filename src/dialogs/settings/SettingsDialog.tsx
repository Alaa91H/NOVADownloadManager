...
import { Settings, Sliders, Cpu, Music, Shield, Info, Palette, Search, ChevronDown, ChevronUp, X, Download, Video } from 'lucide-react';
...
type SettingsDialogPayload = {
  tab?: SettingsTabId;
  subTab?: AutomationSubTab | DiagnosticsSubTab;
};
...
export const SettingsDialog: React.FC = () => {
  ...
  const [expandedTabId, setExpandedTabId] = useState<string>(
    payload.tab === 'integrations_automation' || payload.tab === 'diagnostics_system' ? payload.tab : '',
  );
  ...
  const mainTabs = [
    ...
    { id: 'direct_download' as const, label: t('set_tab_direct_download'), desc: t('set_tab_direct_download_desc'), icon: Download },
    { id: 'media_download' as const, label: t('set_tab_media_download'), desc: t('set_tab_media_download_desc'), icon: Video },
      ...
      <div className="flex flex-row flex-1 min-h-0 overflow-hidden gap-4">
        ...
        <div className="flex-1 overflow-y-auto pr-1 pl-1 scrollbar-thin" dir={'ltr'}>
          ...
          {activeTab === 'media_torrent' && (
            <MediaAndTorrent settings={localSettings} updateSetting={updateLocalSetting} />
          )}
