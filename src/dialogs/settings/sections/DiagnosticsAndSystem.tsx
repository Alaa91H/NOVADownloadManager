...
export const DiagnosticsAndSystem: React.FC<Props> = ({
  ...
  const handleSubTabChange = (tab: 'bridge' | 'diagnostics' | 'backup' | 'advanced') => {
    if (onChangeSubTab) {
      onChangeSubTab(tab);
    } else {
      setLocalSubTab(tab);
  ...
  return (
    <div className="space-y-6 text-left animate-in fade-in duration-200">
      <div className="flex gap-1 border-b border-[var(--border-color)] pb-2 mb-4">
        {(['bridge', 'diagnostics', 'backup', 'advanced'] as const).map((tab) => (
          <button
            key={tab}
            type="button"
            onClick={() => {
            ...
            className={`px-3 py-1.5 text-[11px] font-bold rounded-t transition-all cursor-pointer ${
              subTab === tab
