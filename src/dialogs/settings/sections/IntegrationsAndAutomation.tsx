...
export const IntegrationsAndAutomation: React.FC<Props> = ({
  ...
  const handleTestTelegram = () => {
    ...
    setTimeout(() => {
      setTelegramStatus(settings.extra.tgBotToken && settings.extra.tgChatId ? 'ok' : 'fail');
      if (settings.extra.tgBotToken && settings.extra.tgChatId) {
        onAddToast('success', 'Telegram Test', 'A test message was queued successfully.');
      } else {
        onAddToast('error', 'Telegram Test', 'Enter a bot token and chat ID before testing.');
      }
    }, 800);
  };
