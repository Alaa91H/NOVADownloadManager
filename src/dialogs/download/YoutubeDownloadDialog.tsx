...
import { FolderOpen, Music, Video, Code, Download, AlertCircle, Loader2, CheckCircle2, XCircle, Info, CheckSquare, Square, ListMusic } from 'lucide-react';
...
function bestVideoFormat(formats: YtDlpFormat[], heightLimit?: number): YtDlpFormat | null {
  const candidates = formats.filter(f =>
    f.vcodec && f.vcodec !== 'none' && f.height != null && f.height > 0 &&
...
export const YoutubeDownloadDialog: React.FC = () => {
  ...
  const [savePath, setSavePath] = useState(settings?.saveAndCategories?.categoryFolders?.video || settings?.saveAndCategories?.defaultFolder || '');
  ...
  const [probeResult, setProbeResult] = useState<{ id: string; title: string; duration: number; durationString: string; thumbnail: string; webpageUrl: string; formats: YtDlpFormat[] } | null>(null);
  ...
  useEffect(() => {
    novaClient.checkFfmpeg().then(r => setFfmpegAvailable(r.available)).catch(() => setFfmpegAvailable(false));
  }, []);

  const doProbe = useCallback(async (probeUrl: string) => {
    ...
    if (isPlaylistUrl) {
      ...
      try {
        ...
        const videoFormats = result.formats.filter(f => f.vcodec && f.vcodec !== 'none' && f.height != null && f.height > 0);
        if (videoFormats.length > 0) {
          const best = bestVideoFormat(videoFormats, 1080);
          if (best && best.height) setQuality(`${best.height}p`);
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => doProbe(url), 800);
  ...
  const handleStartDownload = async () => {
    ...
    if (isPlaylistUrl && playlistResult && selectAllPlaylist === false && selectedPlaylistItems.size === 0) {
    ...
    if (isPlaylist && playlistResult && !selectAllPlaylist) {
      playlistItemsStr = Array.from<number>(selectedPlaylistItems).sort((a, b) => a - b).join(',');
    }
  ...
  const requiresFfmpeg = (() => {
    ...
    const selectedFormats = probeResult.formats.filter(f =>
      f.height != null && f.height >= height &&
  ...
  const dynamicQualityOptions = (() => {
    const options: Array<{ value: string; label: string; size: string; sizeBytes: number; needsFfmpeg: boolean; codecInfo: string }> = [];
    options.push({ value: 'best', label: 'Best Quality Available (Recommended)', size: '', sizeBytes: 0, needsFfmpeg: true, codecInfo: '' });
  ...
  const dynamicAudioOptions = (() => {
    ...
    options.push({ value: 'mp3', label: 'MP3 (Best Compatibility)', needsFfmpeg: true, bitrate: '320kbps', sizeBytes: 0 });
    options.push({ value: 'm4a', label: 'M4A (AAC - Original Quality)', needsFfmpeg: false, bitrate: '', sizeBytes: 0 });
    ...
    if (probeResult) {
      const audioOnly = probeResult.formats.filter(f => (f.acodec && f.acodec !== 'none') && (!f.vcodec || f.vcodec === 'none'));
  ...
  return (
    <div className="space-y-4 text-ui text-left" dir="ltr">
      ...
      <div className="space-y-3">
          ...
          <div className="bg-[var(--bg-hover)]/40 border border-[var(--border-color)]/30 rounded-lg p-2.5">
            <div className="flex items-start gap-2.5">
              {probeResult.thumbnail && (
                <img src={probeResult.thumbnail} alt="" className="w-14 h-10 rounded object-cover shrink-0 bg-black/40"
        ...
        {isPlaylistUrl && playlistResult && (
          <div className="bg-[var(--bg-hover)]/40 border border-[var(--border-color)]/30 rounded-lg">
            <div className="flex items-center justify-between p-2.5 border-b border-[var(--border-color)]/20">
              ...
              <button
                type="button"
                onClick={() => { setSelectAllPlaylist(!selectAllPlaylist); if (!selectAllPlaylist) setSelectedPlaylistItems(new Set(playlistResult.entries.map(e => e.index))); }}
              ...
              >
                ...
                {selectAllPlaylist ? 'All selected' : `Selected ${selectedPlaylistItems.size}`}
              </button>
            </div>
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (next.has(entry.index)) next.delete(entry.index); else next.add(entry.index);
                  ...
                  >
                    ...
                    {entry.thumbnail && (
                      <img src={entry.thumbnail} alt="" className="w-10 h-7 rounded object-cover shrink-0 bg-black/40"
                        onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                    )}
              ...
              >
                {dynamicQualityOptions.map(opt => (
                  <option key={opt.value} value={opt.value}>
                    {opt.label}{opt.needsFfmpeg ? ' *needs FFmpeg' : ''}
                  </option>
                ))}
              </select>
            ) : (
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <span className="text-xs font-extrabold text-slate-200 flex items-center gap-1.5">
                <Code className={`w-4 h-4 ${ffmpegAvailable === null ? 'text-slate-400' : ffmpegAvailable ? 'text-emerald-400' : 'text-red-400'}`} />
        ...
        <div className="space-y-2 border-t border-[var(--border-color)]/50 pt-3">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-1">
              <button type="button" onClick={() => handleTemplatePreset('%(title)s.%(ext)s')} className="text-[9px] bg-[var(--bg-hover)] hover:bg-[var(--border-color)] text-slate-400 hover:text-slate-200 px-1.5 py-1 rounded">Video Title</button>
              <button type="button" onClick={() => handleTemplatePreset('%(uploader)s - %(title)s.%(ext)s')} className="text-[9px] bg-[var(--bg-hover)] hover:bg-[var(--border-color)] text-slate-400 hover:text-slate-200 px-1.5 py-1 rounded">Uploader - Title</button>
              <button type="button" onClick={() => handleTemplatePreset('%(playlist_index)s - %(title)s.%(ext)s')} className="text-[9px] bg-[var(--bg-hover)] hover:bg-[var(--border-color)] text-slate-400 hover:text-slate-200 px-1.5 py-1 rounded">Playlist Index</button>
            </div>
          </div>
          <TextField label="" value={outputTemplate} onChange={(e) => setOutputTemplate(e.target.value)} placeholder="%(title)s.%(ext)s" id="yt-template" />
        </div>
        ...
        <div className="space-y-3 border-t border-[var(--border-color)]/50 pt-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            <TextField label="Rate Limit (KB/s)" type="number" value={rateLimitKbs} onChange={(e) => setRateLimitKbs(Number(e.target.value))} />
            <TextField label="Retries" type="number" value={retries} onChange={(e) => setRetries(Number(e.target.value))} />
            <TextField label="Fragment Retries" type="number" value={fragmentRetries} onChange={(e) => setFragmentRetries(Number(e.target.value))} />
            <TextField label="Fragments" type="number" value={concurrentFragments} onChange={(e) => setConcurrentFragments(Number(e.target.value))} />
            <TextField label="Sleep (s)" type="number" value={sleepIntervalSec} onChange={(e) => setSleepIntervalSec(Number(e.target.value))} />
            <TextField label="Max Sleep (s)" type="number" value={maxSleepIntervalSec} onChange={(e) => setMaxSleepIntervalSec(Number(e.target.value))} />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[var(--text-secondary)] text-[10px] md:text-[11px] font-bold">Cookies or Cookies File</label>
              <textarea
                rows={3}
                value={mediaCookies}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[var(--text-secondary)] text-[10px] md:text-[11px] font-bold">Expert Media Arguments</label>
            <textarea
              rows={3}
              value={extraArgs}
      ...
      <div className="flex justify-end gap-2 border-t border-[var(--border-color)] pt-3">
        <DialogButton onClick={closeWithoutKeepingLink} variant="ghost">{t('btn_cancel')}</DialogButton>
        <DialogButton onClick={handleStartDownload} variant="primary" className="flex items-center gap-1.5 font-bold bg-red-600 hover:bg-red-700">
