...
export const AddDownloadDialog: React.FC = () => {
        ...
        try {
          ...
          const probed = await novaClient.probeDownload(url, {
            ...
            directOptions: {
              ...
              sourceAddress: isDirectOptionSupported('sourceAddress') ? configuredSourceAddress || undefined : undefined,
            },
          });
          if (cancelled) return;
  ...
  }, [url, referer, userAgent, headers, cookies, proxy, configuredSourceAddress, settings.saveAndCategories, defaultDownloadsDir, buildSavePath, isDirectOptionSupported]);
  ...
  const handleSubmit = async (downloadImmediately: boolean) => {
    ...
    if (submittedUrl.startsWith('magnet:') || submittedUrl.toLowerCase().endsWith('.torrent')) {
      addToast('error', 'Unsupported link', 'Torrent and magnet links require a dedicated torrent engine. The direct engine is libcurl multi.');
      return;
    }
    ...
    <div className="space-y-4">
      ...
      {showAdvanced && supportedDirectOptions.size > 0 && (
        <div className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-hover)]/30 p-2 text-[10px] text-[var(--text-secondary)]">
          Advanced controls are gated by the linked libcurl capability matrix. Unsupported options are hidden or omitted before the request reaches the daemon.
        </div>
      )}
      ...
      <div className="space-y-1">
        <div className="grid grid-cols-[1fr_110px] gap-3 items-center">
          ...
          {(() => {
            ...
            return (
              <div
                className={`flex items-center justify-center shrink-0 select-none text-center h-[26px] px-2.5 rounded-full border ${borderColor} transition-all duration-300`}
                title={isFetchingInfo ? 'Calculating size...' : 'File size'}
              >
      ...
      <div className="flex justify-between items-center pt-3 mt-2.5 border-t border-[var(--border-color)]">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={!url || isFetchingInfo || !directEngineReady}
          >
            {t('add_dl_queue_only')}
          </button>
