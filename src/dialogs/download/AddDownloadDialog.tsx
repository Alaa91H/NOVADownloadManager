...
import { Globe, FolderOpen, Sliders, Calendar, Youtube, ArrowRight } from 'lucide-react';
...
export const AddDownloadDialog: React.FC = () => {
  ...
  // Read URL from drag-and-drop payload if provided
  useEffect(() => {
    if (dialog.payload) {
  ...
  const formatBytes = (bytes: number) => {
    ...
    let formattedVal = parseFloat(temp.toFixed(2));
    if (formattedVal >= k && i < sizes.length - 1) {
      formattedVal = parseFloat((formattedVal / k).toFixed(2));
      i++;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        <div className="relative">
          <input
            ...
            className="w-full bg-[var(--bg-input)] border border-[var(--border-color)] rounded text-[11px] font-mono transition-all focus:border-[var(--accent-primary)] focus:outline-none py-1.5 px-2.5 pr-12 text-left text-[var(--text-primary)]"
            style={{ direction: 'ltr' }}
            autoFocus
          />
          <button 
      ...
      <div className="grid grid-cols-[1fr_110px] gap-3 items-center">
        {/* Destination folder selector with Folder icon and chevron */}
        <div className="relative">
          <input 
            type="text" 
            value={savePath} 
        ...
        <div className="p-2.5 bg-[var(--bg-sidebar)] border border-[var(--border-color)] rounded space-y-2.5 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <SelectField 
              ...
              onChange={(e) => {
                ...
                setCategory(newCat);
                setFileType(newCat);
                const configuredFolder = settings?.saveAndCategories?.categoryFolders?.[newCat] || `C:\\Users\\User\\Downloads\\NOVA\\${newCat.charAt(0).toUpperCase() + newCat.slice(1)}`;
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <SelectField
              ...
              onChange={(e) => setFileAllocation(e.target.value as any)}
              options={[
