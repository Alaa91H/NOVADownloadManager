          ...
          <div className="border-t border-[var(--border-color)]/40 pt-3 mt-1">
            ...
            >
              ...
              <span className="text-[10px] text-[var(--text-muted)] group-hover:text-[var(--text-secondary)] transition-colors">
                {showOverrideSection ? '▾ Hide All' : '▸ Show Advanced Overrides'}
              </span>
            </button>
            ...
            {showOverrideSection && (
              <div className="space-y-3 mt-2 animate-in fade-in duration-150">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <SelectField
                      ...
                      options={[
                        ...
                        { value: 'negotiate', label: 'Negotiate (Kerberos)' },
                        { value: 'any', label: 'Any (auto-negotiate)' },
                      ]}
                      disabled={!isDirectOptionSupported('authType')}
                    />
                    <TextField label="Username" value={authUsername} onChange={(e) => setAuthUsername(e.target.value)} placeholder="username" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('username')} />
                    <TextField label="Password" value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} placeholder="password" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('password')} />
                    <TextField label="OAuth2 Bearer" value={oauth2Bearer} onChange={(e) => setOauth2Bearer(e.target.value)} placeholder="eyJhbGciOi..." className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('oauth2Bearer')} />
                    <TextField label="Proxy Username" value={proxyUser} onChange={(e) => setProxyUser(e.target.value)} placeholder="proxy-user" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('proxyUser')} />
                    <TextField label="Proxy Password" value={proxyPassword} onChange={(e) => setProxyPassword(e.target.value)} placeholder="proxy-pass" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('proxyPassword')} />
                    <div className="flex items-center gap-6 pt-5 md:col-span-2">
                      <Checkbox label="Send auth on cross-host redirects" checked={unrestrictedAuth} onChange={setUnrestrictedAuth} disabled={!isDirectOptionSupported('unrestrictedAuth')} />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[var(--bg-input)]/30 rounded-lg p-3 border border-[var(--border-color)]/30">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <SelectField label="HTTP Version" value={httpVersion} onChange={(e) => setHttpVersion(e.target.value)} options={[{ value: '', label: 'Auto' }, { value: '1.0', label: 'HTTP/1.0' }, { value: '1.1', label: 'HTTP/1.1' }, { value: '2', label: 'HTTP/2' }, { value: '2-prior-knowledge', label: 'HTTP/2 Prior Knowledge' }, { value: '3', label: 'HTTP/3' }]} disabled={!isDirectOptionSupported('httpVersion')} />
                    <SelectField label="TLS Min Version" value={tlsMin} onChange={(e) => setTlsMin(e.target.value)} options={[{ value: '', label: 'Default' }, { value: '1.0', label: 'TLS 1.0' }, { value: '1.1', label: 'TLS 1.1' }, { value: '1.2', label: 'TLS 1.2' }, { value: '1.3', label: 'TLS 1.3' }]} disabled={!isDirectOptionSupported('tlsMin')} />
                    <div className="flex items-center gap-6 pt-5"><Checkbox label="Skip TLS verification (insecure)" checked={insecure} onChange={setInsecure} disabled={!isDirectOptionSupported('insecure')} /></div>
                    <TextField label="CA Certificate Path" value={caCert} onChange={(e) => setCaCert(e.target.value)} placeholder="/path/to/cacert.pem" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('caCert')} />
                    <TextField label="Client Certificate" value={clientCert} onChange={(e) => setClientCert(e.target.value)} placeholder="/path/to/client.crt" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('cert')} />
                    <TextField label="Client Key" value={clientKey} onChange={(e) => setClientKey(e.target.value)} placeholder="/path/to/client.key" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('key')} />
                    <TextField label="Cipher Suites" value={tlsCiphers} onChange={(e) => setTlsCiphers(e.target.value)} placeholder="ECDHE+AESGCM:!aNULL" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('ciphers')} />
                  </div>
                </div>
                ...
                <div className="bg-[var(--bg-input)]/30 rounded-lg p-3 border border-[var(--border-color)]/30">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <TextField label="Max Redirects" type="number" value={maxRedirs} onChange={(e) => setMaxRedirs(Number(e.target.value))} disabled={!isDirectOptionSupported('maxRedirs')} />
                    <TextField label="Keepalive Interval (s)" type="number" value={keepaliveTimeSec} onChange={(e) => setKeepaliveTimeSec(Number(e.target.value))} disabled={!isDirectOptionSupported('keepaliveTimeSec')} />
                    <div className="flex items-center gap-6 pt-5"><Checkbox label="TCP No-Delay" checked={tcpNoDelay} onChange={setTcpNoDelay} disabled={!isDirectOptionSupported('tcpNoDelay')} /></div>
                    <TextField label="DNS Servers" value={dnsServers} onChange={(e) => setDnsServers(e.target.value)} placeholder="1.1.1.1, 8.8.8.8" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('dnsServers')} />
                    <TextField label="No-Proxy Bypass" value={noproxy} onChange={(e) => setNoproxy(e.target.value)} placeholder="localhost, 127.0.0.1, .local" className="font-mono" style={{ direction: 'ltr', textAlign: 'left' }} disabled={!isDirectOptionSupported('noproxy')} />
                    <div className="flex items-center gap-6 pt-5 md:col-span-2">
                      <Checkbox label="Force fresh connection" checked={freshConnect} onChange={setFreshConnect} disabled={!isDirectOptionSupported('freshConnect')} />
                      <Checkbox label="Forbid connection reuse" checked={forbidReuse} onChange={setForbidReuse} disabled={!isDirectOptionSupported('forbidReuse')} />
                    </div>
                  </div>
                </div>
              </div>
