...
import { Youtube, ArrowRight, Magnet } from 'lucide-react';
...
export const AddDownloadDialog: React.FC = () => {
  ...
  useEffect(() => {
    let cancelled = false;
    tauriClient.getDownloadsDir().then(dir => {
  ...
  const buildSavePath = useCallback((type: FileType, name: string) => {
    const folder = settings?.saveAndCategories?.categoryFolders?.[type] || settings?.saveAndCategories?.defaultFolder || defaultDownloadsDir || '';
