...
import { Globe, Link2, AlertCircle, Chrome, FileEdit, ArrowRight } from 'lucide-react';
...
export const UpdateLinkDialog: React.FC = () => {
  ...
  const task: DownloadItem | undefined = dialog.payload;
  ...
  const handleUpdateManual = () => {
    if (!newUrl.trim().startsWith('http')) {
      addToast('error', t('toast_error_title'), 'Enter a valid direct download link.');
      return;
    }

    updateTaskProperties(task.id, { url: newUrl.trim() });
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 bg-[var(--bg-hover)]/20 p-3 rounded-lg border border-[var(--border-color)]/30">
        ...
        <div>
          ...
          <div className="bg-[var(--bg-input)]/50 text-[var(--text-muted)] px-3 py-1.5 rounded border border-[var(--border-color)]/30 text-[10px] font-mono truncate select-all" style={{ direction: 'ltr' }}>
            {task.url}
          </div>
        </div>
      </div>
      ...
      {mode === 'browser' ? (
        <div className="bg-emerald-500/5 border border-emerald-500/20 p-4 rounded-lg space-y-3">
          <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium leading-relaxed">
            NOVA will open the source page in your system browser. Paste the renewed direct link in the manual tab afterward.
          </p>
