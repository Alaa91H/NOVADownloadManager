...
export const Modal: React.FC<ModalProps> = ({
  ...
  size = 'md',
  isDirty = false,
  dirtyMessage = 'You have unsaved changes. Close anyway?',
  role = 'dialog',
  id
}) => {
  const { dialog, setActiveProgressMinimizedToTaskbar, setMinimizedProgressTask, closeDialog } = useAppStore();
  ...
  // Reset position and minimized state when modal opens or when maximized toggled
  useEffect(() => {
    if (isOpen) {
      setPosition({ x: 0, y: 0 });
  ...
  return (
    ...
    >
      <div 
        ...
        tabIndex={-1}
        style={{ 
          transform: isMinimized ? 'none' : isMaximized ? 'none' : `translate(${position.x}px, ${position.y}px)`,
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            {id === 'active-progress-modal' ? (
              <svg className="w-3.5 h-3.5 text-emerald-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ animation: 'spin 12s linear infinite' }}>
            ...
            <h3 id="modal-title" className="text-[10px] md:text-[11px] font-bold text-[var(--text-primary)] font-sans tracking-wide truncate select-all" style={{ direction: 'ltr' }}>
              {isMinimized ? `[Minimized] ${title}` : title}
            </h3>
          </div>

          {/* Right Side: Windows-Style Window Controls (Ordered: Minimize, Maximize, Close) */}
          <div className="flex items-center gap-0.5" style={{ direction: 'ltr' }} onClick={(e) => e.stopPropagation()}>
            {id === 'active-progress-modal' && (
              <button 
            ...
            <button 
              ...
              title="Close"
            >
              <svg viewBox="0 0 24 24" fill="none" className="w-2.5 h-2.5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
        ...
        {!isMinimized && (
          <div className="flex-1 overflow-y-auto p-2.5 md:p-3 bg-[var(--bg-surface-elevated)]">
            {children}
          </div>
        )}
      </div>
