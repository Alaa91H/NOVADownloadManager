...
export function useColumnState() {
  ...
  const handleDragOver = (e: React.DragEvent, colKey: string) => {
    // preventDefault must run on every dragover for the drop to be accepted.
    // Key it off the drag payload type (available synchronously during the
    // drag) rather than the async `draggingCol` state, which may not have
    // committed yet on the first dragover events — the cause of drops silently
    // being rejected.
    if (draggingCol !== colKey && e.dataTransfer.types.includes('application/x-nova-column')) {
      e.preventDefault();
      e.stopPropagation();
    }
  };
