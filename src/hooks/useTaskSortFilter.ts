...
import type { DownloadItem, FileType } from '../types/desktop-ui.types';
import type { SortColumn } from '../utils/taskTableUtils';

type WorkspaceView = 'all' | 'unfinished' | 'finished' | 'queued' | FileType | 'browser' | 'scheduler' | 'diagnostics';

export function useTaskSortFilter(tasks: DownloadItem[], searchQuery: string, workspaceView: WorkspaceView) {
  ...
  const sortedTasks = useMemo(() => {
    return [...filteredTasks].sort((a, b) => {
      ...
      if (sortBy === 'smartCategory') {
        const aStr = typeof valA === 'string' ? valA : '';
        const bStr = typeof valB === 'string' ? valB : '';
