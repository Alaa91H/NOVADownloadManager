...
export const TaskTable: React.FC = () => {
  ...
  const buildContextMenuOptions = (task: DownloadItem): ContextMenuOption[] => {
    ...
    if (task.status === 'paused' || task.status === 'error') {
      opts.push({
        id: 'resume',
        label: task.status === 'error' ? 'Retry Download' : 'Resume',
  ...
  const handleBatchDelete = () => {
    ...
    if (!confirm(`Are you sure you want to delete ${String(ids.length)} files from the download list?`)) return;
        ...
        <div className="sticky top-0 z-30 px-3 py-2 bg-[var(--accent-primary)]/10 border-b border-[var(--accent-primary)]/20 flex items-center gap-2">
          ...
          <div className="flex gap-1 ml-auto">
            <button
              ...
              className="px-2 py-1 text-[10px] font-bold bg-emerald-500/20 text-emerald-500 rounded hover:bg-emerald-500/30 transition-all cursor-pointer inline-flex items-center gap-1"
            >
              <Play className="w-3 h-3" /> Resume
            </button>
            <button
              onClick={handleBatchPause}
              className="px-2 py-1 text-[10px] font-bold bg-amber-500/20 text-amber-500 rounded hover:bg-amber-500/30 transition-all cursor-pointer inline-flex items-center gap-1"
            >
            ...
            <button
              ...
              className="px-2 py-1 text-[10px] font-bold bg-red-500/20 text-red-500 rounded hover:bg-red-500/30 transition-all cursor-pointer inline-flex items-center gap-1"
            >
                      ...
                      case 'connections':
                        return (
                          <td
                            ...
                            style={{ width }}
                          >
                            {task.connections === 0 ? `Auto (${String(task.segments.length || 8)})` : task.connections}
                          </td>
                        );
                      case 'crc32':
                        return (
