...
import { AlertCircle, CheckCircle, Info, X, RefreshCw, Minus, Square, X as XIcon } from 'lucide-react';
...
export const AppShell: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void pollClipboard();
    const interval = window.setInterval(pollClipboard, 1500);
    return () => {
      cancelled = true;
      window.clearInterval(interval);
    };
  }, [settings.general.monitorClipboard, bridge.status, dialog.active]);
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (!droppedText && e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      addToast('warning', 'Local file ignored', `"${file.name}" is already local; drop a real URL to create a download.`);
      return;
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen w-screen bg-[var(--bg-app)] text-[var(--text-primary)] gap-6">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          {connectTimer > 25 && (
            <button
              onClick={() => { setConnectTimer(0); window.location.reload(); }}
  ...
  return (
    ...
    >
      ...
      {isDraggingOver && (
        <div 
          className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-slate-950/85 backdrop-blur-md border-4 border-dashed border-[var(--accent-primary)] animate-in fade-in duration-200"
        >
      ...
      {/* Custom Title Bar */}
      <div data-tauri-drag-region className="bg-[var(--bg-sidebar)] h-8 px-2 flex items-center justify-between text-[11px] font-semibold text-[var(--text-secondary)] select-none shrink-0" style={{ direction: 'ltr' }}>
        <div data-tauri-drag-region className="flex items-center gap-2">
          <Logo size={16} />
          <span className="text-[11px] font-bold text-[var(--text-primary)]">NOVA Download Manager</span>
        </div>
        <div className="flex items-center h-full">
          <button onClick={() => { if (isTauri()) { getCurrentWindow().minimize(); } }} className="h-full px-3 hover:bg-[var(--bg-hover)] transition-colors flex items-center justify-center cursor-pointer" title="Minimize">
            <Minus className="w-3 h-3 text-[var(--text-secondary)]" />
          </button>
          <button onClick={() => { if (isTauri()) { getCurrentWindow().toggleMaximize(); } }} className="h-full px-3 hover:bg-[var(--bg-hover)] transition-colors flex items-center justify-center cursor-pointer" title="Maximize">
            <Square className="w-3 h-3 text-[var(--text-secondary)]" />
          </button>
          <button onClick={() => { if (isTauri()) { getCurrentWindow().close(); } }} className="h-full px-3 hover:bg-red-500 hover:text-white transition-colors flex items-center justify-center cursor-pointer" title="Close">
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 max-w-sm select-none" style={{ direction: 'ltr' }}>
        {toasts.map(toast => (
          <div 
            key={toast.id}
