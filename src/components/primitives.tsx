...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  const baseStyle = 'interactive-btn inline-flex items-center gap-2 font-medium justify-center transition-all disabled:opacity-50 disabled:pointer-events-none';
  ...
  const variantStyles = {
    primary: 'bg-[var(--accent-primary)] hover:bg-[var(--accent-hover)] text-white shadow-sm hover:shadow accent-glow border border-[var(--accent-border)]',
    secondary: 'bg-[var(--bg-hover)] text-[var(--text-primary)] hover:bg-[var(--border-color-hover)] border border-[var(--border-color)]',
...
export const IconButton: React.FC<IconButtonProps> = ({ 
  ...
  const variantStyles = {
    primary: 'bg-[var(--accent-primary)] text-white hover:bg-[var(--accent-hover)] border border-[var(--accent-border)] shadow-sm',
    secondary: 'bg-[var(--bg-hover)] text-[var(--text-primary)] hover:bg-[var(--border-color-hover)] border border-[var(--border-color)]',
...
export const TextField: React.FC<TextFieldProps> = ({ 
  ...
  return (
    <div className="flex flex-col gap-0.5 w-full text-ui">
      {label && <label htmlFor={id} className="text-[var(--text-secondary)] text-[10px] md:text-[11px] font-bold">{label}</label>}
...
export const SelectField: React.FC<SelectFieldProps> = ({ 
  ...
  return (
    <div className="flex flex-col gap-0.5 w-full text-ui">
      ...
      >
        {options.map(opt => (
          <option key={opt.value} value={opt.value} className="bg-[var(--bg-surface-elevated)] text-[var(--text-primary)]">
            {opt.label}
          </option>
        ))}
      </select>
...
export const Switch: React.FC<SwitchProps> = ({ checked, onChange, label, id }) => {
  return (
    <label id={id} className={`inline-flex items-center justify-between gap-2.5 cursor-pointer select-none text-ui ${label ? 'w-full' : ''}`}>
  ...
  return (
    <label id={id} className="inline-flex items-center gap-1.5 cursor-pointer select-none text-ui">
      <input 
        ...
        {...props}
      />
      <span className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors text-[11px] font-semibold">{label}</span>
    </label>
  );
};
...
export const FormRow: React.FC<FormRowProps> = ({ label, children, id }) => {
  return (
    <div id={id} className="flex flex-col gap-1.5 border-b border-[var(--border-color)]/50 py-2 density-py">
      <span className="text-[11px] md:text-xs font-bold text-[var(--text-secondary)] block w-full" dir="auto">{label}</span>
      <div className="w-full flex justify-start" dir="auto">{children}</div>
    </div>
  );
};
...
export const Tabs: React.FC<TabsProps> = ({ options, activeId, onChange, id }) => {
  return (
    <div id={id} className="flex gap-1 bg-[var(--bg-hover)] p-1 rounded-lg border border-[var(--border-color)] overflow-x-auto">
      {options.map(opt => {
        const Icon = opt.icon;
...
export const StatusPill: React.FC<StatusPillProps> = ({ status }) => {
  ...
  const config = meta[status] || { bg: 'bg-gray-500/10 text-gray-400 border-gray-500/20', text: 'Unknown' };
  
  return (
    <span className={`inline-flex items-center px-2 py-0.5 text-[10px] md:text-xs font-medium rounded-md border ${config.bg}`}>
      {config.text}
    </span>
  );
};
