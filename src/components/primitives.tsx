...
export const StatusPill: React.FC<StatusPillProps> = ({ status }) => {
  ...
  const meta: Record<DownloadStatus, { bg: string; key: string }> = {
    downloading: { bg: 'bg-blue-500/10 text-blue-500 border-blue-500/20', key: 'status_downloading' },
    completed: { bg: 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20', key: 'status_completed' },
    paused: { bg: 'bg-amber-500/10 text-amber-500 border-amber-500/20', key: 'status_paused' },
    // Transient states reuse the closest stable visual and label.
    pausing: { bg: 'bg-amber-500/10 text-amber-500 border-amber-500/20', key: 'status_paused' },
    stopping: { bg: 'bg-amber-500/10 text-amber-500 border-amber-500/20', key: 'status_paused' },
    queued: { bg: 'bg-slate-500/10 text-slate-400 border-slate-500/20', key: 'status_queued' },
    error: { bg: 'bg-rose-500/10 text-rose-500 border-rose-500/20', key: 'status_error' },
  };
