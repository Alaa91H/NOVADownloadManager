...
import { 
  Plus, Layers, Play, Pause, Square, Trash2, Settings, Cpu, Search, 
  Menu, Eye, EyeOff, Info, HelpCircle, Shield, Sliders, Gauge, Clock, Bell, Globe, Youtube, ChevronDown
} from 'lucide-react';
...
export const TopBar: React.FC = () => {
  ...
  const canResumeAll = !hasSelectedTask && tasks.some(t => t.status === 'paused' || t.status === 'failed' || t.status === 'queued');
  const isResumeDisabled = hasSelectedTask ? !canResumeSelected : !canResumeAll;

  // Pause State
  const canPauseSelected = hasSelectedTask && selectedTask && selectedTask.status === 'downloading';
  const canPauseAll = !hasSelectedTask && tasks.some(t => t.status === 'downloading');
  const isPauseDisabled = hasSelectedTask ? !canPauseSelected : !canPauseAll;

  // Stop State
  const canStopSelected = hasSelectedTask && selectedTask && selectedTask.status === 'downloading';
  const canStopAll = !hasSelectedTask && tasks.some(t => t.status === 'downloading');

  // Delete State
  const canDeleteSelected = hasSelectedTask; 
  const canDeleteAll = !hasSelectedTask && tasks.length > 0;
  const isDeleteDisabled = hasSelectedTask ? !canDeleteSelected : !canDeleteAll;
