...
export const TopBar: React.FC = () => {
  ...
  // Pause State
  const canPauseSelected = !!selectedTask && selectedTask.status === 'downloading';
  ...
  const handleResumeAll = () => {
    ...
    addToast('success', 'Resume All', `Resuming download of ${String(inactive.length)} files.`);
  };
  ...
  const handlePauseAll = () => {
    ...
    addToast('warning', 'Pause All', `Paused all active downloads (${String(active.length)} files).`);
  };
  ...
  const handleStopAll = () => {
    ...
    addToast('warning', 'Stop All', `Stopped all active downloads (${String(active.length)} files).`);
  };
  ...
  const handleDeleteAll = async () => {
    ...
    addToast('warning', 'Delete All', `Deleted all files from download list (${String(tasks.length)} files).`);
  };
      ...
      <div className="flex flex-nowrap items-center gap-1.5 shrink-0">
        ...
        <div className="relative">
          <div className="flex items-stretch rounded-lg bg-[var(--accent-primary)] hover:scale-[1.03] active:scale-[0.97] transition-all duration-150 shadow-md accent-glow select-none shrink-0 overflow-hidden">
            ...
            >
              <ChevronDown
                className={`w-3.5 h-3.5 transition-transform duration-200 ${isNewDownloadDropdownOpen ? 'rotate-180' : ''}`}
              />
            </button>
          </div>
