  ...
  if (queues.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[75vh] gap-3">
        <p className="text-xs text-[var(--text-secondary)]">No queues available.</p>
      </div>
    );
  }
    ...
    <div className={`flex flex-col h-[75vh] min-h-[550px] max-h-[700px] text-left`} dir={'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-[var(--border-color)] pb-3 mb-4 gap-3">
        <div className="space-y-0.5">
          ...
          <p className="text-[11px] text-[var(--text-secondary)]">{t('sched_desc')}</p>
        </div>
              ...
              <div className="max-w-2xl mx-auto w-full space-y-4 overflow-y-auto pr-1 pl-1 flex-1 pb-4">
                {activeTab === 'basic' && (
                  <SchedulerBasicTab
                    ...
                    onScheduleTypeChange={setSmartScheduleType}
                    isScheduled={isScheduled}
        ...
        <div className="flex gap-2">
          {activeTab === 'files' ? (
            <>
              ...
              <DialogButton onClick={closeDialog} variant="secondary" size="md">
                {'Close'}
              </DialogButton>
            </>
          ) : (
            <DialogButton onClick={closeDialog} variant="primary" size="md">
              {'Close'}
            </DialogButton>
          )}
        </div>
