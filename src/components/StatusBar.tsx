...
export const StatusBar: React.FC = () => {
  ...
  const [browserHealth, setBrowserHealth] = useState<BrowserExtensionHealth | null>(null);

  const statusVisible = (id: keyof typeof settings.ui.statusBar) => settings.ui.statusBar[id].visible;

  // Poll browser extension health to reflect actual connection status
  const isAnyBrowserEnabled = Object.values(settings.general.integrateWithBrowsers).some((val) => val);
  const healthTimerRef = useRef<number | null>(null);
  useEffect(() => {
    if (!isAnyBrowserEnabled) {
      return;
    }
  ...
  // Yellow: enabled but not paired / incomplete health
  // Red: all disabled or no health
  const browserIconState: 'connected' | 'degraded' | 'disconnected' = useMemo(() => {
      ...
      <div className="flex items-center gap-2 min-w-0 overflow-x-auto scrollbar-none">
        ...
        {statusVisible('speed') && (
          <div className="flex items-center gap-1.5" title={t('statusbar_speed_tip')}>
            <Activity className="w-3.5 h-3.5 text-[var(--success)] animate-pulse shrink-0" />
        ...
        {statusVisible('counts') && (
          <div className="flex items-center gap-1.5" title={t('statusbar_counts_tip')}>
            <Download className="w-3.5 h-3.5 text-[var(--info)] shrink-0" />
            <span>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {!statusVisible('daemon') && isDegradedMode && (
          <button
            ...
            className="p-1.5 hover:bg-[var(--bg-hover)] rounded transition-all cursor-pointer flex items-center justify-center text-[var(--warning)] animate-pulse"
            ...
            <button
              ...
              className={`p-1.5 rounded-lg cursor-pointer transition-all flex items-center justify-center hover:bg-[var(--bg-hover)] ${
                settings.connection.speedLimiter.enabled
                  ? 'text-[var(--warning)] drop-shadow-[0_0_4px_rgba(245,158,11,0.3)] font-bold'
                  : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'
              }`}
        ...
        <>
          ...
          >
            ...
            >
              ...
              {settings.extra.tgEnabled && <Check className="w-3.5 h-3.5 text-[var(--success)]" />}
            </button>
            <button
              onClick={() => {
                void testTelegram();
        ...
        <>
          ...
          >
            <div className="px-3 py-1.5 text-[10px] text-[var(--text-secondary)] border-b border-[var(--border-color)] font-extrabold text-center break-words">
              ...
              {settings.connection.speedLimiter.enabled && (
                <div className="text-[var(--warning)] font-bold mt-0.5 text-[10px] leading-tight break-words">
            ...
            >
              <div className="flex items-center gap-2">
                <Gauge className="w-3.5 h-3.5 text-[var(--warning)] shrink-0" />
                <span>{t('statusbar_enable_limiter')}</span>
              </div>
              {settings.connection.speedLimiter.enabled && <Check className="w-3.5 h-3.5 text-[var(--success)]" />}
            </button>
            ...
            {[
              ...
              return (
                ...
                >
                  ...
                  {isActive && <Check className="w-3.5 h-3.5 text-[var(--success)]" />}
                </button>
              );
            })}
