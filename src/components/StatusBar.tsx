...
import { Bell, BellOff, Gauge, Shield, Wifi, WifiOff, Download, Activity, HelpCircle, Check } from 'lucide-react';
...
export const StatusBar: React.FC = () => {
  ...
  const minimizedProgressPercent = minimizedRealTask && minimizedRealTask.sizeBytes > 0
  ...
  const isExtensionConnected = Object.values(settings.general.integrateWithBrowsers).some(val => val === true);

  // 3. Toggle Speed Limiter
  const handleToggleSpeedLimiter = () => {
  ...
  return (
    <footer 
      className="bg-[var(--bg-sidebar)] border-t border-[var(--border-color)] h-9 px-4 flex items-center justify-between gap-4 text-[11px] font-semibold text-[var(--text-secondary)] select-none shrink-0" 
      style={{ direction: 'ltr' }}
    >
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button 
          onClick={() => openDialog('browserIntegration')}
        ...
        <button 
          ...
          className={`p-1.5 hover:bg-[var(--bg-hover)] rounded transition-all cursor-pointer relative flex items-center justify-center ${
            isNotificationsMuted ? 'text-rose-500 hover:text-rose-400' : 'text-[var(--text-secondary)] hover:text-amber-500'
          }`}
          title={isNotificationsMuted ? "Notifications are muted (click to enable)" : "Notifications are active (click to mute)"}
        >
          {isNotificationsMuted ? (
      ...
      {speedMenuVisible && (
        <>
          ...
          <div 
            ...
            onClick={() => setSpeedMenuVisible(false)}
          />
          {/* Context Menu Dropup */}
          <div 
            style={{ 
              position: 'fixed', 
              bottom: '42px',
              left: `${speedMenuCoords.x}px`,
            }}
            className="z-[101] min-w-[190px] bg-[var(--bg-surface-elevated)] border border-[var(--border-color)] rounded-lg shadow-2xl py-1 text-left animate-in fade-in slide-in-from-bottom-2 duration-100 font-bold"
            dir="ltr"
          >
            <div className="px-3 py-1.5 text-[10px] text-[var(--text-secondary)] border-b border-[var(--border-color)] font-extrabold text-center">
              Speed Limit
              {settings.connection.speedLimiter.enabled && (
            ...
            {[
              ...
              const isActive = settings.connection.speedLimiter.enabled && settings.connection.speedLimiter.maxSpeedKbs === preset.value;
              return (
                <button
                  key={preset.value}
                  onClick={() => {
