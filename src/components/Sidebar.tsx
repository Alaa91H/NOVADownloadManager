...
import { 
  Download, FileText, Cpu, Sliders, Film, Music, Shield, Globe, 
  Settings, Layers, Clock, AlertCircle, BarChart3, Moon, Sun, 
  Tv, Eye, ChevronLeft, ChevronRight, CheckCircle, RefreshCw
} from 'lucide-react';
import { useAppStore } from '../state/appStore';
import { FileType, AppThemeSettings } from '../types/desktop-ui.types';
...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside 
      className="w-64 bg-[var(--bg-sidebar)] border-x border-[var(--border-color)] h-full flex flex-col justify-between select-none p-4 overflow-y-auto shrink-0 transition-all duration-300 fixed md:static inset-y-0 relative z-40 md:z-auto shadow-2xl md:shadow-none"
    >
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-[var(--accent-primary)] font-extrabold uppercase font-mono tracking-wider">NOVA Engine</p>
          </div>
        </div>

        {/* Local Daemon Bridge Connection Widget */}
        <div 
          onClick={() => openDialog('diagnostics')}
        ...
        >
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${bridge.status === 'connected' ? 'bg-emerald-500 animate-pulse' : 'bg-orange-500'}`} />
            <div className="text-[10px]">
              <span className="font-semibold block text-[var(--text-primary)]">{bridge.status === 'connected' ? t('daemon_bridge_connected') : t('daemon_bridge_disconnected')}</span>
              <span className="text-[9px] text-[var(--text-muted)] font-mono">{bridge.version || t('daemon_bridge_disconnected')}</span>
            </div>
          </div>
        ...
        <div className="space-y-0.5">
          <h3 className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] px-1">{t('download_lists')}</h3>
          ...
          >
            <span className="flex items-center gap-1.5"><Layers className="w-3.5 h-3.5" /> {t('all_downloads')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono font-bold text-[var(--text-secondary)]">{getTaskCount('all')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {t('downloading')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono font-bold text-[var(--text-secondary)]">{getTaskCount('unfinished')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5"><CheckCircle className="w-3.5 h-3.5" /> {t('completed')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono font-bold text-[var(--text-secondary)]">{getTaskCount('finished')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {t('queued_downloads')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono font-bold text-[var(--text-secondary)]">{getTaskCount('queued')}</span>
          </button>
        </div>

        {/* File category lists */}
        <div className="space-y-0.5">
          <h3 className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] px-1">{t('categories')}</h3>
          
          <button 
            onClick={() => handleFilterClick('compressed')}
          ...
          >
            <span className="flex items-center gap-1.5"><Sliders className="w-3.5 h-3.5 text-amber-500" /> {t('compressed')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono text-[var(--text-muted)]">{getTaskCount('compressed')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5"><Cpu className="w-3.5 h-3.5 text-emerald-500" /> {t('programs')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono text-[var(--text-muted)]">{getTaskCount('program')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5"><Film className="w-3.5 h-3.5 text-sky-500" /> {t('videos')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono text-[var(--text-muted)]">{getTaskCount('video')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5"><Music className="w-3.5 h-3.5 text-violet-500" /> {t('audio')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono text-[var(--text-muted)]">{getTaskCount('audio')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-1.5"><FileText className="w-3.5 h-3.5 text-red-500" /> {t('documents')}</span>
            <span className="bg-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] font-mono text-[var(--text-muted)]">{getTaskCount('document')}</span>
          </button>
        </div>
        ...
        <div className="space-y-0.5">
          ...
          >
            <span className="flex items-center gap-1.5"><Globe className="w-3.5 h-3.5 text-sky-500" /> System Web Browser</span>
            <span className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 px-1 py-0.5 rounded text-[9px] font-bold">OS</span>
          </button>
        </div>
        ...
        <div className="space-y-0.5">
          ...
          <button 
            onClick={() => handleFilterClick('scheduler')}
            className={`w-full flex items-center justify-between px-2 py-1 rounded text-[11px] transition-all ${dialog.active === 'scheduler' ? 'bg-[var(--bg-selected)] text-[var(--accent-primary)] font-bold' : 'text-[var(--text-secondary)] hover:bg(--bg-hover)'}`}
          >
            <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-amber-500 animate-pulse" /> {t('organize_scheduler')}</span>
            <span className="bg-amber-500/10 border border-amber-500/20 text-amber-500 px-1 py-0.5 rounded text-[9px] font-bold">{t('active_indicator')}</span>
          </button>
        </div>
      ...
      <div className="pt-4 border-t border-[var(--border-color)] space-y-3 mt-4 text-[11px]">
        ...
        <div className="flex items-center justify-between">
          ...
          <select 
            ...
            onChange={(e) => updateThemeSettings('density', e.target.value)}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1">
            {accents.map(acc => (
              <button 
                key={acc.id}
                onClick={() => updateThemeSettings('accent', acc.id)}
