...
export const SpeedLimitInput: React.FC<SpeedLimitInputProps> = ({ maxSpeedKbs, onChange, compact = false }) => {
  ...
  const [inputValue, setInputValue] = useState<string>(isMbInitial ? String(maxSpeedKbs / 1024) : String(maxSpeedKbs));

  // Sync state if prop changes from outside (e.g. from state store),
  // adjusting during render instead of in an effect to avoid an extra paint.
  const [prevMaxSpeedKbs, setPrevMaxSpeedKbs] = useState(maxSpeedKbs);
  if (maxSpeedKbs !== prevMaxSpeedKbs) {
    setPrevMaxSpeedKbs(maxSpeedKbs);
  ...
  return (
    <div className={`flex items-center gap-1 ${compact ? 'text-[10px]' : 'text-xs'}`} style={{ direction: 'ltr' }}>
      ...
      <div
        className={`relative flex items-center bg-[var(--bg-input)] border border-[var(--border-color)] rounded overflow-hidden transition-all focus-within:border-[var(--accent-primary)] focus-within:ring-1 focus-within:ring-[var(--accent-primary)] ${compact ? 'h-6.5 w-[56px]' : 'h-8.5 w-[85px]'}`}
      >
        <input
          type="text"
          value={inputValue}
        ...
        <div className="absolute right-0 top-0 bottom-0 flex flex-col border-l border-[var(--border-color)] w-4 bg-[var(--bg-hover)]/30">
          <button
            ...
            className="flex-1 flex items-center justify-center hover:bg-[var(--border-color-hover)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"
              aria-label={t('speed_limit_increase')}
          >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center hover:bg-[var(--border-color-hover)] border-t border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"
              aria-label={t('speed_limit_decrease')}
          >
      ...
      {/* KB / MB Selector Tabs */}
      <div
        className={`flex border border-[var(--border-color)] rounded overflow-hidden bg-[var(--bg-input)] ${compact ? 'h-6.5' : 'h-8.5'}`}
      >
        <button
          type="button"
          onClick={() => {
          ...
          className={`px-1.5 flex items-center justify-center font-mono font-bold transition-colors ${
            unit === 'KB'
              ...
              aria-label={t('speed_limit_switch_kbs')}
        >
          KB
        </button>
        <button
          ...
          className={`px-1.5 flex items-center justify-center font-mono font-bold border-l border-[var(--border-color)] transition-colors ${
            unit === 'MB'
              ...
              aria-label={t('speed_limit_switch_mbs')}
        >
          MB
        </button>
      </div>
