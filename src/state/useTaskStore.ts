...
export function useTaskStore(
  ...
  const pauseTask = async (id: string) => {
    ...
    try {
      ...
      addToast(
        'error',
        'NOVA daemon',
        error instanceof Error ? error.message : 'The local engine could not pause the download.',
      );
    }
  };
  ...
  const deleteTask = async (id: string, deleteDisk: boolean) => {
    ...
    try {
      ...
      let diskMessage = '';
      if (deleteDisk) {
        if (targetItem.savePath) {
          const diskResult = await tauriClient.deleteDownloadedFile(targetItem.savePath);
          if (diskResult.success) {
            diskMessage = diskResult.changed
              ? ' The local file was deleted from disk.'
              : ' The local file was already missing from disk.';
          } else {
            diskMessage = ' The download entry was removed, but the local file could not be deleted.';
            addToast('error', 'Disk delete failed', diskResult.message);
          }
        } else {
          diskMessage = ' No saved file path was available for disk deletion.';
        }
      }
