...
export function useQueueStore(
  ...
  const undoStackRef = useRef<{ queues: Queue[]; tasks: DownloadItem[] }[]>([]);

  const snapshotForUndo = useCallback(() => {
    undoStackRef.current = [{ queues: JSON.parse(JSON.stringify(queues)) as Queue[], tasks: JSON.parse(JSON.stringify(tasks)) as DownloadItem[] }];
  }, [queues, tasks]);
  ...
  const reorderQueues = (fromIndex: number, toIndex: number) => {
    setQueues((prev) => {
      const newQueues = [...prev];
      const [moved] = newQueues.splice(fromIndex, 1);
      newQueues.splice(toIndex, 0, moved);
      return newQueues;
    });
  };
