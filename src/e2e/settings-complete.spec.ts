...
test.describe('Settings Complete — browser integration toggles', () => {
  ...
  for (const browserName of browsers) {
    test(`${browserName} integration toggle exists`, async ({ page }) => {
      ...
      const toggle = page.locator('label, div').filter({ hasText: new RegExp(browserName, 'i') }).first();
    ...
    test(`${browserName} toggle can be switched`, async ({ page }) => {
      ...
      const toggle = page.locator('label:has(> div[class*="rounded-full"])').filter({ hasText: new RegExp(browserName, 'i') }).first();
      if (await toggle.isVisible().catch(() => false)) {
        await toggle.click();
        await page.waitForTimeout(200);
        const wasToggled = await toggle.evaluate(el => {
          const inner = el.querySelector('div');
          return inner?.getAttribute('class')?.includes('translate-x') ||
            inner?.getAttribute('class')?.includes('bg-[var(--accent-primary)]') || false;
        });
...
test.describe('Settings Complete — sound selection', () => {
  ...
  test('sound selection dropdown exists', async ({ page }) => {
    const soundSelect = page.locator('select').filter({ hasText: /sound|صوت/i }).first();
...
test.describe('Settings Complete — browser intercept key selector', () => {
  ...
  test('intercept key dropdown or input exists', async ({ page }) => {
    const interceptField = page.locator('select, input').filter({ hasText: /intercept|alt|ctrl|shift/i }).first();
...
test.describe('Settings Complete — default headers add/remove', () => {
  ...
  test('add header button exists', async ({ page }) => {
    const addHeaderBtn = page.locator('button').filter({ hasText: /add header|إضافة رأس|add/i }).first();
  ...
  test('clicking add header shows input fields', async ({ page }) => {
    ...
    if (await addHeaderBtn.isVisible().catch(() => false)) {
      ...
      await page.waitForTimeout(300);
      const nameInput = page.locator('input[placeholder*="name"], input[placeholder*="User-Agent"], input[placeholder*="الاسم"]').first();
  ...
  test('header rows have delete buttons', async ({ page }) => {
    ...
    if (count > 0) {
      const deleteBtn = headerRows.first().locator('button').filter({ has: page.locator('svg') }).last();
...
test.describe('Settings Complete — settings import', () => {
  ...
  test('import settings button exists', async ({ page }) => {
    const importBtn = page.locator('button').filter({ hasText: /import|استيراد|restore|استعادة/i }).first();
...
test.describe('Settings Complete — settings export', () => {
  ...
  test('export settings button exists', async ({ page }) => {
    const exportBtn = page.locator('button').filter({ hasText: /export|تصدير|backup|نسخ/i }).first();
