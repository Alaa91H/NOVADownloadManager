...
fn restart_daemon(app: tauri::AppHandle) -> Result<(), String> {
    ...
    // Kill orphaned daemon processes on the port, then restart.
    kill_old_daemon();
    std::thread::sleep(std::time::Duration::from_millis(2000));
    let default_port: u16 = std::env::var("NOVA_DAEMON_PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(DEFAULT_DAEMON_PORT);
