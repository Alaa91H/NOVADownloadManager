...
fn validate_file_path(path: &str) -> Result<PathBuf, String> {
    ...
    if target.is_absolute() && target.is_file() {
        let canonical = target.canonicalize().map_err(|_| "Path does not exist".to_string())?;
        Ok(canonical)
    } else {
        Ok(target)
    }
...
fn check_tcp_endpoint(host: String, port: u16) -> Result<bool, String> {
    ...
    let is_internal = match ip {
        std::net::IpAddr::V4(v4) => v4.is_loopback() || v4.is_private() || v4.is_link_local() || v4.is_multicast(),
        std::net::IpAddr::V6(v6) => v6.is_loopback() || (v6.segments()[0] & 0xffc0) == 0xfe80 || v6.is_multicast(),
    };
    if is_internal {
...
fn save_config(app: tauri::AppHandle, settings: String) -> Result<(), String> {
    ...
    if settings.len() > MAX_CONFIG_SIZE {
        return Err(format!("Config size ({} bytes) exceeds limit ({} bytes)", settings.len(), MAX_CONFIG_SIZE));
    }
...
fn kill_old_daemon() {
    ...
    let port: u16 = std::env::var("NOVA_DAEMON_PORT")
        .ok()
