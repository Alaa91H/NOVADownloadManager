...
fn get_daemon_url(state: tauri::State<DaemonUrl>) -> String {
    state.0.lock().map(|g| g.clone()).unwrap_or_else(|_| format!("http://127.0.0.1:{}", DEFAULT_DAEMON_PORT))
}
...
#[tauri::command]
fn restart_daemon(app: tauri::AppHandle, daemon_url: tauri::State<DaemonUrl>) -> Result<(), String> {
...
fn kill_old_daemon() {
    // Terminate every aria2 engine we spawned first, so no orphan keeps the
    // RPC port busy for the next daemon instance or the next app launch.
    daemon::aria2::kill_tracked_aria2();
    let port = std::env::var("NOVA_DAEMON_PORT").unwrap_or_else(|_| DEFAULT_DAEMON_PORT.to_string());
    let our_pid = std::process::id();
    // Use PowerShell's Get-NetTCPConnection to avoid locale issues with netstat.
    let script = format!(
        "Get-NetTCPConnection -LocalPort {} -State Listen | ForEach-Object {{ \
            $p = Get-Process -Id $_.OwningProcess -ErrorAction SilentlyContinue; \
            if ($p -and $p.Id -ne {}) {{ taskkill /F /PID $p.Id -ErrorAction SilentlyContinue }} \
        }}",
        port, our_pid
    );
