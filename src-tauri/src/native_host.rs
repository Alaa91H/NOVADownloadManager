...
fn read_native_message() -> io::Result<Value> {
    ...
    if len == 0 || len > MAX_NATIVE_REQUEST_BYTES {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "native message size is invalid"));
    }
...
fn write_native_message(value: &Value) -> io::Result<()> {
    ...
    if bytes.len() > MAX_NATIVE_RESPONSE_BYTES {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "native response exceeds browser limit"));
    }
...
fn handle_native_request(client: &reqwest::blocking::Client, request: Value) -> Value {
    let id = request.get("id").cloned().unwrap_or_else(|| json!("unknown"));
...
fn task_command(client: &reqwest::blocking::Client, route: &str, params: Value) -> Result<Value, String> {
    let task_id = params
        .get("taskId")
...
fn http_json(
    ...
    let response = request.send().map_err(|error| format!("NOVA loopback bridge is unavailable: {error}"))?;
    let status = response.status();
    let value = response.json::<Value>().map_err(|error| format!("Invalid NOVA loopback response: {error}"))?;
    if !status.is_success() {
        return Err(value.get("error").and_then(Value::as_str).unwrap_or("NOVA loopback request failed").to_string());
    }
    Ok(value)
}
