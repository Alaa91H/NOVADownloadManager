...
#[inline]
pub fn direct_bool(direct_options: &HashMap<String, serde_json::Value>, key: &str) -> Option<bool> {
    direct_options.get(key).and_then(serde_json::Value::as_bool)
}

#[inline]
pub fn direct_u64(direct_options: &HashMap<String, serde_json::Value>, key: &str) -> Option<u64> {
    direct_options
        .get(key)
        .and_then(|v| v.as_u64().or_else(|| v.as_f64().map(|n| n.max(0.0) as u64)))
}

pub fn direct_array(direct_options: &HashMap<String, serde_json::Value>, key: &str) -> Vec<String> {
    direct_options
        .get(key)
...
pub fn build_curl_args_with_capabilities(
    ...
    if let Some(dopts) = &body.direct_options {
        ...
        if let Some(rate) = direct_str(dopts, "rate") {
            push_arg(&mut args, "--rate", rate);
        }
        push_optional_u64(
            &mut args,
            "--speed-limit",
