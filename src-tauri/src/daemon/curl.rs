...
const FALLBACK_USER_AGENT: &str = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36";
...
fn installed_ca_bundle_path() -> String {
    ...
    // Check in the CARGO_MANIFEST_DIR (development / debug builds)
...
pub fn init_download_ssl() {
    ...
    if !ca_path.is_empty() {
        ...
        // This env var is read by OpenSSL on init.
...
pub fn start_curl_process(state: &SharedState, id: &str) {
    let record = {
        ...
        let worker_was_started = job.run_generation.load(Ordering::Relaxed) > 0;
