...
use ::curl::easy::{Auth, Easy2, Handler, HttpVersion, IpResolve, List, NetRc, ProxyType, SslOpt, SslVersion, TimeCondition, WriteError};
...
use crate::daemon::utils::{build_segments, infer_file_type, kill_process, now_str, push_arg, DEFAULT_USER_AGENT};
...
fn collect_multi_errors<H: Handler>(multi: &Multi, handles: &[Easy2Handle<H>], label: &str) -> Vec<String> {
...
fn check_multi_messages<H: Handler>(multi: &Multi, handles: &[Easy2Handle<H>], label: &str) -> Result<(), String> {
    ...
    F: FnMut(),
{
    ...
    if runtime.sockets.is_empty() {
        running = multi.timeout().map_err(|e| format!("libcurl multi timeout action failed: {e}"))?;
        runtime.drain_updates(multi)?;
    }
...
pub fn init_download_ssl() {
    SSL_INIT.get_or_init(|| {
        ...
        if !ca_path.is_empty() {
            // Tell OpenSSL (and by extension curl) where to find CA certs.
            // This env var is read by OpenSSL on init. Called via OnceLock
            // to guarantee single execution before any concurrent access.
...
fn apply_easy_options(easy: &mut Easy2<SegmentWriter>, plan: &DirectDownloadPlan, range: Option<(u64, u64)>) -> Result<(), String> {
    ...
    if direct_u64(opts, "maxRedirs").is_none() {
        easy.max_redirections(20).map_err(|e| format!("Could not configure default redirect limit: {e}"))?;
    }
    // Automatically set the Referer header when following redirects (crucial for CDNs)
    easy.autoreferer(true).map_err(|e| format!("Could not enable auto-referer: {e}"))?;
    // Enable TCP keepalive for long-running downloads
    easy.tcp_keepalive(true).map_err(|e| format!("Could not enable TCP keepalive: {e}"))?;
    ...
    if direct_bool(opts, "insecure") == Some(true) {
        log::warn!("TLS verification disabled via 'insecure' option for download from {}", plan.url);
    ...
    if let Some(doh) = direct_str(opts, "dohUrl") {
        easy.doh_url(Some(doh)).map_err(|e| format!("Could not configure DNS-over-HTTPS: {e}"))?;
    }
    if let Some(dns) = direct_str(opts, "dnsServers") {
        easy.dns_servers(dns).map_err(|e| format!("Could not configure DNS servers: {e}"))?;
    }
    if let Some(sec) = direct_u64(opts, "dnsCacheTimeoutSec") {
        easy.dns_cache_timeout(Duration::from_secs(sec)).map_err(|e| format!("Could not configure DNS cache timeout: {e}"))?;
    }
    if let Some(jar) = direct_str(opts, "cookieJar") {
        easy.cookie_jar(jar).map_err(|e| format!("Could not configure cookie jar: {e}"))?;
    }
    if let Some(pubkey) = direct_str(opts, "pinnedPubKey") {
        easy.pinned_public_key(pubkey).map_err(|e| format!("Could not configure certificate pinning: {e}"))?;
    }
    if let Some(size) = direct_u64(opts, "maxFilesize").filter(|v| *v > 0) {
        easy.max_filesize(size).map_err(|e| format!("Could not configure max file size: {e}"))?;
    }
    if let Some(size) = direct_u64(opts, "bufferSize").filter(|v| *v > 0) {
        easy.buffer_size(size as usize).map_err(|e| format!("Could not configure buffer size: {e}"))?;
    }
    ...
    if let Some(user) = direct_str(opts, "username") {
        easy.username(user).map_err(|e| format!("Could not configure username: {e}"))?;
    }
    if let Some(pass) = direct_str(opts, "password") {
        easy.password(pass).map_err(|e| format!("Could not configure password: {e}"))?;
    }
    if let Some(auth_type) = direct_str(opts, "authType") {
        ...
        match lower.as_str() {
            ...
            "ntlm" => { auth.ntlm(true); }
            "negotiate" | "gssapi" => { auth.gssnegotiate(true); }
            "any" => { auth.auto(true); }
            _ => { auth.basic(true); }
        }
        easy.http_auth(&auth).map_err(|e| format!("Could not configure HTTP auth: {e}"))?;
    }
    // ── Netrc ──
    if let Some(netrc_val) = direct_str(opts, "netrc") {
        let lower = netrc_val.to_ascii_lowercase();
        match lower.as_str() {
            "optional" | "yes" => easy.netrc(NetRc::Optional).map_err(|e| format!("Could not configure netrc: {e}"))?,
            "required" | "true" | "on" => easy.netrc(NetRc::Required).map_err(|e| format!("Could not configure netrc: {e}"))?,
            "no" | "false" | "off" | "ignored" => easy.netrc(NetRc::Ignored).map_err(|e| format!("Could not configure netrc: {e}"))?,
            _ => easy.netrc(NetRc::Optional).map_err(|e| format!("Could not configure netrc: {e}"))?,
        }
    }
    // ── Client certificates (mTLS) ──
    if let Some(cert) = direct_str(opts, "cert") {
        easy.ssl_cert(cert).map_err(|e| format!("Could not configure SSL certificate: {e}"))?;
    }
    if let Some(cert_type) = direct_str(opts, "certType") {
        easy.ssl_cert_type(cert_type).map_err(|e| format!("Could not configure certificate type: {e}"))?;
    }
    if let Some(key) = direct_str(opts, "key") {
        easy.ssl_key(key).map_err(|e| format!("Could not configure SSL key: {e}"))?;
    }
    if let Some(key_type) = direct_str(opts, "keyType") {
        easy.ssl_key_type(key_type).map_err(|e| format!("Could not configure key type: {e}"))?;
    }
    if let Some(key_pass) = direct_str(opts, "pass") {
        easy.key_password(key_pass).map_err(|e| format!("Could not configure key password: {e}"))?;
    }
    // ── TLS version and cipher configuration ──
    if let Some(ciphers) = direct_str(opts, "ciphers") {
        easy.ssl_cipher_list(ciphers).map_err(|e| format!("Could not configure TLS cipher list: {e}"))?;
    }
    if let Some(tls_max) = direct_str(opts, "tlsMax") {
        ...
        if let Some(ver) = max_ver {
            easy.ssl_min_max_version(SslVersion::Default, ver).map_err(|e| format!("Could not configure TLS max version: {e}"))?;
        }
    }
    // CA path (directory of CA certs)
    if let Some(ca_path) = direct_str(opts, "caPath") {
        easy.capath(ca_path).map_err(|e| format!("Could not configure CA path: {e}"))?;
    }
    if direct_bool(opts, "sslReqd") == Some(true) {
        easy.ssl_verify_peer(true).map_err(|e| format!("Could not enable SSL peer verification: {e}"))?;
        easy.ssl_verify_host(true).map_err(|e| format!("Could not enable SSL host verification: {e}"))?;
    }
    ...
    if let Some(proxy_user) = direct_str(opts, "proxyUser") {
        if let Some(proxy_pass) = direct_str(opts, "proxyPassword") {
            ...
            easy.proxy_username(&cred).map_err(|e| format!("Could not configure proxy credentials: {e}"))?;
        } else {
            easy.proxy_username(proxy_user).map_err(|e| format!("Could not configure proxy username: {e}"))?;
        }
    }
    if let Some(proxy_auth_val) = direct_str(opts, "proxyAnyAuth") {
        ...
        match lower.as_str() {
            ...
            "ntlm" => { auth.ntlm(true); }
            "negotiate" | "gssapi" => { auth.gssnegotiate(true); }
            "any" | "auto" => { auth.auto(true); }
            _ => { auth.auto(true); }
        }
        easy.proxy_auth(&auth).map_err(|e| format!("Could not configure proxy auth: {e}"))?;
    }
    // ── FTP options ──
    ...
    if direct_bool(opts, "remoteTime") == Some(true) {
        easy.fetch_filetime(true).map_err(|e| format!("Could not enable remote time fetch: {e}"))?;
    }
    if let Some(method) = direct_str(opts, "requestMethod") {
        easy.custom_request(method).map_err(|e| format!("Could not configure custom request method: {e}"))?;
    }
    ...
    if let Some(time_cond) = direct_str(opts, "timeCond") {
        ...
        if let Some(c) = cond {
            easy.time_condition(c).map_err(|e| format!("Could not configure time condition: {e}"))?;
            if let Some(ts) = direct_u64(opts, "timeValue") {
                easy.time_value(ts as i64).map_err(|e| format!("Could not configure time value: {e}"))?;
            }
        }
    }
    // ── TCP tuning ──
    if direct_bool(opts, "tcpNoDelay") == Some(true) {
        easy.tcp_nodelay(true).map_err(|e| format!("Could not enable TCP no-delay: {e}"))?;
    }
    if direct_bool(opts, "pathAsIs") == Some(true) {
        easy.path_as_is(true).map_err(|e| format!("Could not enable path-as-is: {e}"))?;
    }
    if let Some(keepalive_sec) = direct_u64(opts, "keepaliveTimeSec").filter(|v| *v > 0) {
        let dur = Duration::from_secs(keepalive_sec);
        easy.tcp_keepidle(dur).map_err(|e| format!("Could not configure keepalive idle time: {e}"))?;
        easy.tcp_keepintvl(dur).map_err(|e| format!("Could not configure keepalive interval: {e}"))?;
    }
    ...
    if !resolve_entries.is_empty() {
        ...
        for entry in &resolve_entries {
            list.append(entry.as_str()).map_err(|e| format!("Could not add DNS resolve entry: {e}"))?;
        }
        easy.resolve(list).map_err(|e| format!("Could not configure DNS resolve overrides: {e}"))?;
    }
    let connect_to_entries = direct_array(opts, "connectTo");
    if !connect_to_entries.is_empty() {
        let mut list = List::new();
        for entry in &connect_to_entries {
            list.append(entry.as_str()).map_err(|e| format!("Could not add connect-to entry: {e}"))?;
        }
        easy.connect_to(list).map_err(|e| format!("Could not configure connect-to overrides: {e}"))?;
    }
    // ── Connection pool tuning ──
    if let Some(max_connects) = direct_u64(opts, "maxConnects").filter(|v| *v > 0) {
        easy.max_connects(max_connects.min(u32::MAX as u64) as u32).map_err(|e| format!("Could not configure max connects: {e}"))?;
    }
    ...
    if let Some(ip) = direct_str(opts, "ipResolve") {
        ...
        match lower.as_str() {
            "4" | "ipv4" | "4-only" => easy.ip_resolve(IpResolve::V4).map_err(|e| format!("Could not force IPv4: {e}"))?,
            "6" | "ipv6" | "6-only" => easy.ip_resolve(IpResolve::V6).map_err(|e| format!("Could not force IPv6: {e}"))?,
            _ => {}
        }
    }
    // ── TLS minimum version ──
    if let Some(tls_min) = direct_str(opts, "tlsMin") {
        ...
        if let Some(ver) = min_ver {
            easy.ssl_min_max_version(ver, SslVersion::Default).map_err(|e| format!("Could not configure TLS min version: {e}"))?;
        }
    }
    // ── TLS version range (tlsMin + tlsMax together) ──
    if direct_str(opts, "tlsMin").is_some() && direct_str(opts, "tlsMax").is_some() {
        // Already handled above individually; both will have been set via ssl_min_max_version
        // The last call wins for the range, so we re-apply as a range.
        let tls_min = direct_str(opts, "tlsMin").unwrap();
        let tls_max = direct_str(opts, "tlsMax").unwrap();
        ...
        if let (Some(min_v), Some(max_v)) = (parse_ver(tls_min), parse_ver(tls_max)) {
            easy.ssl_min_max_version(min_v, max_v).map_err(|e| format!("Could not configure TLS version range: {e}"))?;
        }
    }
    ...
    if let Some(opts_str) = direct_str(opts, "sslOptions") {
        ...
        if lower.contains("no-revoke") { ssl_opt.no_revoke(true); }
        if lower.contains("no-partial") { ssl_opt.no_partial_chain(true); }
        if lower.contains("native-ca") { ssl_opt.native_ca(true); }
        if lower.contains("auto-client-cert") { ssl_opt.auto_client_cert(true); }
        easy.ssl_options(&ssl_opt).map_err(|e| format!("Could not configure SSL options: {e}"))?;
    }
    // ── Certificate Revocation List ──
    if let Some(crl) = direct_str(opts, "crlFile") {
        easy.crlfile(crl).map_err(|e| format!("Could not configure CRL file: {e}"))?;
    }
    // ── Issuer certificate ──
    if let Some(issuer) = direct_str(opts, "issuerCert") {
        easy.issuer_cert(issuer).map_err(|e| format!("Could not configure issuer certificate: {e}"))?;
    }
    // ── SSL session ID cache ──
    if direct_bool(opts, "sslSessionIdCache") == Some(false) {
        easy.ssl_sessionid_cache(false).map_err(|e| format!("Could not disable SSL session ID cache: {e}"))?;
    }
    // ── Proxy TLS configuration ──
    if let Some(proxy_cainfo) = direct_str(opts, "proxyCaInfo") {
        easy.proxy_cainfo(proxy_cainfo).map_err(|e| format!("Could not configure proxy CA info: {e}"))?;
    }
    if let Some(proxy_capath) = direct_str(opts, "proxyCaPath") {
        easy.proxy_capath(proxy_capath).map_err(|e| format!("Could not configure proxy CA path: {e}"))?;
    }
    if let Some(proxy_cert) = direct_str(opts, "proxyCert") {
        easy.proxy_sslcert(proxy_cert).map_err(|e| format!("Could not configure proxy SSL cert: {e}"))?;
    }
    if let Some(proxy_cert_type) = direct_str(opts, "proxyCertType") {
        easy.proxy_sslcert_type(proxy_cert_type).map_err(|e| format!("Could not configure proxy cert type: {e}"))?;
    }
    if let Some(proxy_key) = direct_str(opts, "proxyKey") {
        easy.proxy_sslkey(proxy_key).map_err(|e| format!("Could not configure proxy SSL key: {e}"))?;
    }
    if let Some(proxy_key_type) = direct_str(opts, "proxyKeyType") {
        easy.proxy_sslkey_type(proxy_key_type).map_err(|e| format!("Could not configure proxy key type: {e}"))?;
    }
    if let Some(proxy_key_pass) = direct_str(opts, "proxyKeyPassword") {
        easy.proxy_key_password(proxy_key_pass).map_err(|e| format!("Could not configure proxy key password: {e}"))?;
    }
    if let Some(proxy_cipher) = direct_str(opts, "proxyCiphers") {
        easy.proxy_ssl_cipher_list(proxy_cipher).map_err(|e| format!("Could not configure proxy cipher list: {e}"))?;
    }
    if let Some(proxy_tls_max) = direct_str(opts, "proxyTlsMax") {
        ...
        if let Some(ver) = max_ver {
            easy.proxy_ssl_min_max_version(SslVersion::Default, ver).map_err(|e| format!("Could not configure proxy TLS max version: {e}"))?;
        }
    }
    if let Some(proxy_tls_min) = direct_str(opts, "proxyTlsMin") {
        ...
        if let Some(ver) = min_ver {
            easy.proxy_ssl_min_max_version(ver, SslVersion::Default).map_err(|e| format!("Could not configure proxy TLS min version: {e}"))?;
        }
    }
    if direct_bool(opts, "proxyVerifyPeer") == Some(false) {
        easy.proxy_ssl_verify_peer(false).map_err(|e| format!("Could not disable proxy peer verification: {e}"))?;
    }
    if direct_bool(opts, "proxyVerifyHost") == Some(false) {
        easy.proxy_ssl_verify_host(false).map_err(|e| format!("Could not disable proxy host verification: {e}"))?;
    }
    // ── Proxy type ──
    if let Some(proxy_type_val) = direct_str(opts, "proxyType") {
        ...
        if let Some(pt_val) = pt {
            easy.proxy_type(pt_val).map_err(|e| format!("Could not configure proxy type: {e}"))?;
        }
    }
    // ── Proxy tunnel (HTTP CONNECT) ──
    if direct_bool(opts, "proxyTunnel") == Some(true) {
        easy.http_proxy_tunnel(true).map_err(|e| format!("Could not enable proxy tunnel: {e}"))?;
    }
    // ── Unrestricted auth on redirects ──
    if direct_bool(opts, "unrestrictedAuth") == Some(true) {
        easy.unrestricted_auth(true).map_err(|e| format!("Could not enable unrestricted auth: {e}"))?;
    }
    // ── HTTP transfer encoding ──
    if direct_bool(opts, "transferEncoding") == Some(true) {
        easy.transfer_encoding(true).map_err(|e| format!("Could not enable transfer encoding: {e}"))?;
    }
    // ── HTTP 0.9 allowed ──
    if direct_bool(opts, "http09Allowed") == Some(true) {
        easy.http_09_allowed(true).map_err(|e| format!("Could not enable HTTP/0.9: {e}"))?;
    }
    // ── Expect 100-continue timeout ──
    if let Some(timeout) = direct_u64(opts, "expect100TimeoutMs") {
        easy.expect_100_timeout(Duration::from_millis(timeout)).map_err(|e| format!("Could not configure expect-100 timeout: {e}"))?;
    }
    // ── Fresh connection (bypass connection pool) ──
    if direct_bool(opts, "freshConnect") == Some(true) {
        easy.fresh_connect(true).map_err(|e| format!("Could not force fresh connection: {e}"))?;
    }
    // ── Forbid connection reuse ──
    if direct_bool(opts, "forbidReuse") == Some(true) {
        easy.forbid_reuse(true).map_err(|e| format!("Could not forbid connection reuse: {e}"))?;
    }
    // ── Max connection age ──
    if let Some(age) = direct_u64(opts, "maxAgeConn") {
        easy.maxage_conn(Duration::from_secs(age)).map_err(|e| format!("Could not configure max connection age: {e}"))?;
    }
    ...
    if let Some(range) = direct_str(opts, "localPortRange") {
        if let Some((start, _end)) = range.split_once('-') {
            if let Ok(s) = start.trim().parse::<u16>() {
                easy.local_port_range(s).map_err(|err| format!("Could not configure local port range: {err}"))?;
            }
        }
    }
    // ── DoH TLS verification ──
    if direct_bool(opts, "dohSslVerifyPeer") == Some(false) {
        easy.doh_ssl_verify_peer(false).map_err(|e| format!("Could not disable DoH peer verification: {e}"))?;
    }
    if direct_bool(opts, "dohSslVerifyHost") == Some(false) {
        easy.doh_ssl_verify_host(false).map_err(|e| format!("Could not disable DoH host verification: {e}"))?;
    }
    ...
    if !has_custom {
        ...
        list.append("Accept: */*").map_err(|e| format!("Could not add Accept header: {e}"))?;
        list.append("Accept-Language: en-US,en;q=0.9").map_err(|e| format!("Could not add Accept-Language header: {e}"))?;
        list.append("Cache-Control: no-cache").map_err(|e| format!("Could not add Cache-Control header: {e}"))?;
        list.append("Connection: keep-alive").map_err(|e| format!("Could not add Connection header: {e}"))?;
        if let Some(bearer) = direct_str(opts, "oauth2Bearer") {
            list.append(&format!("Authorization: Bearer {}", bearer)).map_err(|e| format!("Could not add OAuth2 bearer header: {e}"))?;
        }
        easy.http_headers(list).map_err(|e| format!("Could not configure default headers: {e}"))?;
...
fn run_single_libcurl(state: &SharedState, id: &str, plan: &DirectDownloadPlan, cancel: Arc<AtomicBool>) -> Result<u64, String> {
    ...
    let mut socket_runtime = if matches!(EventLoopMode::from_options(&plan.direct_options), EventLoopMode::MultiSocket) {
    ...
    let mut tick = || update_single_progress(state, id, &plan.output_path, plan.total_size, &mut last_total, &mut last_tick);
    ...
    let response = handles[0].response_code().map_err(|e| format!("Could not read HTTP response code: {e}"))?;
    if response >= 400 {
...
fn run_segmented_libcurl(state: &SharedState, id: &str, plan: &DirectDownloadPlan, cancel: Arc<AtomicBool>) -> Result<u64, String> {
    ...
    configure_multi_limits(
        &mut multi,
        ConnectionLimits::from_options(&plan.direct_options, plan.connections, MAX_DIRECT_CONNECTIONS),
    )?;
    ...
    let mut last_tick = Instant::now();
    let mut tick = || update_curl_task_progress(state, id, plan.total_size, &active, &mut last_total, &mut last_tick);
    if let Some(runtime) = socket_runtime.as_mut() {
    ...
    for (idx, handle) in handles.iter().enumerate() {
        let code = handle.response_code().map_err(|e| format!("Segment {idx}: could not read HTTP response code: {e}"))?;
...
fn run_libcurl_download(state: &SharedState, id: &str, plan: DirectDownloadPlan, cancel: Arc<AtomicBool>) -> Result<u64, String> {
    ...
    for attempt in 0..retry_policy.attempts {
        ...
        match result {
            ...
            Err(error) if error == "cancelled" || cancel.load(Ordering::Acquire) => return Err("cancelled".to_string()),
            Err(error) => {
                last_error = error;
...
pub fn start_curl_process(state: &SharedState, id: &str) {
    let record = {
        ...
        let generation = job.run_generation.fetch_add(1, Ordering::Release).saturating_add(1);
