...
struct DirectDownloadPlan {
    ...
    /// Strong validator for conditional resume (RFC 7233 §3.2).
    /// Prefer ETag; fall back to Last-Modified date.
    validator: Option<String>,
    /// Whether the `validator` is an ETag (true) or Last-Modified date (false).
    /// Used to decide whether weak-ETag `W/` prefix stripping is needed for
    /// If-Range headers (RFC 7233 §3.2).
    validator_is_etag: bool,
    /// Server-advertised sha-256 digest (RFC 3230 / RFC 9530) for post-download
    /// integrity verification.
    digest_sha256: Option<String>,
    /// Mirror URLs advertised via Link headers (RFC 6249).
    /// Fed into MirrorManager registration for failover.
    link_mirrors: Vec<String>,
    /// Per-mirror priorities extracted from `pri=N` in Link headers (RFC 6249 §4).
    /// Index-aligned with `link_mirrors`; defaults to 1 when absent.
    mirror_priorities: Vec<u32>,
}

/// Response metadata captured from headers during a transfer, used to implement
/// conditional resume (RFC 9110 If-Range), 416 handling, server-advertised
/// mirrors (RFC 6249) and server integrity digests (RFC 3230 / RFC 9530).
#[derive(Default, Clone)]
struct ResponseCapture {
    /// Last HTTP status line code seen (final response after redirects).
    status_code: u16,
    /// Strong validator for conditional resume: ETag if present, else the
    /// `Last-Modified` value.
    validator: Option<String>,
    /// Lower-case base64/hex sha-256 advertised via `Repr-Digest`/
    /// `Content-Digest`/`Digest`.
    digest_sha256: Option<String>,
    /// Alternate download URLs advertised via `Link: <url>; rel=duplicate`.
    mirrors: Vec<String>,
}
...
struct SegmentProgress {
    ...
    /// Seconds requested by the server via a `Retry-After` header on a
    /// 429/503 response (0 when absent). Consumed by the retry backoff.
    retry_after: Arc<AtomicU64>,
    /// Response metadata captured from headers during the transfer.
    capture: Arc<Mutex<ResponseCapture>>,
    /// Shared slot for the streaming SHA-256 hex digest computed during write().
    /// The caller retains a clone and reads the result after the transfer completes.
...
struct SegmentWriter {
    ...
    /// Incremental SHA-256 hasher fed in write() for streaming digest verification.
    /// Only created when the server sends a `Content-Digest` / `Repr-Digest` header.
    streaming_hasher: Option<sha2::Sha256>,
}
...
impl Handler for SegmentWriter {
    fn write(&mut self, data: &[u8]) -> Result<usize, WriteError> {
        ...
        // Feed streaming hasher for incremental digest verification.
    ...
    fn header(&mut self, data: &[u8]) -> bool {
        ...
        // Status line of each (possibly redirected) response — keep the last.
        ...
        match name.as_str() {
            // Retry-After: honour a rate-limit backoff hint (RFC 9110 §15.3.7).
            // Supports both integer-seconds and HTTP-date forms.
            "retry-after" => {
            ...
            // Prefer a strong ETag validator; skip weak ETags (RFC 7232 §2.3):
            // "A client MUST NOT generate an If-Range header field containing
            //  an entity tag that is marked as weak."
            // Only store strong ETags; fall back to Last-Modified for weak.
            "etag" => {
...
fn safe_value(value: &str) -> bool {
    !value.is_empty()
        && !value.bytes().any(|b| {
            b == 0 || b == b'\n' || b == b'\r' || (b < 0x09 && b != 0) || (b > 0x09 && b < 0x20)
        })
}
...
fn plan_from_job(job: &CurlJob) -> DirectDownloadPlan {
    ...
    // Extract conditional-resume validators from direct_options.
    // The frontend/route handler stores the probe's ETag/Last-Modified here.
    let etag = direct_str(&opts, "etag").map(str::to_string);
    let last_modified = direct_str(&opts, "lastModified").map(str::to_string);
    ...
    let digest_sha256 = direct_str(&opts, "digestSha256").map(str::to_string);
    let link_mirrors: Vec<String> = opts
        .get("linkMirrors")
        .and_then(|v| v.as_array())
        .map(|arr| {
            arr.iter()
        ...
        .unwrap_or_default();
    // Extract per-mirror priorities from the Link header metadata (RFC 6249 §4).
    let mirror_priorities = opts
...
/// Return the `scheme://host/` origin of a URL, used as a default Referer.
...
/// Build the `If-Range` header value from a plan's validator.
/// RFC 7233 §3.2 — "A client MUST NOT generate an If-Range header field
/// containing an entity tag that is marked as weak."  Only strong ETags
/// are used; for weak ETags the Last-Modified date is preferred instead.
...
fn apply_easy_options<H: Handler>(
    easy: &mut Easy2<H>,
    ...
    // Allow up to 20 redirects by default (some CDNs chain multiple redirects)
    ...
    // Collect conditional headers as strings and merge them into the final
    // header list at the bottom of this function.  We must NOT call
    // http_headers() here because it *replaces* the entire header list;
    // the default/custom headers block and etagCompare further down also
    // call it, so any early call would be immediately clobbered.
    ...
    // RFC 7233 §3.2 — If-Range with a validator protects against silent
    // corruption when the resource has changed.  Shared between segment
    // and resume paths to avoid duplication.
    ...
    if let Some(referer) = plan.referer.as_deref() {
        ...
        // Default to a same-origin Referer: this is exactly what a browser sends
        // when the user clicks a download link on the site, so it satisfies the
        // common Referer-based hotlink protection used by many CDNs without
        // breaking hosts that ignore the header.
    ...
    // by CDNs and mirrors) that accepts the socket but never
    ...
    // Start with either custom headers from the caller or browser-like defaults,
    // then append any conditional headers (If-Range, If-None-Match) collected
    // above so that http_headers() is called exactly once and nothing clobbers.
    let mut header_list: List = if let Some(headers) = direct_headers(opts)? {
        ...
        if plan.digest_sha256.is_none() {
            // RFC 3230: Want-Digest requests a digest in the response.
            // RFC 9530 supersedes with Want-Content-Digest / Want-Representation-Digest.
            // Send both for maximum server compatibility.
    ...
    // Append conditional headers (If-Range for resume, If-None-Match for etagCompare).
    for hdr in &conditional_headers {
        header_list
            .append(hdr)
            .map_err(|e| format!("Could not add conditional header: {e}"))?;
    }
    // ── etagCompare (RFC 7232 ETag persistence) ──
...
fn run_single_libcurl(
    ...
    // Track whether a resume was attempted so we can detect server returning
    // 200 OK instead of 206 Partial Content (which would corrupt the file).
    ...
    // ── 304 Not Modified ──
    // RFC 7232 §4.1: The server confirms the resource has not changed since
    // the validator was last sent.  If we were resuming, the local partial
    // content is still valid — return the existing file size so the caller
    // treats it as a successful resume (the server simply had nothing new).
    // If we were NOT resuming (full GET got 304), the file is already complete.
    if response == 304 {
    ...
    // RFC 7232 §4.2: The server says the resource has changed since our
    // If-Range/If-Match validator.  Discard local partial data and signal
    ...
    // RFC 9110 §15.5.5: The server cannot satisfy the requested Range header
    // (e.g. the resume offset exceeds the current resource size because the
    // file shrunk or was replaced).  Discard partial data and signal a restart
    // without a Range header so the caller can re-download from scratch.
    if response == 416 {
        let existing = FileWriter::current_size(&plan.output_path);
        if existing > 0 {
            log::info!(
    ...
    // ── 200 OK on resume without If-Range (RFC 7233 §4.1) ──
    // If we attempted a resume (had partial data + validator) but the server
    // returned 200 instead of 206, the full body was written starting from
    // the resume offset, corrupting the file.  Truncate and signal restart.
...
fn run_segmented_libcurl(
    ...
    for (idx, handle) in handles.iter().enumerate() {
        ...
        // ── 304 Not Modified ──
        // Server confirms resource unchanged — the segment data is still valid.
        if code == 304 {
            continue;
        }
        // ── 412 Precondition Failed ──
        // Resource changed since our conditional request — discard all partial
        // segments and signal a restart.
        if code == 412 {
        ...
        // ── 416 Range Not Satisfiable ──
        // The server cannot satisfy the byte-range request (e.g. the resource
        // shrank).  Discard all partial segments and signal a restart.
        if code == 416 {
            for r in ranges.iter() {
                let _ = std::fs::remove_file(&r.path);
            }
    ...
    // Read back the first segment's captured validator (ETag / Last-Modified)
    // so the caller can persist it via etagSave for future conditional resume.
...
fn run_libcurl_download(
    ...
    let compressed = direct_bool(&plan.direct_options, "compressed") != Some(false);
    ...
    // Populated from a server `Retry-After` header (seconds) when it rate-limits.
    let retry_after = Arc::new(AtomicU64::new(0));
    // Shared slot for the streaming SHA-256 digest computed during download.
    let streaming_digest_out = Arc::new(Mutex::new(None::<String>));
    let streaming_digest_reader = streaming_digest_out.clone();
    // Skip the parallel attempt for hosts we've already learned only tolerate a
    // single range connection, so we don't waste an attempt stalling.
    ...
    for attempt in 0..retry_policy.attempts {
        ...
        match result {
            Ok((size, captured_validator)) => {
                ...
                // ── Digest verification (RFC 3230 / RFC 9530) ──
                if let Some(ref expected_raw) = plan.digest_sha256 {
                    // Prefer the streaming hash computed during write() to avoid
                    // re-reading the entire file from disk.
                    ...
                    if let Some(actual_hex) = actual_hex {
                        // The server may send the digest as base64 (RFC 3230
                        // `:BASE64:`) or as hex.  Normalise to hex for comparison.
                ...
                // ── etagSave: persist the captured ETag for future requests ──
                if let Some(etag_file) = direct_str(&plan.direct_options, "etagSave") {
            ...
            Err(error) => {
                ...
                if let Ok(managers) = state.mirror_managers.lock() {
                    if let Some(mgr) = managers.get(id) {
                        // Register link_mirrors from the download plan (RFC 6249)
                        // so they participate in failover.  Use per-mirror
                        // priority from the `pri=N` Link parameter when available.
                ...
                // Many CDNs and mirrors accept only one
                ...
                if plan.segmented {
                    ...
                    if !cancel.load(Ordering::Acquire) {
                        match run_single_libcurl(
                            ...
                            Ok((size, _captured)) => {
                                // Remember this host only tolerates one connection.
                            ...
                            Err(fb_error) => {
                                ...
                                // Do NOT re-enable segmentation — the server
                                // rejected parallel ranges, so retry with a
                ...
                if attempt + 1 < retry_policy.attempts {
                    // Honour a server `Retry-After` hint when present, otherwise
                    // use the policy's exponential backoff with jitter.
    ...
    // Guaranteed single-connection fallback: some hosts and mirrors accept
    // only one range connection and stall the rest,
...
pub async fn create_curl_task(
    ...
    FileWriter::ensure_parent(&output_path)?;
