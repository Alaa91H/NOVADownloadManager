            ...
            rt.block_on(async {
                ...
                *SHUTDOWN_TX.lock().unwrap() = Some(shutdown_tx);
                ...
                let shutdown_signal = async move {
                    ...
                    let native_shutdown_snapshots = {
                        let mut native = lock_or_err!(shutdown_state.native_media_jobs);
                        let mut snapshots = Vec::with_capacity(native.len());
                        for job in native.values_mut() {
                            job.cancel_token
                                .store(true, std::sync::atomic::Ordering::Release);
                            let current = TaskState::from_status(&job.task.status);
                            if let Some(current) = current {
                                if current != TaskState::Completed {
                                    let target = if current.is_active() {
                                        TaskState::Pausing
                                    } else {
                                        TaskState::Paused
                                    };
                                    if let Err(error) = transition_task_state(
                                        ...
                                        log::info!(
                                            "Native media task {} kept in state '{}' during shutdown: {error}",
                                            job.task.id,
                                            job.task.status
                                        );
                                    }
                    ...
                    };
                    {
                        ...
                        for task in native_shutdown_snapshots {
...
fn restore_persisted_tasks(
    ...
    for mut task in restored.tasks {
        ...
        if requires_reauth && task.status != "completed" {
            ...
            if task.status != "completed" {
                if let Some(mut request) = restored.native_media_requests.get(&task.id).cloned() {
                    // Restored jobs never auto-start. Resume is an explicit
                    // lifecycle action and refreshes expiring media URLs.
                    request.start_immediately = Some(false);
                    ...
                    task.error_message = Some(
                        "The native media request needed to resume this task is missing. Re-add the media URL."
                            .to_owned(),
                    );
                    task.speed_bytes_per_sec = 0;
                }
