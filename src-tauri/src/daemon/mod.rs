...
use crate::daemon::routes::{
    handle_browser_ext_config, handle_browser_ext_health,
...
pub fn start_daemon(resource_dir: String, data_dir: String, port: u16) {
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            let curl_bin = resolve_engine_binary(&resource_dir, curl_binary);
            ...
            let app = Router::new()
                ...
                .layer(
                    CorsLayer::new()
                        ...
                        .allow_headers([axum::http::header::CONTENT_TYPE, axum::http::header::AUTHORIZATION, axum::http::header::ORIGIN, axum::http::header::ACCEPT]),
                );
