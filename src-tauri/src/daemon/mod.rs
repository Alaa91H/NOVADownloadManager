...
pub fn start_daemon(resource_dir: String, data_dir: String, port: u16) {
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            let bundled_curl = bin_dir.join(if cfg!(windows) { "curl.exe" } else { "curl" });
            let bundled_ytdlp = bin_dir.join(if cfg!(windows) { "yt-dlp.exe" } else { "yt-dlp" });
            let bundled_ffmpeg = bin_dir.join(if cfg!(windows) { "ffmpeg.exe" } else { "ffmpeg" });
            if bundled_curl.exists() {
                state.curl_bin = bundled_curl.display().to_string();
            }
            if bundled_ytdlp.exists() {
                state.ytdlp_bin = bundled_ytdlp.display().to_string();
            }
            if bundled_ffmpeg.exists() {
                state.ffmpeg_bin = bundled_ffmpeg.display().to_string();
            }
