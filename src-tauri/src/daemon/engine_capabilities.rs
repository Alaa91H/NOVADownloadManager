...
fn libcurl_version_matches_expected() -> bool {
    ...
    expected == "unmanaged" || expected == linked_libcurl_version()
}
...
pub fn validate_linked_libcurl_integrity() -> Result<(), String> {
    ...
    let protocol_set = lower_set(&protocols);
    let feature_set = lower_set(&features);
    if expected != "unmanaged" && expected != linked_libcurl_version() {
        return Err(format!(
...
pub fn ytdlp_status(ytdlp_bin: &str) -> Value {
    ytdlp_status_with_context(ytdlp_bin, false, false)
}
