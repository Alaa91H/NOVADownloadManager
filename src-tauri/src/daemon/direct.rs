...
extern "C" {
    ...
    fn curl_url_get(url: *const CurlUrl, part: CurlUrlPart, value: *mut *mut c_char, flags: c_uint) -> CurlUrlCode;
    fn curl_url_set(url: *mut CurlUrl, part: CurlUrlPart, value: *const c_char, flags: c_uint) -> CurlUrlCode;
...
impl CurlUrlHandle {
    ...
    fn set_url(&self, value: &str) -> Result<(), String> {
        let value = CString::new(value).map_err(|_| "Invalid direct download URL: contains NUL byte".to_string())?;
        let code = unsafe { curl_url_set(self.raw, CURLUPART_URL, value.as_ptr(), 0) };
        if code == CURLUE_OK {
            Ok(())
        } else {
            Err(format!("Invalid direct download URL: {}", curl_url_error(code)))
        }
    }
    ...
    fn get_part(&self, part: CurlUrlPart, flags: c_uint) -> Result<Option<String>, String> {
        ...
        if code != CURLUE_OK {
            return Err(format!("Could not read libcurl URL part: {}", curl_url_error(code)));
        }
        if value.is_null() {
            return Ok(None);
        }
        let text = unsafe { CStr::from_ptr(value) }.to_string_lossy().to_string();
        unsafe { curl_free(value.cast()) };
        Ok(Some(text))
    }
}
...
impl DirectUrl {
    pub fn parse(raw: &str) -> Result<Self, String> {
        ...
        if !matches!(scheme.as_str(), "http" | "https" | "ftp" | "ftps") {
            return Err(format!(
                "Unsupported direct download protocol '{}'. Use HTTP, HTTPS, FTP, or FTPS.",
                scheme
            ));
        }
        let host = parsed.get_part(CURLUPART_HOST, 0)?;
        if matches!(scheme.as_str(), "http" | "https" | "ftp" | "ftps") && host.is_none() {
...
mod tests {
    ...
    fn event_loop_defaults_to_multi_socket() {
        ...
        assert_eq!(EventLoopMode::from_options(&options), EventLoopMode::MultiSocket);

        let mut options = HashMap::new();
        options.insert("eventLoop".to_string(), serde_json::json!("waitPerform"));
        assert_eq!(EventLoopMode::from_options(&options), EventLoopMode::WaitPerform);
    }
