...
fn test_ssl_cert(host: &str, port: u16) -> serde_json::Value {
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    {
        let output = std::process::Command::new("sh")
            .args(["-c", &format!("echo | openssl s_client -connect {}:{} -servername {} 2>/dev/null | openssl x509 -noout -subject -issuer -dates 2>/dev/null || echo 'failed'", host, port, host)])
            .output();
        match output {
    ...
    {
        let script = format!("try {{$req=[System.Net.HttpWebRequest]::Create('https://{0}:{1}/'); $req.GetResponse() | Out-Null; $cert=$req.ServicePoint.Certificate; $subject=$cert.Subject; $issuer=$cert.Issuer; Write-Output \"subject=$subject`nissuer=$issuer\"}} catch {{Write-Error $_}}", host, port);
...
pub async fn run_e2e_test(timeout_secs: u64) -> serde_json::Value {
    ...
    let dns_hosts = [
        ...
        "videolan.org", "speed.cloudflare.com",
    ];
    let dns_results: Vec<serde_json::Value> = dns_hosts.iter().map(|h| test_dns(h)).collect();

    // Test TCP connectivity
    let tcp_hosts = [
        ("cloudflare.com", 443u16), ("google.com", 443u16),
        ("github.com", 443u16), ("videolan.org", 443u16),
    ];
    let tcp_results: Vec<serde_json::Value> = tcp_hosts.iter()
        .map(|(h, p)| test_tcp(h, *p, 5))
        .collect();
    ...
    let ssl_results: Vec<serde_json::Value> = ssl_hosts.iter()
        .map(|h| test_ssl_cert(h, 443))
        .collect();
...
pub async fn full_diagnostics(
    ...
    let summary_status = {
        let dns_ok = e2e.pointer("/dns")
            .and_then(|v| v.as_array())
            .map_or(false, |arr| arr.iter().any(|d| d["resolved"] == true));
        if dns_ok { "ok" } else { "degraded" }
    };

    let network_ok = e2e.pointer("/tcp")
        .and_then(|v| v.as_array())
        .map_or(false, |arr| arr.iter().any(|t| t["reachable"] == true));
