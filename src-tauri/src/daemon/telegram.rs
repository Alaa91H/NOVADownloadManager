...
pub async fn handle_telegram_test(
    ...
    let ok = telegram_send_message(&cfg.token, cfg.chat_id, &msg).await;
    Json(serde_json::json!({"ok": ok}))
}

async fn telegram_send_message(token: &str, chat_id: i64, text: &str) -> bool {
                            ...
                            for update in updates {
                                ...
                                if let Some(msg) = update.get("message") {
                                    if let Some(text) = msg.get("text").and_then(|t| t.as_str()) {
                                        ...
                                        if chat_id == 0 || from_chat == chat_id {
                                            handle_telegram_command(&state, &token, from_chat, text);
                                        }
                                    }
                                }
                            }
...
fn handle_telegram_command(state: &SharedState, token: &str, chat_id: i64, text: &str) {
    let text = text.trim();
    if text.starts_with('/') {
        ...
        match cmd {
            ...
            "/list" => {
                let rt = match tokio::runtime::Runtime::new() {
                    Ok(rt) => rt,
                    Err(e) => {
                        send_telegram_msg_blocking(token, chat_id, &format!("Runtime error: {}", e));
                        return;
                    }
                };
...
pub async fn telegram_notify(state: &SharedState, text: &str) {
    ...
    if cfg.enabled && !cfg.token.is_empty() && cfg.chat_id != 0 {
        telegram_send_message(&cfg.token, cfg.chat_id, text).await;
    }
}
