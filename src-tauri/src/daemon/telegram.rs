...
fn blocking_client() -> Option<&'static reqwest::blocking::Client> {
    ...
    CLIENT.get_or_init(|| {
        reqwest::blocking::Client::builder()
            .build()
            .inspect_err(|e| log::error!("Failed to create blocking HTTP client for Telegram: {e}"))
            .ok()
    })
    .as_ref()
}
        ...
        match cmd {
            ...
            "/add" => {
                ...
                if let Err(e) = crate::daemon::utils::is_safe_target_url(arg) {
                    send_telegram_msg_blocking_with_api(api_base, token, chat_id, &format!("Blocked: {}", e));
                    return;
                }
            ...
            "/pause" | "/resume" | "/delete" => {
                ...
                match cmd {
                    ...
                    "/delete" => {
                        ...
                        let _ = match result {
                            Ok(_) => send_telegram_msg_blocking_with_api(api_base, token, chat_id, &format!("Removed from list: {}", arg)),
                            Err(e) => send_telegram_msg_blocking_with_api(api_base, token, chat_id, &e),
                        };
                    }
                    _ => {}
                }
...
pub async fn handle_telegram_send_file(
    ...
    let raw_path = body.get("path").and_then(|v| v.as_str()).unwrap_or("").trim();
    ...
    let data_dir = std::path::Path::new(&state.data_dir)
        .canonicalize()
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"ok": false, "error": "Cannot resolve data directory"}))))?;

    // Open the file first to get a stable handle (prevents symlink swap race)
    let file = tokio::fs::File::open(&raw_path)
        .await
    ...
    if !canonical.starts_with(&data_dir) {
        return Ok(Json(serde_json::json!({"ok": false, "error": "Access denied: file is outside the data directory"})));
    }
