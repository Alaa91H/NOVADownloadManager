...
pub struct AppState {
    ...
    pub data_dir: String,
    /// aria2 RPC port; re-chosen on every (re)spawn when the default is taken.
    pub aria2_rpc_port: AtomicU16,
    /// Single-flight guard so only one task (re)starts aria2 at a time.
    pub aria2_starting: AtomicBool,
