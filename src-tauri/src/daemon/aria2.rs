...
use crate::daemon::utils::{base64_encode, build_segments, infer_file_type, kill_process, map_aria_status};
...
pub async fn ensure_aria2(state: &SharedState) -> Result<(), String> {
    ...
    let child = Command::new(&state.aria2_bin)
        .args(&args)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
...
pub async fn create_aria2_task(state: &SharedState, body: &CreateDownloadBody) -> Result<Task, String> {
    ...
    if url.to_lowercase().ends_with(".torrent") || url.contains(".torrent?") {
        let resp = state.http_client.get(url).send().await.map_err(|e| format!("Failed to fetch torrent file: {}", e))?;
...
pub async fn resume_task(state: &SharedState, id: &str) -> Result<Task, String> {
    {
        ...
        if let Some(job) = jobs.get_mut(id) {
            if job.task.status != "completed" {
                ...
                drop(jobs);
                crate::daemon::ytdlp::start_ytdlp_process(state, &job_id);
            }
...
pub async fn delete_task(state: &SharedState, id: &str) -> Result<(), String> {
    ...
    aria2_rpc(state, "forceRemove", vec![serde_json::json!(id)]).await
        .map_err(|e| e.to_string())?;
    let _ = aria2_rpc(state, "removeDownloadResult", vec![serde_json::json!(id)]).await;
