...
const ARIA2_BASE_PORT: u16 = 6800;
const ARIA2_PORT_SCAN_LIMIT: u16 = 50;

/// PIDs of every aria2c spawned by this process. Shutdown and daemon restart
/// kill through this list because the owning daemon state is not reachable
/// from the Tauri side once a new daemon instance replaces it.
static SPAWNED_ARIA2_PIDS: StdMutex<Vec<u32>> = StdMutex::new(Vec::new());

pub fn kill_tracked_aria2() {
    let pids: Vec<u32> = match SPAWNED_ARIA2_PIDS.lock() {
...
fn track_aria2_pid(pid: u32) {
    if let Ok(mut guard) = SPAWNED_ARIA2_PIDS.lock() {
        guard.push(pid);
    }
}

fn untrack_aria2_pid(pid: u32) {
    if let Ok(mut guard) = SPAWNED_ARIA2_PIDS.lock() {
        guard.retain(|p| *p != pid);
    }
}

/// One-time startup cleanup: kill aria2c processes left behind by previous
/// NOVA runs — they hold an RPC port with a secret this run no longer knows.
/// Only processes running from our own bundled binary path are touched.
pub fn kill_stale_bundled_aria2(aria2_bin: &str) {
    if !std::path::Path::new(aria2_bin).is_absolute() {
        return;
    }
    #[cfg(windows)]
    {
        let script = format!(
            "Get-Process aria2c -ErrorAction SilentlyContinue | Where-Object {{ $_.Path -eq '{}' }} | Stop-Process -Force",
            aria2_bin.replace('\'', "''")
        );
...
/// A stale aria2 from a previous crashed run may still own the default port,
/// so bind-test and move to the next free one when needed.
fn find_free_rpc_port() -> u16 {
    for offset in 0..ARIA2_PORT_SCAN_LIMIT {
        let port = ARIA2_BASE_PORT + offset;
        if TcpListener::bind(("127.0.0.1", port)).is_ok() {
            if port != ARIA2_BASE_PORT {
                log::warn!("aria2 RPC port {} is busy; using {} instead", ARIA2_BASE_PORT, port);
            }
            return port;
        }
    }
    log::warn!("No free aria2 RPC port found near {}; using it anyway", ARIA2_BASE_PORT);
    ARIA2_BASE_PORT
}

/// True when our aria2 answers RPC with our secret within a short deadline.
async fn probe_rpc(state: &SharedState) -> bool {
    matches!(
        tokio::time::timeout(Duration::from_millis(1500), aria2_rpc(state, "getVersion", vec![])).await,
        Ok(Ok(_))
    )
}

/// Kill and clear the currently tracked aria2 child, if any.
fn discard_aria2_child(state: &SharedState) {
    if let Ok(mut proc) = state.aria2_process.lock() {
        if let Some(mut child) = proc.take() {
            let pid = child.id();
            let _ = child.kill();
            let _ = child.wait();
            untrack_aria2_pid(pid);
        }
    }
}

pub async fn aria2_rpc(state: &SharedState, method: &str, params: Vec<serde_json::Value>) -> Result<serde_json::Value, String> {
    let port = state.aria2_rpc_port.load(Ordering::Relaxed);
...
pub async fn ensure_aria2(state: &SharedState) -> Result<(), String> {
    // Single-flight: concurrent callers wait for whoever holds the guard so
    // two tasks can never probe, kill, or respawn the same child at once.
    if state.aria2_starting.swap(true, Ordering::SeqCst) {
        return wait_for_other_start(state).await;
    }
    let result = ensure_aria2_locked(state).await;
    state.aria2_starting.store(false, Ordering::SeqCst);
    result
}

async fn wait_for_other_start(state: &SharedState) -> Result<(), String> {
    for _ in 0..100 {
        tokio::time::sleep(Duration::from_millis(200)).await;
        if !state.aria2_starting.load(Ordering::SeqCst) {
            if probe_rpc(state).await {
                return Ok(());
            }
            return Err("Aria2 did not become ready in time".to_string());
        }
    }
    Err("Timed out waiting for aria2 to start".to_string())
}

async fn ensure_aria2_locked(state: &SharedState) -> Result<(), String> {
    // Reap a child that already exited so it never masks a dead engine.
    let child_running = {
        let mut proc = state.aria2_process.lock().map_err(|e| e.to_string())?;
        match proc.as_mut() {
            Some(child) => match child.try_wait() {
                Ok(Some(status)) => {
                    log::warn!("aria2 exited unexpectedly ({}); relaunching", status);
                    let pid = child.id();
                    *proc = None;
                    untrack_aria2_pid(pid);
                    false
                ...
                Ok(None) => true,
                Err(e) => {
                    log::warn!("Could not check aria2 process status: {}", e);
                    true
                }
            },
            None => false,
    ...
    if child_running {
        ...
        // Running but not answering with our secret (e.g. it lost the RPC
        // port to an orphaned instance from a previous run) — replace it.
        log::warn!("aria2 is running but RPC is unresponsive; restarting it");
        discard_aria2_child(state);
    }

    spawn_aria2(state).await
}

async fn spawn_aria2(state: &SharedState) -> Result<(), String> {
    let tc = state.torrent_config.lock().map_err(|e| e.to_string())?.clone();

    let rpc_port = find_free_rpc_port();
    state.aria2_rpc_port.store(rpc_port, Ordering::SeqCst);
    ...
    let mut args = vec![
        ...
        // aria2 exits with NOVA so app quits and crashes never leave an
        // orphaned engine holding the RPC port for the next launch.
    ...
    for _ in 0..30 {
        if aria2_rpc(state, "getVersion", vec![]).await.is_ok() {
            log::info!("Aria2 daemon started (pid {}, rpc port {})", pid, rpc_port);
            return Ok(());
        }
        tokio::time::sleep(Duration::from_millis(200)).await;
    }

    // Never became ready: clear the child so the next attempt respawns
    // instead of trusting a broken process forever.
    discard_aria2_child(state);
