...
fn probe_payload(url: &str, final_url: &str, headers: &reqwest::header::HeaderMap, status: u16, method: &str) -> serde_json::Value {
    ...
    let supports_ranges = accept_ranges.eq_ignore_ascii_case("bytes") || content_range.to_ascii_lowercase().starts_with("bytes ");
    let ranged_total = content_range
        .rsplit('/')
        .next()
        .and_then(|value| value.parse::<u64>().ok())
        .unwrap_or(0);
    let size = if ranged_total > 0 { ranged_total } else { content_length };
    let final_name = content_disposition_filename(&content_disposition).unwrap_or_else(|| fallback_file_name(final_url));
    ...
    serde_json::json!({
        ...
        "sizeBytes": size,
        "resumable": supports_ranges,
        "supportsSegments": supports_ranges && size > 0,
...
async fn probe_url_with_options(
    ...
    if let Ok(resp) = apply_probe_request_options(
        client.head(url).timeout(Duration::from_secs(8)),
        body,
    )
    .send()
    .await
    ...
    if let Ok(resp) = apply_probe_request_options(
        client.get(url).header("range", "bytes=0-0").timeout(Duration::from_secs(8)),
        body,
    )
    .send()
    .await
    {
        ...
        if status < 400 || status == 416 {
            return Ok(Json(probe_payload(url, &final_url, resp.headers(), status, "GET range=0-0")));
        }
    }
    ...
    Ok(Json(serde_json::json!({
        "url": url,
        "finalUrl": url,
        "fileName": fallback_file_name(url),
        "fileType": infer_file_type(&fallback_file_name(url)),
        "sizeBytes": 0,
        "resumable": false,
    ...
    if let Some(candidates) = body.get("candidates").and_then(|v| v.as_array()) {
        for candidate in candidates {
            let wrapper = serde_json::json!({
                "candidate": candidate,
                "source": body.get("source").cloned().unwrap_or_else(|| serde_json::json!("adm-extension")),
