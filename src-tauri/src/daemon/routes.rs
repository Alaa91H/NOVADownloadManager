...
use reqwest::header::{HeaderName, HeaderValue, ACCEPT_ENCODING, RANGE};
...
const PROBE_USER_AGENT: &str = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36";
...
pub async fn handle_health(State(state): State<SharedState>) -> Json<serde_json::Value> {
    let status = engine_capabilities::all_engine_status(&state.curl_bin, &state.ytdlp_bin, &state.ffmpeg_bin);
...
pub async fn handle_engine_capabilities(State(state): State<SharedState>) -> Json<serde_json::Value> {
    Json(engine_capabilities::all_engine_status(&state.curl_bin, &state.ytdlp_bin, &state.ffmpeg_bin))
}
...
pub async fn handle_v1_pair_auto(Json(_body): Json<serde_json::Value>) -> Json<serde_json::Value> {
    let nanos = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let token = format!("nova-local-pair-{}-{}-runtime", std::process::id(), nanos);
    Json(serde_json::json!({
        "ok": true,
        "pairToken": token,
        "autoApproved": true,
...
pub async fn handle_add_torrent(
    ...
    Json(_body): Json<TorrentBody>,
) -> Result<Json<Task>, (StatusCode, Json<serde_json::Value>)> {
    Err((
        StatusCode::BAD_REQUEST,
        Json(serde_json::json!({
            "error": "Torrent/magnet support was provided by the legacy aria2 engine and is disabled after switching the direct-download core to curl. Add a dedicated torrent engine before re-enabling this endpoint."
        })),
    ))
}
...
fn content_disposition_filename(value: &str) -> Option<String> {
    for part in value.split(';') {
        ...
        if let Some(name) = trimmed.strip_prefix("filename*=") {
            let name = name.split("''").last().unwrap_or(name).trim_matches('"').trim();
            if !name.is_empty() {
...
/// 1. HEAD with browser UA + identity encoding  (fast, minimal traffic)
...
async fn probe_url_with_options(
    ...
    // ── Stage 1: HEAD ──────────────────────────────────────────────────────
    // Use a browser-like UA by default unless the caller overrides it via
    // directOptions.userAgent to avoid 403s from CDNs / hotlink protection.
    ...
    .await
    {
        ...
        if status < 400 {
            ...
            if has_size && has_range {
                // Perfect result – size and ranges confirmed in one request.
                return Ok(Json(payload));
            }
            if has_size {
                // Size known but server didn't declare ranges; keep as best
                // candidate and continue to range probe.
    ...
    // Even servers that don't answer HEAD correctly usually respond to a
    // one-byte ranged GET with a 206 that carries Content-Range with total.
    if let Ok(resp) = apply_probe_request_options(
        client
            .get(url)
    ...
    .await
    {
        let status = resp.status().as_u16();
        let final_url = resp.url().to_string();
        // 206 = Partial Content (range accepted)
        // 416 = Range Not Satisfiable – server still returns Content-Range
        // 200 = Server ignored range but we still get headers
        if status == 206 || status == 416 {
            let payload = probe_payload(url, &final_url, resp.headers(), status, "GET range=0-0");
            return Ok(Json(payload));
        }
        if status < 400 {
            let payload = probe_payload(url, &final_url, resp.headers(), status, "GET range=0-0 (no-range)");
            let has_size = payload.get("sizeBytes").and_then(|v| v.as_u64()).unwrap_or(0) > 0;
            if has_size {
                // Server returned size but doesn't support ranges.
                // Only update best if we don't have one yet.
    ...
    // Some servers (especially nginx with SendFile) respond differently to a
    // larger range.  We only use this if stages 1 & 2 gave us nothing useful.
    if best_payload.is_none() {
        if let Ok(resp) = apply_probe_request_options(
            client
                .get(url)
        ...
        .await
        {
            ...
            if status == 206 || status < 400 {
                let payload = probe_payload(url, &final_url, resp.headers(), status, "GET range=0-1023");
                let has_size = payload.get("sizeBytes").and_then(|v| v.as_u64()).unwrap_or(0) > 0;
                if has_size {
                    return Ok(Json(payload));
                }
...
pub async fn handle_browser_ext_config(
    ...
    let capabilities = engine_capabilities::all_engine_status(&state.curl_bin, &state.ytdlp_bin, &state.ffmpeg_bin);
    ...
    Json(serde_json::json!({
        ...
        "version": env!("CARGO_PKG_VERSION"),
        "captureEndpoint": "/captures",
        "directDownloads": capabilities.get("directReady").cloned().unwrap_or_else(|| serde_json::json!(false)),
        "mediaDownloads": capabilities.get("mediaReady").cloned().unwrap_or_else(|| serde_json::json!(false)),
        "postProcessing": capabilities.get("postProcessingReady").cloned().unwrap_or_else(|| serde_json::json!(false)),
        "directEngine": "libcurl-multi",
        "mediaEngine": "yt-dlp",
        "postProcessor": "ffmpeg",
...
fn disk_free_gb(path: &str) -> u64 {
    #[cfg(target_os = "windows")]
    {
        ...
        let script = format!("$p='{}'; $drive=(Get-Item -LiteralPath $p).PSDrive; [math]::Round($drive.Free / 1GB)", escaped);
        if let Ok(output) = hidden_output("powershell", &["-NoProfile", "-NonInteractive", "-Command", &script]) {
            if output.status.success() {
                return String::from_utf8_lossy(&output.stdout).trim().parse::<u64>().unwrap_or(0);
            }
        }
...
pub async fn handle_diagnostics(State(state): State<SharedState>) -> Json<serde_json::Value> {
    ...
    let uptime_secs = DAEMON_START
        ...
        .unwrap_or(0);
    let engine_capabilities = engine_capabilities::all_engine_status(&state.curl_bin, &state.ytdlp_bin, &state.ffmpeg_bin);
    let curl_available = engine_capabilities.pointer("/engines/curl/available").and_then(|v| v.as_bool()).unwrap_or(false);
    let ytdlp_available = engine_capabilities.pointer("/engines/ytdlp/available").and_then(|v| v.as_bool()).unwrap_or(false);
    let ffmpeg_available = engine_capabilities.pointer("/engines/ffmpeg/available").and_then(|v| v.as_bool()).unwrap_or(false);
    Json(serde_json::json!({
        "version": env!("CARGO_PKG_VERSION"),
        "daemonVersion": env!("CARGO_PKG_VERSION"),
        "pid": std::process::id(),
        "uptime": uptime_secs,
        "jobs": media_jobs_count + curl_jobs_count,
        "curlJobs": curl_jobs_count,
        "mediaJobs": media_jobs_count,
        "curlAvailable": curl_available,
        "curlVersion": curl_version(&state.curl_bin),
        ...
        "memoryUsageMb": process_memory_usage_mb(),
        "diskFreeGb": disk_free_gb(&state.data_dir),
        "activeThreads": media_jobs_count + curl_jobs_count,
...
fn extension_candidate_to_download_body(body: &serde_json::Value, start_immediately: bool) -> Result<CreateDownloadBody, String> {
    ...
    let media_options = if is_stream_manifest {
        let mut media = crate::daemon::types::MediaDownloadOptions::default();
        media.mode = Some("video".to_string());
        media.playlist = Some(false);
        media.ffmpeg_enabled = Some(true);
        media.embed_metadata = Some(true);
        media.concurrent_fragments = Some(8);
        media.retries = Some(5);
        media.fragment_retries = Some(10);
        media.referer = referer.clone();
        Some(media)
    } else {
        None
    };
...
pub async fn handle_v1_stream_add(
    ...
    let mut media_options = crate::daemon::types::MediaDownloadOptions::default();
    media_options.mode = Some("video".to_string());
    media_options.playlist = Some(false);
    media_options.ffmpeg_enabled = Some(true);
    media_options.embed_metadata = Some(true);
    media_options.concurrent_fragments = Some(8);
    media_options.retries = Some(5);
    media_options.fragment_retries = Some(10);
    media_options.referer = manifest.get("referrer").or_else(|| manifest.get("pageUrl")).and_then(|v| v.as_str()).map(|v| v.to_string());
