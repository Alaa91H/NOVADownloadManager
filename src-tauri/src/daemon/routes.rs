...
pub async fn handle_health(State(state): State<SharedState>) -> Json<serde_json::Value> {
    ...
    .and_then(|r| r.ok())
    .is_some();
    let aria2_available = std::path::Path::new(&state.aria2_bin).exists();
    ...
    let ytdlp_version = YTDLP_VERSION.get_or_init(|| {
        ...
        std::process::Command::new(&ytdlp_bin)
            .arg("--version")
            .output()
            .ok()
...
pub async fn handle_ytdlp_probe(
    ...
    let output = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || {
            std::process::Command::new(&ytdlp_bin)
                .args(["--dump-json", "--no-playlist", "--no-warnings", &url2])
                .output()
        }),
    )
    .await
...
pub async fn handle_ytdlp_probe_playlist(
    ...
    let output = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || {
            std::process::Command::new(&ytdlp_bin)
                .args(["--flat-playlist", "--dump-json", "--no-warnings", &url2])
                .output()
        }),
    )
    .await
...
pub async fn handle_v1_stream_resolve(
    ...
    let resolved = if std::path::Path::new(&state.ytdlp_bin).exists() {
        ...
        tokio::time::timeout(
            ...
            tokio::task::spawn_blocking(move || {
                std::process::Command::new(&ytdlp_bin)
                    .args(["--dump-json", "--no-playlist", "--no-warnings", "--skip-download", &url2])
                    .output()
                    .map(|o| o.status.success())
                    .unwrap_or(false)
            }),
