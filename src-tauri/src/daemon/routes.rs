...
/// Parsed Content-Range header (RFC 7233 §4.2).
///
/// Supports:
/// - `bytes 0-99/10000`  → unit=bytes, start=0, end=99, total=10000
/// - `bytes */10000`     → unit=bytes, unsatisfied=true, total=10000
/// - Malformed or unknown unit → returns None
#[derive(Clone, Debug, PartialEq, Eq)]
struct ContentRangeParsed {
    unit: String,
    start: Option<u64>,
    end: Option<u64>,
    total: Option<u64>,
...
impl ContentRangeParsed {
    fn parse(header: &str) -> Option<Self> {
        ...
        // byte-range-spec: `START-END/TOTAL` or `START-*/TOTAL`
...
/// Split a Content-Disposition header value into `;`-separated parameters,
/// respecting quoted strings so that semicolons inside `"..."` are not treated
/// as parameter separators.
fn split_cd_params(value: &str) -> Vec<&str> {
...
/// Unescape a quoted-string value per RFC 7230 §3.2.6:
/// Remove surrounding `"..."` and replace `\"` with literal `"`.
fn unescape_quoted_string(s: &str) -> &str {
    let s = s.trim();
...
/// Resolve backslash escapes inside a quoted-string value (`\"` → `"`).
fn resolve_quote_escapes(s: &str) -> String {
...
fn content_disposition_filename(value: &str) -> Option<String> {
    ...
    // First pass: filename* (RFC 5987) — highest priority.
    // RFC 6266 §4: parameter names are case-insensitive.
    for param in &params {
...
/// Percent-decode a string interpreting raw bytes as ISO-8859-1 (Latin-1).
/// Each byte 0x00–0xFF maps to the same Unicode code point, so the result
/// is always valid UTF-8.
fn percent_decode_str_iso8859(input: &str) -> String {
    let bytes = input.as_bytes();
    ...
    while i < bytes.len() {
        if bytes[i] == b'%' && i + 2 < bytes.len() {
            if let Some(byte) = hex_pair_to_byte(bytes[i + 1], bytes[i + 2]) {
                // ISO-8859-1 byte values 0x00–0xFF map directly to Unicode
...
/// Extract a SHA-256 digest from `Content-Digest`, `Digest`, or `Repr-Digest`
/// headers (RFC 3230 / RFC 9530).  Supports both base64 (`:BASE64:`) and hex
/// formats.  Returns the hex-encoded value for comparison.
fn extract_sha256_digest(headers: &reqwest::header::HeaderMap) -> Option<String> {
...
async fn resolve_direct_body_for_immediate_download(
    ...
    // Forward Content-Digest / Link mirror metadata from the probe into
    // direct_options so the download engine can use them (Want-Digest
    // response verification and MirrorManager registration).
    if let Some(digest) = json_non_empty_str(&payload, "digestSha256") {
        let options = body.direct_options.get_or_insert_with(HashMap::new);
        options.insert(
    ...
    // Forward ETag and Last-Modified for conditional resume (RFC 7232).
    // Without these, the download engine cannot send If-Range headers,
    // risking silent file corruption on resume.
    if let Some(etag) = json_non_empty_str(&payload, "etag") {
        let options = body.direct_options.get_or_insert_with(HashMap::new);
        options
            .entry("etag".to_string())
            .or_insert_with(|| serde_json::Value::String(etag.to_string()));
    }
    if let Some(last_modified) = json_non_empty_str(&payload, "lastModified") {
        let options = body.direct_options.get_or_insert_with(HashMap::new);
        options
            .entry("lastModified".to_string())
            .or_insert_with(|| serde_json::Value::String(last_modified.to_string()));
    }
    if let Some(mirrors) = payload.get("linkMirrors").and_then(|v| v.as_array()) {
        if !mirrors.is_empty() {
            let options = body.direct_options.get_or_insert_with(HashMap::new);
            options.insert(
                "linkMirrors".to_string(),
                serde_json::Value::Array(mirrors.clone()),
            );
            // Forward per-mirror priorities from `pri=N` Link parameters (RFC 6249 §4).
            if let Some(prios) = payload.get("mirrorPriorities").and_then(|v| v.as_array()) {
                options.insert(
                    "mirrorPriorities".to_string(),
                    serde_json::Value::Array(prios.clone()),
                );
            }
        }
    }

    // Connection counts are no longer hard-coded per host: the download engine
    // starts with the requested connections and, if a host stalls or rejects
    // parallel range connections, it falls back to a single stream and records a
    // learned per-host ceiling so later downloads adapt automatically.

    // Ensure the original URL is set as the Referer so the CDN
    // does not block the download for missing Referer header.
    // Some mirror/interstitial hosts require this.
    if body
        .referer
        .as_deref()
        .map(str::trim)
...
fn browser_ext_response(state: &SharedState) -> Json<serde_json::Value> {
...
mod meta_refresh_tests {
    use super::{decode_html_entities, parse_meta_refresh_url};

    #[test]
    fn extracts_single_quoted_uppercase_url() {
        // VideoLAN-style interstitial: `URL='...'` (uppercase, single quotes).
    ...
    fn extracts_bare_url_and_decodes_entities() {
        // SourceForge-style interstitial: bare `url=` with `&amp;`-escaped query.
