...
for (const file of files) {
  ...
  if (!allowedLoopbackLiteral(name) && /http:\/\/127\.0\.0\.1|ws:\/\/127\.0\.0\.1/.test(text)) {
    violations.push(`${name}: loopback literals must stay behind TransportManager or explicit Open ADM/permission boundaries.`);
  }
...
console.log('Architecture guard passed: ADM access remains behind BridgeManager/TransportManager boundaries.');
