...
const requiredDocs = [
  ...
  '../docs/extension/NOVA_BROWSER_EXTENSION_DESKTOP_DEVELOPER_HANDOFF.md',
  '../docs/extension/DESKTOP_RUNTIME_REQUIREMENTS.md',
];

const allMissing = requiredDocs.every((p) => !existsSync(p));
if (allMissing) {
  console.log('Documentation readiness check skipped: root docs/extension and docs/release directories are not present.');
  process.exit(0);
}
...
const checks: Array<{ path: string; terms: string[] }> = [
  ...
  {
    path: '../docs/extension/NOVA_BROWSER_EXTENSION_DESKTOP_DEVELOPER_HANDOFF.md',
    terms: [
      'single-instance',
      'Minimize to system tray',
      'Default: ON',
  ...
  {
    path: '../docs/extension/DESKTOP_RUNTIME_REQUIREMENTS.md',
    terms: [
      'single-instance',
      'process lock',
      'Minimize to system tray',
  ...
  {
    path: '../docs/extension/ZERO_CLICK_PAIRING.md',
    terms: [
      'Zero-click',
      'Native Messaging',
      '/v1/pair/auto',
