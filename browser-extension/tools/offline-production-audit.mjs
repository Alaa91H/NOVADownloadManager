...
const requiredFiles = [
  ...
  '.github/actions/setup-extension-ci/action.yml',
...
if (read('.nvmrc').trim() !== '24') fail('.nvmrc must stay pinned to Node 24');
...
if (scripts['verify:offline'] !== 'pnpm preflight:production && pnpm audit:offline && pnpm audit:release') {
  fail('package.json verify:offline must chain preflight, offline audit, and release audit');
}
...
for (const term of ['Download unpacked browser builds', 'adm-extension-unpacked', 'EXTENSION_UNPACKED_DIR: dist/chromium', 'Run Playwright smoke tests against the existing Chromium build']) {
...
assertIncludes('.github/actions/setup-extension-ci/action.yml', 'version: 11.6.0', 'pnpm action must pin version 11.6.0');
assertIncludes('.github/actions/setup-extension-ci/action.yml', 'node-version-file: .nvmrc', 'setup-node must use .nvmrc');
assertIncludes('.github/actions/setup-extension-ci/action.yml', 'package-manager-cache: false', 'setup-node cache must stay disabled until lockfile policy changes');
assertIncludes('Dockerfile.ci', 'FROM node:24', 'Dockerfile.ci must pin Node 24');
assertIncludes('Dockerfile.ci', 'corepack prepare pnpm@11.6.0 --activate', 'Dockerfile.ci must activate pnpm 11.6.0');
...
assertRegex('.devcontainer/devcontainer.json', /"image"\s*:\s*"mcr\.microsoft\.com\/devcontainers\/javascript-node:1-24-bookworm"/, 'devcontainer must use Node 24 image');
...
const scanner = read('src/content/scanner.ts');
const scannerTerms = [
  ...
  'keyboardNudgePx',
  'adm.downloadOverlayPosition.v2',
  'adm.downloadOverlayDiagnostics.v1',
];
for (const term of scannerTerms) {
  if (!scanner.includes(term)) fail(`src/content/scanner.ts floating overlay hardening term missing: ${term}`);
}
const diagnosticsWriter = scanner.split('function writeOverlayClientDiagnostics', 2)[1]?.split('function overlayRectSnapshot', 1)[0] ?? '';
...
if (!router.includes("browser.storage.local.remove([OVERLAY_DIAGNOSTICS_STORAGE_KEY, 'adm.diagnostics'])")) {
  fail('message router must clear overlay and legacy diagnostics together');
}
if (!router.includes("key.startsWith('adm.downloadOverlayPosition.v2.')")) {
...
for (const term of ['RUN_REAL_EXTENSION_E2E', '#adm-video-download-overlay-host', '.adm-video-download-label', '#adm-candidate-picker-host']) {
