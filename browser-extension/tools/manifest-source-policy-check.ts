...
assert(manifest.manifest_version === 3, 'src/manifest.json must remain MV3-compatible for ADM-extension layout compatibility.');
...
assert(!/127\.0\.0\.1:\*/.test(csp) && !/localhost:\*/.test(csp), 'Loopback CSP must pin ADM port 3199 and must not use wildcard ports.');
assert(csp.includes('http://127.0.0.1:3199') && csp.includes('ws://127.0.0.1:3199'), 'CSP must allow only the official ADM loopback port for HTTP and WebSocket.');
