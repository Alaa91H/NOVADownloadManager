...
/** Keys that child processes need to function correctly. */
const CHILD_SAFE_ENV_KEYS = new Set([
  'PATH', 'HOME', 'USERPROFILE', 'APPDATA', 'LOCALAPPDATA',
  'SystemRoot', 'ProgramFiles', 'ProgramFiles(x86)',
  'NODE_VERSION', 'PNPM_HOME',
  // Build-specific
  'BUILD_TAG', 'VITE_APP_VERSION', 'WXT_VERSION',
  'NOVA_CURL', 'NOVA_YTDLP', 'NOVA_FFMPEG',
  'NOVA_LIBCURL_FEATURE_PROFILE', 'CMAKE_TOOLCHAIN_FILE', 'VCPKG_TARGET_TRIPLET',
]);

function run(command: string, args: string[], env: typeof process.env = process.env): Promise<void> {
  const filteredEnv = Object.fromEntries(
    Object.entries(env).filter(([key]) => CHILD_SAFE_ENV_KEYS.has(key)),
  );
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, { stdio: 'inherit', shell: process.platform === 'win32', env: filteredEnv });
    child.on('exit', (code) => {
      if (code === 0) resolve();
