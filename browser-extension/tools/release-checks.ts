...
function validateManifestVersion(raw: string): string {
  const version = raw.trim().replace(/^v/, '').split('+', 1)[0];
  if (!version) {
    throw new Error(
      `Invalid extension version: ${raw}. Use a Git tag like v1.2.3, v1.2.3.4, or v1.2.3-beta.4.`,
    );
  }
  if (!/^\d+\.\d+\.\d+(?:\.\d+)?$/.test(version)) {
    ...
    if (prerelease) {
      ...
      const prereleaseNumber = prereleaseLabel
        .split('.')
