...
const metadataNames = listFiles(assetsDir).filter((file) => ['release-manifest.json', 'SHA256SUMS.txt', 'CHANGELOG.md'].includes(file));
...
const body = [
  ...
  '## Integrity',
  '',
  'Use `SHA256SUMS.txt` from the release assets to verify downloaded packages.',
  '',
].join('\n');

const telegramChangeLines = lines.map(formatTelegramChangeLine).filter(Boolean).slice(0, 8);
const checksumAsset = metadataRows.find(({ file }) => file === 'SHA256SUMS.txt');
const notification = [
  ...
  checksumAsset ? `â€¢ <a href="${escapeHtml(checksumAsset.url)}">SHA256 checksums</a>` : '',
  '',
  'ðŸ“ <b>Change log:</b>',
