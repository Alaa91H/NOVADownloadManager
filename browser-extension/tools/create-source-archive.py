...
OUT = ROOT / 'dist' / 'packages' / 'ADM-extension-source.zip'
...
with zipfile.ZipFile(OUT, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
    for path in sorted((p for p in ROOT.rglob('*') if should_include(p)), key=lambda item: item.relative_to(ROOT).as_posix()):
