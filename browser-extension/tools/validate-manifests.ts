...
async function validateTarget(target: BrowserTarget, path: string): Promise<void> {
  ...
  assert(manifest.name === '__MSG_extensionName__' || manifest.name === '__MSG_extensionNameEdge__' || manifest.name === 'NOVA Download Manager Extension' || manifest.name === 'NOVA Download Manager Extension for Edge', `${target}: expected ADM-extension product name or localized manifest token.`);
