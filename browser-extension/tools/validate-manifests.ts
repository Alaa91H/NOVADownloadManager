...
async function validateTarget(target: BrowserTarget, path: string): Promise<void> {
  ...
  // Video capture popup must be packaged and wired to the toolbar action.
  assert(manifest.action?.default_popup === 'popup.html', `${target}: action.default_popup must be popup.html.`);
  assert(await pathExists(join(dirname(path), 'popup.html')), `${target}: popup.html must be packaged.`);
