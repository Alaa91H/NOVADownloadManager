...
def validate_version(raw: str) -> str:
    version, _, build = raw.strip().removeprefix("v").partition("+")
    if re.match(r"^\d+\.\d+\.\d+(?:\.\d+)?$", version):
        numeric_build_parts = [part for part in build.split(".") if part.isdecimal()]
        if re.match(r"^\d+\.\d+\.\d+$", version) and numeric_build_parts:
            return f"{version}.{numeric_build_parts[-1]}"
        return version
    ...
    raise SystemExit(
        f"Invalid extension version: {raw}. Use a tag like v1.2.3, v1.2.3.4, v1.2.3-beta.4, or v1.2.3+45"
    )
