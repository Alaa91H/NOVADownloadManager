...
def test_loopback_url_policy_is_enforced_by_transports():
    ...
    assert "assertAdmLoopbackOrigin" in policy
    ...
    assert "DEFAULT_ADM_LOOPBACK_HTTP_URL" in http
    assert "buildAdmLoopbackHttpUrl" in http
    assert "`${this.baseUrl}${route}`" not in http

    sse = read("src/transport/sse-transport.ts")
    ws = read("src/transport/websocket-transport.ts")
    assert "assertAdmLoopbackHttpUrl" in sse
    assert "assertAdmLoopbackWsUrl" in ws
