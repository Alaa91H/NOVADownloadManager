...
def test_popup_uses_polished_cards_and_status_pills():
    ...
    assert 'adm-popup adm-connection-popup' in popup
    assert 'adm-connection-header' in popup
    assert 'adm-connection-panel' in popup
    assert 'adm-header-tools' in popup
    assert 'adm-theme-toggle' in popup
    assert 'color-scheme: dark;' in css
    assert ':root[data-theme="light"]' in css
    assert 'className="adm-candidate"' in candidate_list
    assert 'adm-pill' in connection
...
def test_options_have_control_center_navigation_and_clear_aggressive_copy():
    ...
    assert 'adm-settings-shell' in options
    assert 'adm-sidebar-nav' in options
...
def test_diagnostics_are_structured_not_raw_only():
    ...
    assert 'adm-diagnostics-grid' in panel
