...
def test_setup_node_cache_disabled_until_lockfile_exists():
    action = (ROOT / '../docs/extension/ci-templates/setup-extension-ci-action.yml').read_text()
...
def test_pnpm_setup_pins_requested_version_without_v10_layout_noise():
    action = (ROOT / '../docs/extension/ci-templates/setup-extension-ci-action.yml').read_text()
    pnpm_block = action.split('uses: pnpm/action-setup@v6.0.9', 1)[1].split('    - name: Setup Node.js', 1)[0]
...
def test_extension_preflight_uses_root_lockfile():
    preflight = (ROOT / 'tools/preflight.mjs').read_text()
    audit = (ROOT / 'tools/offline-production-audit.mjs').read_text()
    assert "'../pnpm-lock.yaml'" in preflight
    assert "'../pnpm-lock.yaml'" in audit
    assert "'pnpm-lock.yaml'" not in preflight.split('const required = [', 1)[1].split('];', 1)[0]
