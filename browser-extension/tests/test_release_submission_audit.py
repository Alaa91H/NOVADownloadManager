...
def test_release_submission_audit_is_dependency_light_and_store_policy_rich() -> None:
    ...
    assert "../docs/extension/ci-templates/Dockerfile.ci" in audit
    assert "../docs/extension/ci-templates/devcontainer.json" in audit
...
def test_release_submission_audit_is_wired_into_package_ci_and_reproducible_env() -> None:
    ...
    dockerfile = read('../docs/extension/ci-templates/Dockerfile.ci')
    bootstrap = read('scripts/bootstrap-node24-pnpm.sh')
    devcontainer = read('../docs/extension/ci-templates/devcontainer.json')
