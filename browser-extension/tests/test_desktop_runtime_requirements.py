...
def test_desktop_runtime_contract_is_enforced_by_runtime_files() -> None:
    native_manifest = read("native-messaging/com.nova.downloadmanager.json")
    native_transport = read("src/transport/native-transport.ts")
    protocol = read("src/contracts/nova.protocol.v4.ts")
    schema = read("contracts/nova.protocol.v4.schema.json")

    assert "com.nova.downloadmanager" in native_manifest
    ...
    assert "host = 'com.nova.downloadmanager'" in native_transport
    assert "NOVA_PROTOCOL_VERSION = 4" in protocol
    assert '"protocolVersion"' in schema
    assert '"const": 4' in schema


def test_extension_ui_exposes_one_click_link_with_nova_fallback() -> None:
    ...
    assert "Link with NOVA" in popup_status
    assert "Link with NOVA" in options
