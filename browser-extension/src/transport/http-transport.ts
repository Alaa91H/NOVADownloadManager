...
import { DEFAULT_ADM_LOOPBACK_HTTP_URL, buildAdmLoopbackHttpUrl } from './loopback-url-policy';
...
export class HttpTransport implements Transport {
  ...
  constructor(private readonly baseUrl = DEFAULT_ADM_LOOPBACK_HTTP_URL) {}

  url(route: string): string {
    return buildAdmLoopbackHttpUrl(this.baseUrl, route);
  }
  ...
  async request<T>(route: string, payload: unknown, schema: z.ZodType<T>, options: HttpTransportOptions = {}): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new AdmExtensionError({ ...parsed, status: response.status });
      }
      return schema.parse(data);
    } catch (error) {
      if (error instanceof z.ZodError) {
        throw new AdmExtensionError({ code: 'VALIDATION_FAILED', message: 'Response validation failed.', retryable: false, details: error.issues });
      }
      throw toAdmExtensionError(error, error instanceof DOMException && error.name === 'AbortError' ? 'TIMEOUT' : 'NETWORK_ERROR');
    } finally {
