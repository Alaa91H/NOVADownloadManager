...
import { assertAdmLoopbackHttpUrl } from './loopback-url-policy';
...
export class SseTransport {
  ...
  async connect(url: string, token: string, handlers: SseHandlers): Promise<void> {
    this.close();
    const safeUrl = assertAdmLoopbackHttpUrl(url);
    ...
    try {
      while (!controller.signal.aborted) {
        ...
        if (byteLength(buffer) > MAX_SSE_BUFFER_BYTES) {
          throw new AdmExtensionError({ code: 'VALIDATION_FAILED', message: 'SSE buffer exceeded the safe event budget.', retryable: true });
        }
        ...
        for (const rawEvent of events) {
          ...
          const parsed = AdmEventSchema.safeParse(parsedJson);
          if (parsed.success) {
          ...
          if (parseErrors > MAX_EVENT_PARSE_ERRORS_PER_CONNECTION) {
            throw new AdmExtensionError({ code: 'VALIDATION_FAILED', message: 'SSE event stream produced too many invalid events.', retryable: true });
          }
        }
      }
    } catch (error) {
