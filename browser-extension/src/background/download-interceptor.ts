  ...
  if (typeof item.id === 'number' && !handoffAttempted.has(item.id) && shouldTakeover(item, settings)) {
    ...
    if (engine.shouldAutoSend(candidate)) {
      try {
        ...
        // Only cancel the browser download if ADM confirmed acceptance.
        // 'sent' = delivered to ADM; 'pending' = persisted in outbox for guaranteed delivery.
        ...
        // If ADM failed (job.status === failed/dead-letter), browser download continues naturally
      } catch {
        // ADM handoff failed — browser download continues
      }
      return;
    }
  }
