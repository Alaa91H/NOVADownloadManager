...
async function openAdm(captureUrl?: string): Promise<{ ok: true }> {
...
async function maybeOpenAdm(captureUrl?: string): Promise<void> {
  const settings = await settingsStore.get();
  if (settings.openAdmAfterSend) await openAdm(captureUrl);
}
