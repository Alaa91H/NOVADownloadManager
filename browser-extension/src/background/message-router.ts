...
import { SettingsSchema, type Settings } from '../contracts/settings.schema';
...
import { AdmExtensionError, toAdmExtensionError } from '../core/error-classification';
...
async function dispatchMessage(msg: RuntimeMessage, sender?: RuntimeMessageSenderLike): Promise<unknown> {
  ...
  switch (msg.type) {
    ...
    case 'SEND_BATCH':
      return bridgeManager.sendBatch(msg.candidates).then(async (job) => { await maybeOpenAdm(); return job; });
    case 'RESOLVE_STREAM':
      return bridgeManager.resolveStream({ manifestType: msg.manifestType, url: msg.url, pageUrl: msg.pageUrl });
    case 'SEND_STREAM':
      return sendStream(msg.candidateId, msg.selectedQualityUrl);
...
async function sendStream(candidateId: string, selectedQualityUrl?: string): Promise<unknown> {
  const activeTabId = await getActiveTabId();
  const candidates = await cache.get(activeTabId);
  const candidate = candidates.find((c) => c.id === candidateId);
  if (!candidate) {
    throw new AdmExtensionError({ code: 'VALIDATION_FAILED', message: 'Candidate not found in cache for this tab.', retryable: false });
  }
  ...
  const selectedQuality = selectedQualityUrl
    ...
    : undefined;
  const idempotencyKey = await idempotencyKeyFor([candidate]);
...
async function overlaySendSelected(sender: RuntimeMessageSenderLike | undefined, candidateIds: string[]): Promise<unknown> {
  ...
  if (chosen.length === 0) {
    ...
    throw new AdmExtensionError({ code: 'VALIDATION_FAILED', message: 'None of the selected candidates are handoffable for this tab.', retryable: false });
  }
  await bridgeManager.sendBatch(chosen).then(async () => { await maybeOpenAdm(); });
...
async function updateSettings(partial: Record<string, unknown>): Promise<unknown> {
  ...
  const next = SettingsSchema.parse({
    ...
    overlay: { ...current.overlay, ...(typeof partial.overlay === 'object' && partial.overlay ? partial.overlay : {}) },
    popup: { ...current.popup, ...(typeof partial.popup === 'object' && partial.popup ? partial.popup : {}) },
  });
...
async function overlayPositionDiagnostics(): Promise<Record<string, number | boolean>> {
  try {
    ...
    const scoped = keys.filter((key) => key.startsWith('adm.downloadOverlayPosition.v2.')).length;
    return {
      legacyGlobalPresent: Object.prototype.hasOwnProperty.call(all, 'adm.videoOverlayPosition.v1'),
      scopedPositions: scoped,
      totalPositions: scoped + (Object.prototype.hasOwnProperty.call(all, 'adm.videoOverlayPosition.v1') ? 1 : 0),
    };
  } catch {
...
async function clearLocalData(
  ...
  const overlayPositionKeys = Object.keys(all).filter(
    (key) => key === 'adm.videoOverlayPosition.v1' || key.startsWith('adm.downloadOverlayPosition.v2.'),
  );

  if (scope === 'diagnostics' || scope === 'overlay-diagnostics' || scope === 'all-local') {
    await browser.storage.local.remove([OVERLAY_DIAGNOSTICS_STORAGE_KEY, 'adm.diagnostics']);
  }

  if (scope === 'overlay-positions' || scope === 'all-local') {
    await browser.storage.local.remove(
      overlayPositionKeys.length > 0 ? overlayPositionKeys : ['adm.videoOverlayPosition.v1'],
    );
  }
  ...
  if (scope === 'all-local') {
    await Promise.all([
      ...
      browser.storage.local.remove(['adm.settings', 'adm.bridgeState']),
    ]);
  }
  return { ok: true };
}

async function openAdm(): Promise<{ ok: true }> {
  await browser.tabs.create({ url: 'http://127.0.0.1:3199/' });
  return { ok: true };
}

async function maybeOpenAdm(): Promise<void> {
  const settings = await settingsStore.get();
  if (settings.openAdmAfterSend) await openAdm();
}
...
function normalizeRouterError(error: unknown): { ok: false; code: string; message: string; retryable?: boolean; repairHint?: string; details?: unknown } {
  const normalized = toAdmExtensionError(error);
  return {
    ok: false,
