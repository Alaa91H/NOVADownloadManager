...
function installVideoDownloadOverlay(): void {
  ...
  if (document.documentElement.dataset.admVideoOverlayInstalled === 'true') return;
  document.documentElement.dataset.admVideoOverlayInstalled = 'true';
  ...
  const storageChangeHandler = (changes: Record<string, unknown>, areaName: string): void => {
  ...
  listenersToRemove.push({
    target: browser.storage.onChanged as unknown as EventTarget,
    type: 'storage',
    handler: storageChangeHandler as unknown as EventListener,
  });
