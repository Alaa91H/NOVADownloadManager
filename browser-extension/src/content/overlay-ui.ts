...
async function createVideoOverlayHost(settings: OverlaySettings): Promise<HTMLElement> {
  ...
  popover.className = 'nova-video-download-popover';
  ...
  trigger.className = 'nova-video-download-trigger';
  ...
  if (showLogo) {
    ...
    logo.className = 'nova-video-download-logo';
  ...
  actions.className = 'nova-video-download-actions';
  ...
  label.className = 'nova-video-download-label';
  label.type = 'button';
  ...
  close.className = 'nova-video-download-close';
  close.type = 'button';
...
function openCandidatePicker(
  ...
  const pickerTitleId = 'nova-picker-title';
  ...
  function renderCandidateList(): void {
    ...
    for (const candidate of pickerCandidates) {
      ...
      item.className = 'nova-picker-item';
      ...
      main.className = 'nova-picker-item-main';
      const nameEl = document.createElement('div');
      nameEl.className = 'nova-picker-item-name';
      const displayName = candidateName(candidate, locale);
      nameEl.textContent = displayName;
      nameEl.title = displayName;
      const rowEl = document.createElement('div');
      rowEl.className = 'nova-picker-item-row';

      const extEl = document.createElement('span');
      extEl.className = 'nova-picker-field nova-picker-item-ext';
      extEl.textContent = candidateExtensionText(candidate);
      extEl.title = 'Extension';

      const sizeEl = document.createElement('span');
      sizeEl.className = 'nova-picker-field nova-picker-item-size';
      sizeEl.textContent = candidateSizeText(candidate, locale);
      sizeEl.title = 'Size';

      const resolutionEl = document.createElement('span');
      resolutionEl.className = 'nova-picker-field nova-picker-item-resolution';
  ...
  footer.className = 'nova-picker-footer';
  const statusEl = document.createElement('span');
  statusEl.className = 'nova-picker-status';
  statusEl.textContent = '';
  const sendBtn = document.createElement('button');
  sendBtn.type = 'button';
  sendBtn.className = 'nova-picker-send';
...
function makeDraggable(
  ...
  handle.addEventListener('pointerdown', (event) => {
    if ((event.target as Element | null)?.closest?.('.nova-picker-close')) return;
    const rect = host.getBoundingClientRect();
    dragging = true;
  ...
  handle.addEventListener('pointerdown', (event) => {
    if (
      (event.target as Element | null)?.closest?.(
        '.nova-video-download-label,.nova-video-download-close',
      )
    )
      return;
    const rect = host.getBoundingClientRect();
