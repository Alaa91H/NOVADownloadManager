...
export function calculateCandidateScore(candidate: Candidate): number {
  ...
  const noisePattern = /analytics|tracking|pixel|beacon|telemetry|metrics|logger|adsystem|ads?\/|banner|popunder/i;
  if (noisePattern.test(url)) score -= 80;

  const uiElementPattern = /favicon|1x1|sprite|placeholder|empty|blank|icon|avatar|profile/i;
  if (uiElementPattern.test(url)) score -= 60;

  const staticWebAsset = /\.(js|mjs|css|woff2?|ttf|otf|eot|map|svg|json|xml|graphql|ajax)(\?|#|$)/i;
  if (staticWebAsset.test(url)) score -= 60;

  if ((candidate.sizeBytes ?? Infinity) < 20 * 1_024) {
    if (!candidate.headers?.contentDisposition) {
      score -= 50;
    } else {
      score -= 20;
    }
  ...
  if (url.startsWith('data:')) score -= 60;
  if (url.startsWith('blob:') && !candidate.filename) score -= 30;

  if (/thumb|thumbnail/.test(url)) score -= 40;
...
export function analyticsUrlPenalty(url: string): EvidenceItem {
  return { source: 'dom', reason: 'URL matches analytics/tracking/pixel pattern', weight: -80, observedAt: evidenceNow(), details: { url } };
}

/** URL matches favicon/1x1 image pattern. */
export function faviconPenalty(url: string): EvidenceItem {
  return { source: 'dom', reason: 'URL matches favicon or UI element pattern', weight: -60, observedAt: evidenceNow(), details: { url } };
}

/** URL is a script, stylesheet, or web font. */
export function staticAssetPenalty(ext: string): EvidenceItem {
  return { source: 'dom', reason: `URL is a static web asset (${ext}) not a downloadable file`, weight: -60, observedAt: evidenceNow() };
}

/** File is too small to be a real download. */
export function tinyFilePenalty(sizeBytes: number): EvidenceItem {
  return { source: 'headers', reason: `File size too small to be a real download (${sizeBytes} bytes)`, weight: -50, observedAt: evidenceNow(), details: { sizeBytes } };
}

/** URL uses a non-downloadable scheme (blob:, data:, javascript:). */
export function dangerousSchemePenalty(scheme: string): EvidenceItem {
  return { source: 'dom', reason: `Non-downloadable URL scheme: ${scheme}`, weight: -60, observedAt: evidenceNow() };
}

/** URL matches thumbnail/preview pattern. */
export function thumbnailPenalty(url: string): EvidenceItem {
  return { source: 'dom', reason: 'URL matches thumbnail/preview pattern', weight: -40, observedAt: evidenceNow(), details: { url } };
}
