...
const ALLOWED_NETWORK_SCHEMES = new Set(['http:', 'https:', 'ftp:', 'ftps:']);
...
export function handoffPolicyDecision(candidate: Candidate): HandoffPolicyDecision {
  ...
  if (BLOCKED_SCHEMES.has(parsed.protocol)) {
    return {
      allowed: false,
      reason: `${parsed.protocol} URLs are browser-local or ephemeral and cannot be handed off directly to NOVA.`,
      repairHint: 'Use page extraction or refresh-address support when NOVA exposes it for this site.',
  ...
  return {
    ...
    repairHint: 'Only http(s)/ftp(s) download URLs and magnet links can be handed off directly; the bridge still checks runtime libcurl protocols before sending.',
    scheme: parsed.protocol,
  };
}
...
export function assertCandidateHandoffAllowed(candidate: Candidate): void {
  ...
  throw new NovaExtensionError({
    ...
    message: decision.reason ?? 'Candidate is not valid for NOVA handoff.',
    retryable: false,
