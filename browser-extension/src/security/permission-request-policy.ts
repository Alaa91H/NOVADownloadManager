...
// Loopback origin pattern for the local NOVA daemon. Defined here (a dependency-free
...
export const NOVA_LOOPBACK_ORIGIN_PATTERN = 'http://127.0.0.1/*';

const ALLOWED_OPTIONAL_PERMISSIONS = new Set(['downloads', 'webRequest', 'scripting', 'tabs', 'nativeMessaging', 'activeTab']);
const ALLOWED_EXACT_ORIGINS = new Set(['<all_urls>', NOVA_LOOPBACK_ORIGIN_PATTERN, 'http://localhost/*']);
...
export function validatePermissionRequest(permissions: string[] = [], origins: string[] = []): ValidatedPermissionRequest {
  ...
  if (forbiddenPermissions.length) {
    throw new NovaExtensionError({
      ...
      message: 'The requested browser permission is not part of the NOVA Extension allowlist.',
      retryable: false,
  ...
  if (forbiddenOrigins.length) {
    throw new NovaExtensionError({
      code: 'PERMISSION_MISSING',
      message: 'The requested host permission is outside the NOVA Extension allowlist.',
      retryable: false,
