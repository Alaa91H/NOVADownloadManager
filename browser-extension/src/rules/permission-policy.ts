...
import { validatePermissionRequest, NOVA_LOOPBACK_ORIGIN_PATTERN } from '../security/permission-request-policy';

// Re-exported for backward compatibility with existing importers.
export { NOVA_LOOPBACK_ORIGIN_PATTERN };
...
const permissionCatalog: Record<string, PermissionNeed & { reason: string; degradedFeature?: DegradedFeature }> = {
  ...
  downloads: { permissions: ['downloads'], reason: 'Observe browser-created downloads and hand off only after NOVA accepts the task.', degradedFeature: 'downloads-interception' },
  ...
  loopback: { origins: [NOVA_LOOPBACK_ORIGIN_PATTERN], reason: 'Talk only to the local NOVA daemon on loopback.' },
  nativeMessaging: { permissions: ['nativeMessaging'], reason: 'Wake and verify the NOVA Native Messaging host.', degradedFeature: 'native-messaging' },
};
