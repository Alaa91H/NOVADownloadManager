...
const EXTENSION_REGISTRATION_TIMEOUT_MS = Number(process.env.NOVA_E2E_EXTENSION_REGISTRATION_TIMEOUT_MS ?? 30_000);
...
async function resolveExtensionId(context: Awaited<ReturnType<typeof chromium.launchPersistentContext>>, userDataDir: string): Promise<string> {
  ...
  throw new Error('Chromium launched, but NOVA extension id could not be resolved.');
}
...
test.describe('NOVA Extension popup artifact smoke', () => {
...
test.describe('NOVA Extension real browser profile smoke', () => {
  test.skip(!RUN_REAL_EXTENSION_E2E, 'Real extension profile smoke is opt-in. Set NOVA_RUN_REAL_EXTENSION_E2E=1 under a known-good headed browser runner.');
  test.skip(!fs.existsSync(path.join(EXTENSION_DIR, 'manifest.json')), `Unpacked extension not found at ${EXTENSION_DIR}`);
  ...
  test('loads the MV3 extension popup inside a persistent Chromium profile', async ({ browserName: _browserName }, testInfo) => {
    ...
    const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nova-extension-e2e-'));
