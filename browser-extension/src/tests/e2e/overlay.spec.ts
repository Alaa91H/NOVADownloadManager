...
function startOverlayFixtureServer(): Promise<{ server: Server; url: string }> {
  const server = http.createServer((request, response) => {
    ...
    response.end(`<!doctype html>
      <html lang="en">
        <head><title>ADM overlay fixture</title></head>
        <body>
          <main>
            <h1>ADM overlay fixture</h1>
...
async function waitForExtensionId(userDataDir: string, timeoutMs: number): Promise<string> {
  ...
  throw new Error('ADM extension id could not be resolved from Chromium Preferences.');
}
...
test.describe('ADM floating overlay real content smoke', () => {
  test.skip(
    !RUN_REAL_EXTENSION_E2E,
    'Real content-script overlay smoke is opt-in. Set ADM_RUN_REAL_EXTENSION_E2E=1 after building dist/chromium.',
  );
  test.skip(
  ...
  test('injects overlay, opens edge-aware menu, and exposes picker controls on a media page', async ({ browserName }, testInfo) => {
    ...
    const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adm-overlay-e2e-'));
    ...
    try {
      ...
      const overlay = page.locator('#adm-video-download-overlay-host');
      ...
      await overlay.locator('.adm-video-download-trigger').click();
      await expect(overlay.locator('.adm-video-download-actions')).toBeVisible();
      await expect(overlay.locator('.adm-video-download-label')).toBeVisible();
      await expect(overlay.locator('.adm-video-download-close')).toBeVisible();

      await overlay.locator('.adm-video-download-label').click();
      const picker = page.locator('#adm-candidate-picker-host');
      await expect(picker).toBeVisible();
      await expect(picker.locator('.adm-picker-tool')).toHaveCount(2);
      await expect(picker.locator('.adm-picker-send')).toBeVisible();
    } finally {
