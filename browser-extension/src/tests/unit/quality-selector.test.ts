...
describe('quality selector — resolve contract', () => {
  ...
  it('parses an ADM resolve response with multiple qualities', () => {
    const resp = {
      ok: true,
      manifestType: 'hls',
      qualities: [
...
describe('quality selector — send contract', () => {
  ...
  it('builds a valid SEND_STREAM (add) request with a chosen quality', () => {
    ...
    const req = {
      ...
      source: 'adm-extension' as const,
    };
    expect(StreamAddRequestSchema.safeParse(req).success).toBe(true);
  });

  it('allows sending without a selected quality (let ADM pick best)', () => {
    const req = { idempotencyKey: 'b'.repeat(16), manifest, source: 'adm-extension' as const };
    expect(StreamAddRequestSchema.safeParse(req).success).toBe(true);
  });
});
...
describe('quality selector — send response interpretation', () => {
  ...
  function interpret(result: { ok?: boolean; accepted?: boolean; duplicate?: boolean; message?: string }, qualityUrl?: string): { kind: 'success' | 'duplicate' | 'error'; text: string } {
    const label = qualityUrl ? 'selected quality' : 'best quality (ADM choice)';
    if (result?.duplicate) return { kind: 'duplicate', text: `Already queued in ADM (${label}).` };
    if (result?.ok === false || result?.accepted === false) return { kind: 'error', text: result?.message ?? 'ADM did not accept the stream.' };
    return { kind: 'success', text: `Sent to ADM: ${label}.` };
  }

  it('reports success when ADM accepts', () => {
...
describe('quality selector — IDM-style column formatting', () => {
  ...
  it('uses exact size when ADM reports per-quality bytes', () => {
