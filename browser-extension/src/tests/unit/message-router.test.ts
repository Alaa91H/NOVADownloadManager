...
const harness = vi.hoisted(() => {
  ...
  const browser = {
    runtime: {
      ...
      getManifest: () => ({ name: 'ADM Extension', version: '0.0.0', manifest_version: 3 }),
    ...
    tabs: { create: () => Promise.resolve({ id: 1 }), query: () => Promise.resolve([{ id: 1, active: true }]), sendMessage: () => Promise.resolve(undefined) },
    scripting: {
