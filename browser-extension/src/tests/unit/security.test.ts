...
import { assertAdmLoopbackOrigin, assertSafeLoopbackRoute, buildAdmLoopbackHttpUrl } from '../../transport/loopback-url-policy';
...
describe('loopback URL policy', () => {
  it('accepts valid ADM loopback URL', () => {
    expect(() => assertAdmLoopbackOrigin('http://127.0.0.1:3199', 'http:')).not.toThrow();
  });

  it('rejects non-loopback IP', () => {
    expect(() => assertAdmLoopbackOrigin('http://192.168.1.1:3199', 'http:')).toThrow();
  });

  it('rejects external domain', () => {
    expect(() => assertAdmLoopbackOrigin('http://evil.com:3199', 'http:')).toThrow();
  });

  it('rejects wrong port', () => {
    expect(() => assertAdmLoopbackOrigin('http://127.0.0.1:8080', 'http:')).toThrow();
  });

  it('rejects URLs with credentials', () => {
    expect(() => assertAdmLoopbackOrigin('http://user:pass@127.0.0.1:3199', 'http:')).toThrow();
  });
  ...
  it('builds a valid loopback URL', () => {
    const url = buildAdmLoopbackHttpUrl('http://127.0.0.1:3199', '/v1/add');
    expect(url).toBe('http://127.0.0.1:3199/v1/add');
  });

  it('prevents building URLs to non-loopback base', () => {
    expect(() => buildAdmLoopbackHttpUrl('http://evil.com:3199', '/v1/add')).toThrow();
  });
});
