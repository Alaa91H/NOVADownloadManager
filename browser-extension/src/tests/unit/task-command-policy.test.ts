...
describe('assertTaskIdSafe', () => {
  ...
  it('rejects an empty or whitespace-only id', () => {
    expect(() => assertTaskIdSafe('')).toThrow(AdmExtensionError);
    expect(() => assertTaskIdSafe('   ')).toThrow(AdmExtensionError);
  });

  it('rejects an id longer than the limit', () => {
    expect(() => assertTaskIdSafe('a'.repeat(MAX_TASK_ID_CHARS + 1))).toThrow(AdmExtensionError);
  });
  ...
  it('rejects ids containing control characters', () => {
    for (const bad of ['task\n1', 'task\t1', 'task\x00end', 'task\x7f']) {
      expect(() => assertTaskIdSafe(bad)).toThrow(AdmExtensionError);
    }
  });
  ...
  it('tags rejections as VALIDATION_FAILED', () => {
    try {
      ...
      expect((error as AdmExtensionError).code).toBe('VALIDATION_FAILED');
    }
  });
});
