...
export function ConnectionSettings() {
  ...
  return <section className="nova-section">
    ...
    <p className="nova-help">{t('options.connection.help')}</p>
    ...
    <div className="nova-grid nova-grid-3">
      ...
      <button aria-label={t('options.connection.linkNova')} onClick={() => void run(t('options.connection.noticeLinking'), t('options.connection.actionCompleted'), () => runtimeRequest({ type: 'RESET_PAIRING' }))}>{t('options.connection.linkNova')}</button>{/* Link with NOVA */}
