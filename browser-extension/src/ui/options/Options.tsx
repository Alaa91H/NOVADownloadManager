    ...
    <div className="adm-page-shell">
      <header className="adm-topbar">
        <div className="adm-brand">
          <AppLogo />
          <div>
            <h1 className="adm-title">{t('options.title')}</h1>
            <p className="adm-subtitle">{t('options.subtitle')}</p>
          </div>
        </div>
        <div className="adm-actions-row">
          <span className="adm-pill" data-tone={settings.enabled ? 'success' : 'warning'}>{settings.enabled ? t('options.enabled') : t('options.disabled')}</span>
          <span className="adm-pill" data-tone={aggressive ? 'warning' : 'info'}>{aggressive ? t('options.aggressiveMode') : t('options.standardMode')}</span>
        </div>
      </header>
      {notice ? <div className="adm-notice" data-kind="success" role="status">{notice}</div> : null}
      <div className="adm-settings-shell">
        <aside className="adm-sidebar" aria-label={t('options.navigation')}>
          <div className="adm-brand">
            <AppLogo />
            <div>
              <strong>{t('options.controlCenter')}</strong>
              <p className="adm-subtitle">{t('options.advancedSetup')}</p>
            </div>
          </div>
          <nav className="adm-sidebar-nav" role="tablist">
