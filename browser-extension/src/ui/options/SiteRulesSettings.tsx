  ...
  return <section className="adm-section">
    <div className="adm-section-title-row">
      <div>
        ...
        <p className="adm-help">{t('options.siteRules.help')}</p>
      </div>
      <span className="adm-pill" data-tone={sorted.length ? 'info' : 'warning'}>{sorted.length} rules</span>
    </div>
    ...
    {sorted.length === 0 ? <div className="adm-empty">
      <strong>{t('options.siteRules.emptyTitle')}</strong>
      <p>{t('options.siteRules.emptyHelp')}</p>
    </div> : <div className="adm-grid">
      {sorted.map((rule) => <article key={rule.id} className="adm-rule-card" data-disabled={!rule.enabled}>
        <div className="adm-rule-header">
          <div>
            <h3 className="adm-card-title">{rule.host}</h3>
            <p className="adm-card-description">{t('options.siteRules.ruleDetail', { size: rule.minSizeMB, count: rule.mediaTypes.length, sendBehavior: rule.askBeforeSend ? t('options.siteRules.sendAsk') : t('options.siteRules.sendAuto') })}</p>
          </div>
          <div className="adm-actions-row">
            <span className="adm-pill" data-tone={rule.enabled ? 'success' : 'warning'}>{rule.enabled ? t('options.siteRules.enabled') : t('options.siteRules.disabled')}</span>
            <button data-variant="danger" onClick={() => setDeleteTarget(rule)}>{t('options.siteRules.delete')}</button>
          </div>
        </div>
        <div className="adm-field-grid">
          <label className="adm-toggle"><input type="checkbox" checked={rule.enabled} onChange={(event) => void update(rule, { enabled: event.currentTarget.checked })} /><span><strong>{t('options.siteRules.enabledLabel')}</strong><span>{t('options.siteRules.enabledHelp')}</span></span></label>
          <label className="adm-toggle"><input type="checkbox" checked={rule.autoCapture} onChange={(event) => void update(rule, { autoCapture: event.currentTarget.checked })} /><span><strong>{t('options.siteRules.autoCapture')}</strong><span>{t('options.siteRules.autoCaptureHelp')}</span></span></label>
          <label className="adm-toggle"><input type="checkbox" checked={rule.askBeforeSend} onChange={(event) => void update(rule, { askBeforeSend: event.currentTarget.checked })} /><span><strong>{t('options.siteRules.askBeforeSend')}</strong><span>{t('options.siteRules.askBeforeSendHelp')}</span></span></label>
          <label className="adm-toggle"><span><strong>{t('options.siteRules.minSize')}</strong><span>{t('options.siteRules.minSizeHelp')}</span></span><input type="number" min={0} value={rule.minSizeMB} onChange={(event) => void update(rule, { minSizeMB: Number(event.currentTarget.value) })} /></label>
        </div>
        <div className="adm-check-grid">
          {mediaTypeOptions.map((mediaType) => <label key={mediaType} className="adm-check-chip">
        ...
        <details className="adm-rule-details">
