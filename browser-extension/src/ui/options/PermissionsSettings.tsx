...
export function PermissionsSettings() {
  ...
  return <section className="adm-section">
    ...
    <p className="adm-help">{t('options.permissions.help')}</p>
    ...
    <div className="adm-grid adm-grid-2">
      {Object.entries(status).map(([key, entry]) => <article key={key} className="adm-card">
        <div className="adm-card-header">
          <div>
            <h3 className="adm-card-title">{key}</h3>
            <p className="adm-card-description">{entry.reason}</p>
          </div>
          <span className="adm-pill" data-tone={entry.granted ? 'success' : 'warning'}>{entry.granted ? t('options.permissions.granted') : t('options.permissions.missing')}</span>
        </div>
        {!entry.granted && entry.degradedFeature ? <p className="adm-card-description">{t('options.permissions.degradedFeature', { feature: entry.degradedFeature })}</p> : null}
