...
const THEME_STORAGE_KEY = 'nova-ui-theme';
...
export function PopupApp() {
  ...
  async function repair(): Promise<void> {
    await run(t('popup.notice.linkingNova'), async () => {
  ...
  async function sendCandidates(toSend: Candidate[], emptyMessage: string): Promise<void> {
    const handoffable = toSend.filter((candidate) => isHandoffable(candidate) && isSupportedByRuntime(candidate, bridge)).slice(0, MAX_HANDOFF_CANDIDATES);
  ...
  const handoffableCount = candidates.filter((candidate) => isHandoffable(candidate) && isSupportedByRuntime(candidate, bridge)).length;
    ...
    <section className="nova-connection-panel" aria-label={t('popup.connection.aria')}>
      <header className="nova-connection-header">
        ...
        <div className="nova-header-tools">
          ...
          <span className="nova-pill" data-tone={tone}>{bridge?.canSend ? t('popup.ready') : t('popup.needsCheck')}</span>
        </div>
      </header>

      <nav className="nova-tabs nova-popup-tabs" role="tablist" aria-label={t('popup.tab.aria')}>
        <button type="button" role="tab" className="nova-tab" aria-selected={tab === 'connection'} onClick={() => setTab('connection')}>{t('popup.tab.connection')}</button>
        <button type="button" role="tab" className="nova-tab" aria-selected={tab === 'candidates'} onClick={() => setTab('candidates')}>
          {t('popup.tab.candidates')}{settings.popup.showCandidateCounts && candidates.length ? ` (${candidates.length})` : ''}
        </button>
        {availableTabs.includes('tasks') ? <button type="button" role="tab" className="nova-tab" aria-selected={tab === 'tasks'} onClick={() => setTab('tasks')}>{t('popup.tab.tasks')}</button> : null}
        <button type="button" role="tab" className="nova-tab" aria-selected={tab === 'popup-options'} onClick={() => setTab('popup-options')}>{t('popup.tab.popupOptions')}</button>
        <button type="button" role="tab" className="nova-tab" aria-selected={tab === 'capture-options'} onClick={() => setTab('capture-options')}>{t('popup.tab.captureOptions')}</button>
      </nav>

      {notice ? <div role="status" className="nova-notice" data-kind={notice.kind}>{notice.message}</div> : null}

      {tab === 'connection' ? <>
        <div className="nova-connection-state" data-tone={tone}>
          <span className="nova-connection-dot" aria-hidden="true" />
          <div>
        ...
        {settings.popup.showTechnicalConnectionDetails ? <dl className="nova-connection-details">
          ...
          <dt>Capabilities</dt><dd>{bridge?.capabilities?.items?.length ? `${bridge.capabilities.items.length} runtime-verified` : t('popup.detail.unavailable')}</dd>
        </dl> : null}

        {bridge?.lastError ? <div className="nova-notice" data-kind={bridge.lastError.retryable ? 'info' : 'error'} role="status">
        ...
        <div className="nova-connection-actions">
          ...
          <button type="button" disabled={busy} onClick={() => void repair()}>{t('popup.action.linkNova')}</button>
          <button type="button" onClick={() => void runtimeRequest({ type: 'OPEN_NOVA' })}>{t('popup.action.openNova')}</button>
      ...
      {tab === 'candidates' ? <>
        ...
        />
        <div className="nova-popup-summary-row" aria-label={t('candidate.summary')}>
          <span className="nova-pill" data-tone="success">{handoffableCount} {t('popup.handoffable')}</span>
          <span className="nova-pill" data-tone={protectedCount ? 'warning' : 'info'}>{protectedCount} {t('popup.protected')}</span>
          <span className="nova-pill" data-tone="info">{t('popup.refresh')} {settings.popup.autoRefreshCandidates ? `${settings.popup.candidateRefreshMs}ms` : t('popup.refreshManual')}</span>
        </div>
        <CandidateFilters value={filter} onChange={setFilter} />
        {hiddenByPopupLimit ? <div className="nova-detail-note">{t('popup.candidatesShowing', { shown: limitedCandidates.length, total: visibleCandidates.length })}</div> : null}
        <CandidateList candidates={limitedCandidates} selected={selected} showHandoffWarnings={settings.popup.showHandoffWarnings} isCandidateSupported={(candidate) => isSupportedByRuntime(candidate, bridge)} unsupportedReason={(candidate) => unsupportedRuntimeReason(candidate, bridge)} onToggle={toggleSelected} onStreamSent={() => {
      ...
      {tab === 'popup-options' ? <section className="nova-popup-options-panel" aria-label={t('popup.customization.title')}>
      ...
      {tab === 'capture-options' ? <section className="nova-popup-options-panel" aria-label={t('popup.capture.title')}>
        <div className="nova-section-title-row">
          <div>
            <h2>{t('popup.capture.title')}</h2>{/* Capture customization */}
            <p className="nova-help">{t('popup.capture.help')}</p>
          </div>
          <span className="nova-pill" data-tone={settings.capture.aggressiveMode ? 'warning' : 'info'}>{settings.capture.aggressiveMode ? t('options.aggressiveMode') : t('options.standardMode')}</span>
        </div>

          <div className="nova-card nova-drm-policy-card">
          <div className="nova-card-header">
            <div>
              <h3 className="nova-card-title">{t('popup.capture.drm.title')}</h3>
              <p className="nova-card-description">{t('popup.capture.drm.help')}</p>
            </div>
          </div>
          <div className="nova-field-grid nova-popup-field-grid">
      ...
      <ConfirmDialog
        ...
        tone="warning"
        details={<ul className="nova-dialog-list"><li>{t('confirm.sendAll.items', { count: handoffableCount })}</li><li>{t('confirm.sendAll.protectedItems', { count: protectedCount })}</li></ul>}
