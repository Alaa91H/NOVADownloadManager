...
// Estimate size from bitrate × duration when ADM doesn't report a per-quality size.
 ...
 *
 * Resolves the manifest via ADM (RESOLVE_STREAM) and renders one row per
 * quality with resolution, dimensions, size, container, codec, and a Download
 * button (SEND_STREAM). Qualities are sorted highest-first. ADM owns the actual
...
export function QualitySelector({ candidate, onSent }: { candidate: Candidate; onSent?: () => void }) {
  ...
  const send = useCallback(async (qualityUrl?: string) => {
    ...
    try {
      const result = await runtimeRequest<{ ok?: boolean; accepted?: boolean; duplicate?: boolean; taskId?: string; message?: string }>(
        { type: 'SEND_STREAM', candidateId: candidate.id, selectedQualityUrl: qualityUrl },
      );
      ...
      if (result?.duplicate) {
        ...
        setError(result?.message ?? t('quality.admNotAccepted'));
        return;
      } else {
  ...
  return (
    <div className="adm-quality-selector" aria-label={t('quality.aria')}>
      <div className="adm-quality-header">
        <strong>{t('quality.header', { n: qualities.length })}</strong>
        <button type="button" className="adm-quality-resolve" disabled={status === 'resolving'} onClick={() => void resolve()}>
          {status === 'resolving' ? t('quality.resolving') : resolved ? t('quality.reresolveViaNOVA') : t('quality.resolveViaNOVA')}
        </button>
      </div>

      {meta?.isLive ? <div className="adm-detail-note">{t('quality.liveStream')}</div> : null}

      {qualities.length === 0 ? (
        <div className="adm-detail-note">
          {status === 'done'
            ? t('quality.noneFromNOVA')
            : t('quality.noneLocal')}
        </div>
      ) : (
        <table className="adm-quality-table">
          <thead>
            <tr>
          ...
          <tbody>
            {qualities.map((q) => (
              <tr key={q.url}>
                <td className="adm-q-name">
                  {resolutionLabel(q)}
                  {q.fps ? <span className="adm-q-sub">{Math.round(q.fps)}fps</span> : null}
                </td>
                <td className="adm-q-dim">{dimensionsText(q) ?? '\u2014'}</td>
                <td className="adm-q-size">{sizeText(q, meta?.durationSec)}</td>
                <td className="adm-q-fmt">
                  <span className="adm-pill">{containerText(q)}</span>
                  {codecShort(q.codecs) ? <span className="adm-q-codec">{codecShort(q.codecs)}</span> : null}
                </td>
                <td className="adm-q-action">
                  <button
                    type="button"
                    className="adm-quality-download"
                    data-sent={sentUrl === q.url ? 'true' : undefined}
                    disabled={Boolean(sendingUrl)}
                    onClick={() => void send(q.url)}
      ...
      <div className="adm-quality-footer">
        <button type="button" className="adm-quality-auto" data-sent={sentUrl === 'auto' ? 'true' : undefined} disabled={Boolean(sendingUrl)} onClick={() => void send(undefined)}>
          {sendingUrl === 'auto' ? t('quality.sending') : sentUrl === 'auto' ? t('quality.sentBestQuality') : t('quality.bestQuality')}
        </button>
      </div>

      {notice ? <div className="adm-notice" data-kind="success" role="status">{notice}</div> : null}
      {error ? <div className="adm-notice" data-kind="error" role="status">{error}</div> : null}
      {!resolved && status === 'idle' && localQualities.length > 0 ? (
        <div className="adm-quality-hint">{t('quality.localHint')}</div>
      ) : null}
    </div>
  );
}
