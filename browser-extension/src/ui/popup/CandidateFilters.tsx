...
export function CandidateFilters({ value, onChange }: { value: Filter; onChange(value: Filter): void }) {
  ...
  return <nav className="nova-filter-row" role="tablist" aria-label={t('candidate.filter.aria')}>
    {filters.map((filter) => <button className="nova-filter-chip" role="tab" key={filter.value} onClick={() => onChange(filter.value)} aria-pressed={value === filter.value} aria-selected={value === filter.value}>{filter.label}</button>)}
  </nav>;
}
