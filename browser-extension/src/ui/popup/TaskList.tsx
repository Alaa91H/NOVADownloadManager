...
export function TaskList({ tasks, onRefresh, onPause, onResume, onCancel }: { tasks: TaskSummary[]; onRefresh(): void; onPause(taskId: string): void; onResume(taskId: string): void; onCancel(taskId: string): void }) {
  ...
  const { t } = useI18n();
  return <section className="adm-card" aria-label={t('taskList.aria')}>
    <div className="adm-card-header">
      <div>
        <h2 className="adm-card-title">{t('taskList.title')}</h2>
        <p className="adm-card-description">{t('taskList.help')}</p>
      </div>
      <button onClick={onRefresh}>{t('taskList.refresh')}</button>
    </div>
    {tasks.length === 0 ? <div className="adm-empty">
      <strong>{t('taskList.empty.title')}</strong>
      <p>{t('taskList.empty.help')}</p>
    </div> : <div className="adm-task-list">
      {tasks.slice(0, 8).map((task, index) => {
        const id = taskId(task);
        const title = taskTitle(task, t);
        const state = taskState(task, t);
        const progress = taskProgress(task);
        return <article key={id || index} className="adm-task">
          <div className="adm-task-headline">
            <strong className="adm-task-title" title={title}>{title}</strong>
            <span className="adm-pill" data-tone={taskTone(state)}>{state}</span>
          </div>
          {progress !== undefined ? <div className="adm-progress" aria-label={t('taskList.task.progress', { percent: Math.round(progress) })}><span style={{ width: `${progress}%` }} /></div> : null}
          <span className="adm-task-meta">{id ? `id: ${id}` : t('taskList.task.noId')}{progress !== undefined ? ` · ${Math.round(progress)}%` : ''}</span>
