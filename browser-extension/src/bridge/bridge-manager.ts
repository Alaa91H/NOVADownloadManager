...
import { isAuthError, toAdmExtensionError } from '../core/error-classification';
...
} from '../contracts/adm.protocol.v4';
import { Candidate } from '../contracts/candidate.schema';
import { AdmEvent } from '../contracts/events.schema';
...
export class BridgeManager implements BridgeGateway {
  ...
  private async autoConnectInternal(): Promise<BridgeState> {
    ...
    try {
      ...
      this.retry.reset();
      await this.setState({
        status: discovered.native && discovered.http ? 'connected' : 'degraded',
        canSend: true,
      ...
      const normalized = toAdmExtensionError(error, 'NETWORK_ERROR');
  ...
  subscribeEvents(): void {
    ...
    void tokenPromise.then((token) => {
      ...
      void this.tm.sse.connectFirst([this.tm.http.url('/api/v1/events/stream'), this.tm.http.url('/v1/events')], token, {
        ...
        onEvent: (event: AdmEvent) => this.handleEvent(event),
  ...
  private handleEvent(event: AdmEvent): void {
  ...
  async addStream(manifest: StreamManifestCandidate, selectedQuality: StreamResolveResponse['qualities'][number] | undefined, idempotencyKey: string): Promise<AddTaskResponse> {
    ...
    this.caps.registry.require(cap);
    const request = StreamAddRequestSchema.parse({ idempotencyKey, manifest, selectedQuality, source: 'adm-extension' });
  ...
  async sendCandidateNow(candidate: Candidate, idempotencyKey: string) {
    ...
    const request = AddTaskRequestSchema.parse({ idempotencyKey, candidate, source: 'adm-extension' });
  ...
  async sendBatchNow(candidates: Candidate[], idempotencyKey: string) {
    ...
    const request = AddBatchRequestSchema.parse({ idempotencyKey, candidates, source: 'adm-extension' });
  ...
  private requireCandidateCapabilities(candidate: Candidate): void {
    ...
    if (candidate.mediaType === 'torrent') this.caps.registry.require('candidate.torrent');
    if (candidate.mediaType === 'magnet') this.caps.registry.require('candidate.magnet');
    if (candidate.source === 'hls-manifest') this.caps.registry.require('candidate.hls');
    if (candidate.source === 'dash-manifest') this.caps.registry.require('candidate.dash');
    if (candidate.mediaType !== 'torrent' && candidate.mediaType !== 'magnet' && candidate.mediaType !== 'manifest') this.caps.registry.require('candidate.directUrl');
  }
