...
const keyFor = (tabId: number) => `nova.candidateCache.${tabId}`;
const INDEX_KEY = 'nova.candidateCache.index';
const CANDIDATE_CACHE_UPDATED_MESSAGE_TYPE = 'NOVA_CANDIDATE_CACHE_UPDATED';

type MergeOptions = { notify?: boolean; reason?: string };
// Guardrail retained for regression tests: MAX_CANDIDATES_PER_TAB = 250

/** Sources that only arrive via passive observers (not recreated by a DOM scan). */
const PASSIVE_CANDIDATE_SOURCES = new Set([
  'network',
  'downloads-api',
  'websocket-webrtc',
]);

function isPassiveCandidate(candidate: Candidate): boolean {
  if (PASSIVE_CANDIDATE_SOURCES.has(candidate.source)) return true;
  const assistive = candidate.metadata?.assistiveSource;
  return assistive === 'page-tap-live-quality' || assistive === 'page-tap';
}
...
export class CandidateCache {
  ...
  /**
   * Store DOM/platform scan results without wiping passive network / download /
   * page-tap captures that a static HTML snapshot cannot rediscover.
   */
  async replaceWithScan(tabId: number, scanned: Candidate[]): Promise<Candidate[]> {
    const existing = await this.get(tabId);
    const passive = existing.filter(isPassiveCandidate);
    const merged = capCandidates([...scanned, ...passive]);
  ...
  async clearAll(): Promise<void> {
    ...
    const discoveredKeys = Object.keys(snapshot).filter((key) => key.startsWith('nova.candidateCache.'));
