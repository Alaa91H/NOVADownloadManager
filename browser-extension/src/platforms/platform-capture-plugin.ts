...
export class PlatformCapturePlugin implements CapturePlugin {
  ...
  private toCandidate(
    ...
    const partial = {
      ...
      source: 'platform' as Candidate['source'],
      mediaType: mr.type === 'audio' ? 'audio' as Candidate['mediaType'] : mr.type === 'image' ? 'image' as Candidate['mediaType'] : 'video' as Candidate['mediaType'],
    ...
    partial.confidence = adapter.adjustConfidence(partial as Candidate);
    const enriched = adapter.enrichCandidate(partial as Candidate, context.content);
