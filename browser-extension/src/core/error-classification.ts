...
export class NovaExtensionError extends Error {
...
export function defaultRetryable(code: ErrorCode, status?: number): boolean {
  if (code === 'TIMEOUT' || code === 'NETWORK_ERROR' || code === 'DAEMON_UNAVAILABLE' || code === 'NOVA_NOT_RUNNING') return true;
...
export function toNovaExtensionError(error: unknown, fallbackCode: ErrorCode = 'UNKNOWN_ERROR'): NovaExtensionError {
  if (error instanceof NovaExtensionError) return error;
  if (error instanceof DOMException && error.name === 'AbortError') {
    return new NovaExtensionError({ code: 'TIMEOUT', message: 'Request timed out.', retryable: true });
  }
  if (error instanceof Error) {
    ...
    return new NovaExtensionError({ code, message: error.message, retryable: defaultRetryable(code) });
  }
  if (typeof error === 'string') {
    return new NovaExtensionError({ code: fallbackCode, message: error, retryable: defaultRetryable(fallbackCode) });
  }
  return new NovaExtensionError({ code: fallbackCode, message: 'Extension request failed.', retryable: defaultRetryable(fallbackCode), details: error });
}

export function isAuthError(error: unknown): boolean {
  const novaError = toNovaExtensionError(error);
  return novaError.code === 'TOKEN_EXPIRED' || novaError.code === 'TOKEN_INVALID' || novaError.code === 'TOKEN_MISSING';
}

export function isRetryableHandoffError(error: unknown): boolean {
  return toNovaExtensionError(error).retryable;
}

export function errorMessage(error: unknown): string {
  return toNovaExtensionError(error).message;
}
