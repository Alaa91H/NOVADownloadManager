...
function run(command, args, label, cwd = ROOT, extraEnv = {}) {
  const result = spawnSync(resolveExecutable(command), args, {
    ...
    stdio: 'pipe',
    shell: false,
    // Verbose native builds can exceed spawnSync's 1 MB default and get killed
    // mid-build with no error; allow ample headroom.
...
function writeFallbackManifest() {
  const manifest = {
    ...
    version: 'system',
    tag: 'system',
    featureProfile: 'system-fallback',
    protocols: [],
    features: [],
    curlVersionOutput: '',
    prefix: '',
  ...
  console.log('[native-curl] Fallback manifest written (using system libcurl).');
}

async function main() {
  mkdirSync(BIN_DIR, { recursive: true });

  const optional = process.env.NOVA_NATIVE_CURL_OPTIONAL === '1';

  if (!commandExists('cmake')) {
    if (optional) {
      console.log('[native-curl] CMake not found; skipping native curl build (NOVA_NATIVE_CURL_OPTIONAL=1).');
      console.log('[native-curl] Falling back to system libcurl.');
      writeFallbackManifest();
      return;
    }
    throw new Error('CMake is required to build production libcurl. Install CMake or set NOVA_NATIVE_CURL_OPTIONAL=1 for development only.');
  }
  if (process.platform !== 'win32' && !commandExists('unzip', ['-v'])) {
    if (optional) {
      console.log('[native-curl] unzip not found; skipping native curl build (NOVA_NATIVE_CURL_OPTIONAL=1).');
      writeFallbackManifest();
      return;
    }
  ...
  const cmakeOptions = [
    ...
    // which verify-native-curl.mjs enforces.
    '-DBUILD_CURL_EXE=ON',
  ...
  // Record the real protocols/features from the built curl tool so the manifest
  // is honest (verify-native-curl.mjs enforces these on Windows).
  const curlExeName = process.platform === 'win32' ? 'curl.exe' : 'curl';
  const curlExe =
    findFileRecursive(PREFIX_DIR, (_full, name) => name.toLowerCase() === curlExeName) ||
    findFileRecursive(buildDir, (_full, name) => name.toLowerCase() === curlExeName);
  if (curlExe) {
    const versionResult = spawnSync(curlExe, ['--version'], { encoding: 'utf8', windowsHide: true });
    builtCurlVersionOutput = (versionResult.stdout || '').trim();
    for (const line of builtCurlVersionOutput.split(/\r?\n/)) {
      const protoMatch = line.match(/^Protocols:\s*(.+)$/i);
      if (protoMatch) protocols = protoMatch[1].trim().split(/\s+/).filter(Boolean);
      const featMatch = line.match(/^Features:\s*(.+)$/i);
      if (featMatch) features = featMatch[1].trim().split(/\s+/).filter(Boolean);
    }
  }
  if (protocols.length === 0) {
    // Fallback: derive from the configured build so a runner that cannot exec
    // the freshly built tool still yields a non-empty manifest.
    protocols = ['dict', 'file', 'ftp', 'ftps', 'http', 'https'].filter(
      (proto) => !cmakeOptions.includes(`-DCURL_DISABLE_${proto.toUpperCase()}=ON`),
    );
    if (enableSsh) protocols.push('scp', 'sftp');
  }
  if (features.length === 0) {
    features = ['IPv6', 'Largefile', 'SSL'];
    if (enableHttp2) features.push('HTTP2');
    if (enableCompression) features.push('libz', 'brotli', 'zstd');
  }
