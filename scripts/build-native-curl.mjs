...
const TMP_DIR = join(BIN_DIR, '.tmp-native-curl');
...
function commandExists(command, args = ['--version']) {
  const result = spawnSync(command, args, { encoding: 'utf8', stdio: 'pipe', windowsHide: true, shell: process.platform === 'win32' });
...
async function main() {
  ...
  rmSync(PREFIX_DIR, { recursive: true, force: true });
  ...
  if (process.platform === 'win32') {
    cmakeOptions.push('-DCURL_USE_SCHANNEL=ON', '-DCURL_USE_OPENSSL=OFF', '-DCURL_CA_FALLBACK=ON');
