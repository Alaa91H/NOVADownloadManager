...
requireContains('.github/workflows/ci.yml', 'pnpm install --frozen-lockfile', 'pnpm CI install');
requireContains('.github/workflows/ci.yml', 'cargo check --manifest-path src-tauri/Cargo.toml', 'Rust compile gate');
requireContains('.github/workflows/ci.yml', 'pnpm run extension:package', 'extension package gate in CI');
requireContains('.github/workflows/ci.yml', 'pnpm --filter nova-browser-extension verify:offline', 'extension release gate in CI');
requireContains('.github/workflows/ci.yml', 'pnpm --filter nova-browser-extension typecheck', 'extension TypeScript gate in CI');
...
requireContains('src-tauri/windows/hooks.nsi', 'BuildId=', 'build traceability in install receipt');
requireContains('src-tauri/windows/hooks.nsi', 'BuildCommit=', 'commit traceability in install receipt');
requireContains('src-tauri/windows/hooks.nsi', 'MUI_FONT "Segoe UI"', 'professional NSIS font');
requireContains('src-tauri/windows/hooks.nsi', 'NOVA_NSIS_ACCENT', 'accent color in theme');
requireContains('src-tauri/windows/installer-template.nsi', 'MUI_FINISHPAGE_TITLE', 'branded finish page');
requireContains('src-tauri/windows/installer-template.nsi', 'QuietUninstallString', 'silent uninstall support');
requireContains('src-tauri/windows/installer-template.nsi', 'VIAddVersionKey "Comments"', 'enhanced PE version info');
...
if (/\bnpm (ci|test|run)\b/.test(workflow)) fail.push('.github/workflows/ci.yml: still contains npm workflow commands');
