...
const metadataCodes = LANGUAGE_METADATA.map(item => item.value).sort();
const translationCodes = Object.keys(translations).sort();
if (metadataCodes.join(',') !== translationCodes.join(',')) {
  fail('Language metadata and translation map are out of sync');
}
...
console.log(`[i18n:validate] ${metadataCodes.length} languages validated with ${sourceKeys.length} keys each.`);
