...
requireContains('src-tauri/windows/hooks.nsi', 'DeleteRegKey SHCTX "Software\\Google\\Chrome\\NativeMessagingHosts\\${NOVA_NATIVE_HOST}"', 'Chrome native host cleanup');
requireContains('src-tauri/windows/hooks.nsi', 'DeleteRegKey SHCTX "Software\\Microsoft\\Edge\\NativeMessagingHosts\\${NOVA_NATIVE_HOST}"', 'Edge native host cleanup');
...
requireContains('scripts/build-tauri-assets.mjs', 'generateNativeMessagingManifest', 'native messaging manifest generation');
requireContains('.github/workflows/build.yml', 'pnpm run extension:package', 'extension build before installer packaging');
