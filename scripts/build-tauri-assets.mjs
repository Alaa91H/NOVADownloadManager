...
function generateNativeMessagingManifest() {
  ...
  if (existsSync(extensionManifestPath)) {
    try {
      ...
      throw new Error(`Could not read extension manifest for native messaging metadata: ${error.message}`);
    }
  }

  const chromiumOrigins = splitCsvEnv('NOVA_CHROMIUM_EXTENSION_IDS')
    .map((id) => id.replace(/^chrome-extension:\/\//, '').replace(/\/$/, '').trim())
  ...
  writeFileSync(join(NATIVE_MESSAGING_DIR, 'native-messaging-build.json'), JSON.stringify({
...
function copyBrowserExtensionIfPresent() {
  const outputRoot = join(ROOT, 'browser-extension');
  const candidates = [
    join(outputRoot, '.output', 'chrome-mv3'),
    join(outputRoot, '.output', 'chrome-mv3-dev'),
    join(outputRoot, 'dist', 'chrome-mv3'),
    join(outputRoot, 'dist', 'chrome'),
  ];
  const source = firstExistingDir(candidates);
  const target = join(RESOURCE_DIR, 'browser-extension');
  rmSync(target, { recursive: true, force: true });
  if (!source) return false;
  mkdirSync(target, { recursive: true });
  cpSync(source, target, { recursive: true });
  const copiedFiles = readdirSync(target);
  if (!copiedFiles.includes('manifest.json')) {
    rmSync(target, { recursive: true, force: true });
    throw new Error(`Browser extension output at ${source} does not contain manifest.json`);
  }
  console.log(`[tauri-assets] Bundled browser extension from ${source}`);
  return true;
}
...
function copyLegalNotices() {
  ...
  for (const notice of notices) {
    ...
    if (!copied) {
      throw new Error(`Required legal notice ${notice} is missing from the repository root; it must ship inside the installed application.`);
    }
  }
...
async function main() {
  ...
  const stampedVersion = JSON.parse(
  ...
  const manifest = {
    ...
    files: {
      ...
      nativeLibcurl: existsSync(join(RESOURCE_DIR, NATIVE_CURL_MANIFEST)) ? 'static-ci-built-libcurl' : 'cargo-fallback',
