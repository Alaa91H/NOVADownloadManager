...
// Stamps the release tag (for example v1.2.3 or v1.2.3+45) into every versioned
// project file so installers, the app, the daemon, and the browser extension all
// report the same version. Sources, in priority order: CLI argument, BUILD_TAG,
...
function lastNumericBuildPart(build) {
  return (
    build
      ?.split('.')
...
// 1.2.3.4 and v1.2.3+45 becomes 1.2.3.45 (same normalization as
// browser-extension/wxt.config.ts and build.py).
/** @param {string} version */
function toManifestVersion(version) {
  const [base = '', build = ''] = version.split('+', 2);
  if (/^\d+\.\d+\.\d+(\.\d+)?$/.test(base)) {
    const buildNumber = /^\d+\.\d+\.\d+$/.test(base) ? lastNumericBuildPart(build) : '';
    return buildNumber ? `${base}.${buildNumber}` : base;
  }
...
const targets = [
  ...
  [
    ...
    () => updateJsonVersion(join(ROOT, 'browser-extension', 'src', 'manifest.json'), manifestVersion),
  ],
];
