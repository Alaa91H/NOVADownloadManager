...
async function main() {
  ...
  // Copy engines
  if (existsSync(join(BIN_DIR, CURL_BINARY))) {
    copyFileSync(join(BIN_DIR, CURL_BINARY), join(bundleBin, CURL_BINARY));
  }
  if (existsSync(join(BIN_DIR, YTDLP_BINARY))) {
    copyFileSync(join(BIN_DIR, YTDLP_BINARY), join(bundleBin, YTDLP_BINARY));
  }
  if (existsSync(join(BIN_DIR, FFMPEG_BINARY))) {
    copyFileSync(join(BIN_DIR, FFMPEG_BINARY), join(bundleBin, FFMPEG_BINARY));
  }
  if (existsSync(MANIFEST_PATH)) {
    copyFileSync(MANIFEST_PATH, join(bundleBin, '.bin-manifest.json'));
  }
  if (existsSync(join(BIN_DIR, NATIVE_CURL_MANIFEST))) {
    copyFileSync(join(BIN_DIR, NATIVE_CURL_MANIFEST), join(bundleDist, NATIVE_CURL_MANIFEST));
  }
