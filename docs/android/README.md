...
The current code supplies a buildable native Android UI foundation, validated HTTP(S) ingestion, shared Rust models/range planning, a versioned native bridge, and a real app-private transfer path through `nova-mobile-core -> nova-download-core -> libcurl`. Android 14+ now owns long-running user-started execution through UIDT, earlier devices use a foreground WorkManager fallback, and progress notifications expose native pause/cancel plus a user-visible resume handoff. SAF/MediaStore output, generated high-level UniFFI task bindings, and physical-device compatibility evidence remain pending.
