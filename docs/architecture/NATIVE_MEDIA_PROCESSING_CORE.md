...
- streaming native WebM/Matroska muxing with bounded packet writes, explicit
  BlockDuration values, DTS-based Clusters, PTS composition offsets, final
  Duration patching, and bounded generated Cues for seekable keyframes;
- an EBML interleaver that merges multiple demuxers by decode time and writes
  audio before video when timestamps tie;
...
`mp4_demux`, `fragmented_mp4_demux`, `webm_demux`, `webm_mux`,
`matroska_demux`, and `matroska_mux` are enabled. EBML container sniffing validates the DocType
...
currently accepts VP8, VP9, AV1, and Opus. Matroska output additionally accepts
H.264, HEVC, AAC, MP3, and FLAC. VP9 vpcC metadata is normalized back to WebM
CodecPrivate feature records and Colour metadata, Opus dOps is converted back
to OpusHead with CodecDelay and 80 ms SeekPreRoll, and MP4 AAC esds is reduced
to Matroska AudioSpecificConfig.
