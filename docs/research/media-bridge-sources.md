# external media resolver Installation Sources and Channel Policy

**Last verified:** 2026-08-21 (GMT+2)
**Purpose:** Document the upstream facts that govern NOVA-managed external media resolver installation and YouTube format discovery.

## Authoritative upstream facts

The official external media resolver project publishes native release binaries for all NOVA targets, including `external media resolver.exe` for Windows x64, `external media resolver_linux`, `external media resolver_linux_aarch64`, and `external media resolver_macos`. GitHub release metadata for the selected official asset supplies a SHA-256 digest. NOVA obtains only an HTTPS asset URL from the official GitHub release, verifies the asset against that published digest before installation, applies the executable atomically, and performs a post-install health check.

Upstream documents three binary channels: `stable`, `nightly`, and `master`. It also states that `stable` may become stale when websites change and recommends `nightly` for regular users. Because YouTube changes independently of desktop-release schedules, NOVA's managed installer uses the official **external media resolver nightly** release channel by default. The nightly source remains restricted to `external media resolver/external media resolver-nightly-builds`, and the same GitHub-host allow-list, SHA-256 verification, atomic replacement, registry ownership record, and health validation apply before a tool becomes available to a task.

> NOVA does not invoke external media resolver's self-updater for managed installations. Every installation or update remains inside NOVA's verified acquisition and replacement workflow.

The official FAQ explains that higher-quality YouTube formats commonly contain separate audio and video tracks, requiring FFmpeg to merge them. NOVA therefore sends the selected video `formatId` as `formatId+bestaudio/best` when the selected format has video but no audio. It uses the stable watch URL, rather than an expiring `googlevideo` stream URL, so external media resolver resolves fresh media URLs when the managed task starts.
...
A normal desktop process cannot reliably write to `C:\Program Files` without elevation. The default NOVA-managed per-user destination is therefore writable without elevated privileges, while a machine-wide `C:\Program Files\External Media Resolver` install requires an explicit elevated flow. The backend must never silently select another path while reporting that it installed to Program Files.

## Sources

1. [external media resolver README: release channels, binaries, FFmpeg, and external media resolver-ejs](https://github.com/yt-dlp/yt-dlp#installation)
2. [external media resolver FAQ: cookies, same-IP access, headers, and FFmpeg merging](https://github.com/yt-dlp/yt-dlp/wiki/FAQ)
3. [Official external media resolver nightly releases](https://github.com/yt-dlp/yt-dlp-nightly-builds/releases)
4. [external media resolver public signing key](https://github.com/yt-dlp/yt-dlp/blob/master/public.key)
